...
namespace CMS.EmailEngine
{
    ...
    internal sealed class SMTPServerLookupTable
    {
        ...
        private SMTPServerLookupTable()
        {
            ...
            // Register Settings observer to notify the lookup table about changed SMTP server settings
            string[] settingsToWatch = { "CMSSMTPServer", "CMSSMTPServerUser", "CMSSMTPServerPassword", "CMSUseSSL" };

            SettingsKeyInfoProvider.OnSettingsKeyChanged += (sender, e) =>
            {
                if (settingsToWatch.Contains(e.KeyName))
                {
                    Flush();
                }
            };
        ...
        {
            bool isGlobal = siteName.EqualsCSafe(GLOBAL, true);
        ...
        {
            string site = !siteName.EqualsCSafe(GLOBAL, true) ? siteName : String.Empty;
