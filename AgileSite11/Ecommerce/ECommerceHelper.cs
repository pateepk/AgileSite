...
namespace CMS.Ecommerce
{
    ...
    public static class ECommerceHelper
    {
        ...
        public static decimal GetTableColumnSum(DataTable table, string column)
        {
            // Return zero if table is empty or column not found
            if (DataHelper.DataSourceIsEmpty(table) || !table.Columns.Contains(column))
            {
                return 0;
            }

            return table.Rows.Cast<DataRow>().Sum(row => ValidationHelper.GetDecimal(row[column], 0m));
        }
        ...
        [Obsolete("Method was not intended for public use and will be removed in the next version.")]
        public static void DeleteOrDisable(BaseInfo info)
        {
            if (info == null)
            {
                return;
            ...
            var delete = false;
            var general = info.Generalized;
            if (!general.CheckDependencies(true))
            {
                // No direct dependencies
                delete = true;

                // Check bindings
                if ((general.OtherBindings != null) && general.OtherBindings.Any(binding => binding.Any()))
                {
                    delete = false;
                }
            }
            ...
            {
                // Try disable object
                var enabledColumn = info.TypeInfo.EnabledColumn;
                if (enabledColumn != ObjectTypeInfo.COLUMN_NAME_UNKNOWN)
                {
                    general.SetValue(enabledColumn, false);
                    general.SetObject();
                }
            }
        }
        ...
        public static void TrackAddToShoppingCartConversion(ShoppingCartItemInfo product, string name = null)
        {
            ...
            // Prepare resolver
            MacroResolver mr = ShoppingCartItemInfoProvider.GetShoppingCartItemResolver(product);

            // Get conversion data
            string siteName = product.ShoppingCart.SiteName;
            if (string.IsNullOrEmpty(name))
            {
                name = ECommerceSettings.AddToCartConversionName(siteName);
            }
            string value = ECommerceSettings.AddToCartConversionValue(siteName);
        ...
        private static void TrackConversion(string objectName, double value, int count, string siteName)
        {
            // Do not process
