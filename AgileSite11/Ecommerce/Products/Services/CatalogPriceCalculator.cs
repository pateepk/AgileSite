...
namespace CMS.Ecommerce
{
    ...
    internal class CatalogPriceCalculator : ICatalogPriceCalculator
    {
        ...
        public CatalogPriceCalculator(IProductPricingService pricingService, ICatalogTaxCalculator taxCalculator)
        {
        ...
        {
            result.StandardPrice += accessoryPrices.Sum(p => p.StandardPrice);
            result.Price += accessoryPrices.Sum(p => p.Price);
            result.Tax += accessoryPrices.Sum(p => p.Tax);
        ...
        private ProductCatalogPrices GetProductPrices(SKUInfo sku, PriceParameters priceParameters, TaxCalculationParameters taxParameters, IEnumerable<SKUInfo> attributeAndTextOptions = null)
        {
            ...
            var result = new ProductCatalogPrices(price, standardPrice, prices.AppliedDiscounts)
            {
                Tax = tax
            };
