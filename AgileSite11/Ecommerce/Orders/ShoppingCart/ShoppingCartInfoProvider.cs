...
namespace CMS.Ecommerce
{
    ...
    public class ShoppingCartInfoProvider : AbstractInfoProvider<ShoppingCartInfo, ShoppingCartInfoProvider>
    {
        ...
        /// <exception cref="InvalidOperationException">Thrown when ShoppingCartCustomerID and/or ShoppingCartCurrencyID not set.</exception>
        ...
        protected virtual ShoppingCartInfo GetShoppingCartInfoFromOrderInternal(int orderId)
        {
            ...
            cart.ShoppingCartBillingAddress = orderObj.OrderBillingAddress;
            cart.ShoppingCartShippingAddress = orderObj.OrderShippingAddress;
            cart.ShoppingCartCompanyAddress = orderObj.OrderCompanyAddress;
        ...
        protected virtual void SetOrderInternal(ShoppingCartInfo cart, bool generateInvoice)
        {
            ...
            // Get order object
            OrderInfo oi = OrderInfoProvider.GetOrderInfo(cart.OrderId) ?? new OrderInfo();

            // Set order base parameters
            oi.OrderCustomerID = cart.ShoppingCartCustomerID;
            oi.OrderCurrencyID = cart.ShoppingCartCurrencyID;
            oi.OrderBillingAddress = cart.ShoppingCartBillingAddress;
            oi.OrderPaymentOptionID = cart.ShoppingCartPaymentOptionID;
            oi.OrderShippingAddress = cart.ShoppingCartShippingAddress;

            // Clone billing address and set to shipping address if shipping is needed and billing and shipping are equal
            if ((oi.OrderShippingAddress == null) && cart.IsShippingNeeded)
            {
                oi.OrderShippingAddress = OrderAddressInfoProvider.CreateOrderAddressInfo(oi.OrderBillingAddress);
            }

            oi.OrderShippingOptionID = cart.ShoppingCartShippingOptionID;
            oi.OrderSiteID = cart.ShoppingCartSiteID;
            oi.OrderNote = cart.ShoppingCartNote;
            oi.OrderCouponCodes = cart.CouponCodes.Serialize();
            oi.OrderCompanyAddress = cart.ShoppingCartCompanyAddress;
            oi.OrderCustomData.LoadData(cart.ShoppingCartCustomData.GetData());
            oi.OrderCulture = cart.ShoppingCartCulture;
            ...
            // Set order totals
            oi.OrderGrandTotal = cart.GrandTotal;
            oi.OrderTotalPrice = cart.TotalPrice;
            oi.OrderTotalShipping = cart.TotalShipping;
            oi.OrderTotalTax = cart.TotalTax;
            oi.OrderTaxSummary = cart.TaxSummary?.GetSummaryXml();
            ...
            oi.OrderGrandTotalInMainCurrency = roundingService.Round(CurrencyConverter.ApplyExchangeRate(cart.GrandTotal, rateToMainCurrency), mainCurrency);
            oi.OrderTotalPriceInMainCurrency = roundingService.Round(CurrencyConverter.ApplyExchangeRate(cart.TotalPrice, rateToMainCurrency), mainCurrency);

            // Set order custom parameters
            foreach (var columnName in cart.ColumnNames)
            {
                oi.SetValue(columnName, cart.GetValue(columnName));
            }

            if (newOrder)
            {
                // Set order user
                oi.OrderCompletedByUserID = cart.ShoppingCartUserID;
            }
            ...
            using (var tr = BeginTransaction())
            {
                ...
                // Other payments
                oi.OrderCouponCodes = cart.CouponCodes.Serialize();
                oi.OrderOtherPayments = cart.OtherPaymentsSummary?.GetSummaryXml();
                oi.OrderDiscounts = cart.OrderDiscountSummary?.GetSummaryXml();

                // Create order
                OrderInfoProvider.SetOrderInfo(oi);

                // Update order ID in shopping cart
                cart.OrderId = oi.OrderID;
                cart.Order = oi;
                ...
                if (newOrder)
                {
                    oi.OrderInvoiceNumber = OrderInfoProvider.GenerateInvoiceNumber(cart);

                    // Set order status after creation of order items to ensure that status changed notification email has all data                    
                    oi.OrderStatusID = status?.StatusID ?? 0;
                }

                if (generateInvoice)
                {
                    oi.OrderInvoice = GetOrderInvoice(cart);
                }
            ...
            if (newOrder && (oi.OrderID > 0))
            {
                EcommerceEvents.NewOrderCreated.StartEvent(oi);
            }
        }
        ...
        {
            // Do not process
            if ((itemParams == null) || (itemParams.Quantity <= 0) || (itemParams.SKUID <= 0))
            {
                return null;
            }
