                    ...
                    if (indexInfo != null)
                    {
                        if (indexInfo?.IndexSettings?.Items?.Count > 0)
                        {
                            ...
                            foreach (Guid key in keys)
                            {
                                ...
                                if (sis != null)
                                {
                                    if (CMSString.Compare(ValidationHelper.GetString(sis.ClassNames, String.Empty), currentItemClassName, true) == 0)
                                    {
                                        whereCondition = sis.WhereCondition;
                                        indexFound = true;
                                        break;
                                    }
                                }
                            ...
                            if (indexFound)
                            {
                                whereCondition = SqlHelper.AddWhereCondition($"{sti.SearchTaskField} = {itemId}", whereCondition);

                                var items = CustomTableHelper.GetSearchDocuments(indexInfo, currentItemClassName, whereCondition, 0);
        ...
        public override void Rebuild(SearchIndexInfo indexInfo)
        {
            ...
            if (indexInfo?.IndexSettings?.Items?.Count > 0)
            {
                ...
                foreach (Guid key in keys)
                {
                    // Get settings info
                    SearchIndexSettingsInfo sis = indexInfo.IndexSettings.Items[key];
                    string[] queryWhere = new[] { sis.ClassNames, sis.WhereCondition };
            ...
            if (customTables.Count > 0)
            {
                ...
                foreach (DictionaryEntry entry in customTables)
                {
                    ...
                    if (!DataHelper.DataSourceIsEmpty(tableDs))
                    {
                        ...
                        {
                            result[dr[ti.IDColumn] + ";" + className.ToLowerCSafe() + "_" + CustomTableInfo.OBJECT_TYPE_CUSTOMTABLE] = dr;
                        }
                    }
                }
            }
