...
namespace CMS.Community
{
    ...
    public class CommunityContext : AbstractContext<CommunityContext>
    {
        ...
        private UserInfo mCurrentFriend;
        private FriendInfo mCurrentFriendship;
        ...
        /// </summary>
        public static UserInfo CurrentFriend
        {
            get
            {
                return GetCurrentFriend();
            }
            set
            {
                Current.mCurrentFriend = value;
            }
        }
        ...
        /// </summary>
        public static FriendInfo CurrentFriendship
        {
            get
            {
                return GetCurrentFriendship();
            }
            set
            {
                Current.mCurrentFriendship = value;
            }
        }
        ...
        {
            // Try to get the user info from the request items collection
            var c = Current;

            UserInfo ui = c.mCurrentFriend;
            if (ui == null)
            {
                FriendInfo fi = null;
            
                // Try to get user by friendship FriendID first
                int friendId = QueryHelper.GetInteger("friendid", 0);
                if (friendId > 0)
                {
                    fi = FriendInfoProvider.GetFriendInfo(friendId);
                }

                // If friendship was not found by its FriendID
                if (fi == null)
                {
                    // Try to get friend user by its GUID
                    Guid guid = QueryHelper.GetGuid("friendguid", Guid.Empty);
                    if (guid != Guid.Empty)
                    {
                        fi = FriendInfoProvider.GetFriendInfo(guid);
                    }
                }
                ...
                if (fi != null)
                {
                    int friendUserId = (fi.FriendUserID == MembershipContext.AuthenticatedUser.UserID) ? fi.FriendRequestedUserID : fi.FriendUserID;
                    ui = UserInfoProvider.GetUserInfo(friendUserId);
                }

                // Save the user to the request items if new user was loaded from DB
                c.mCurrentFriend = ui;
            }
        ...
        public static FriendInfo GetCurrentFriendship()
        {
            ...
            FriendInfo fi = c.mCurrentFriendship;
            if (fi == null)
            {
                // Try to get friendship ID
                int friendId = QueryHelper.GetInteger("friendid", 0);
                if (friendId > 0)
                {
                    fi = FriendInfoProvider.GetFriendInfo(friendId);
                ...
                if (fi == null)
                {
                    // Try to get friendship by its GUID
                    Guid guid = QueryHelper.GetGuid("friendguid", Guid.Empty);
                    if (guid != Guid.Empty)
                    {
                        fi = FriendInfoProvider.GetFriendInfo(guid);
                ...
                // Save the friendship to the request items if new friendship was loaded from DB
                c.mCurrentFriendship = fi;
            }
            return fi;
        }
