...
namespace CMS.Community
{
    ...
    public class GroupMemberInfoProvider : AbstractInfoProvider<GroupMemberInfo, GroupMemberInfoProvider>
    {
        ...
        [Obsolete("Use CMS.DataEngine.ObjectQuery<GroupMemberInfo> instead")]
        public static TypedDataSet GetGroupMembers(string where, string orderBy, int topN, string columns)
        {
            var query = GetGroupMembers().Where(where)
                                         .OrderBy(orderBy)
                                         .TopN(topN)
                                         .Columns(columns)
                                         .BinaryData(true);
        ...
        protected virtual GroupMemberInfo GetGroupMemberInfoInternal(int userId, int groupId)
        {
            if ((userId > 0) && (groupId > 0))
            {
                ...
                return GetGroupMembers().Where(where).TopN(1).FirstObject;
            }
