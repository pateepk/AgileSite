...
namespace CMS.OnlineForms
{
    ...
    public static class BizFormImport
    {
            ...
            foreach (DataRow dr in table.Rows)
            {
                ...
                try
                {
                    ...
                    if (ValidationHelper.GetBoolean(settings.GetSettings(ImportExportHelper.SETTINGS_BIZFORM_DATA), true))
                    {
                        ...
                        if (bizForm == null)
                        {
                            throw new NullReferenceException("[ImportProvider.ImportBizFormsData]: Existing BizForm '" + bizFormName + "' not found.");
                        }
                        ...
                        if (dataClass == null)
                        {
                            throw new Exception("[ImportProvider.ImportBizFormsData]: Class name for BizForm '" + bizFormName + "' not found.");
                        }
                        ...
                        {
                            AddImportFiles(settings, ds, bizFormName, dataClass);
                        }
                    }
        ...
        private static void AddImportFiles(SiteImportSettings settings, DataSet data, string formName, DataClassInfo dataClass)
        {
            // Get file columns
            string[] fileColumns = BizFormInfoProvider.GetBizFormFileColumns(dataClass.ClassFormDefinition);
            if (fileColumns == null)
            {
                return;
            }
