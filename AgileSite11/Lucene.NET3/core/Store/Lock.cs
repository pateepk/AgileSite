...
namespace Lucene.Net.Store
{
	...
	public abstract class Lock
	{
        ...
        /// Lock poll interval
        /// </summary>
        private static long? mLockPollInterval = null;


        /// <summary>
        /// Gets lock poll interval.
        /// </summary>
        private static long LockPollInterval
        {
            get
            {
                if (mLockPollInterval == null)
                {
                    string settingValue = ConfigurationManager.AppSettings["CMSSmartSearchLockPollInterval"];
                    mLockPollInterval = string.IsNullOrEmpty(settingValue) ? LOCK_POLL_INTERVAL : Convert.ToInt64(settingValue);
                }

                return mLockPollInterval.Value;
            }
        }
		...
		public virtual bool Obtain(long lockWaitTimeout)
		{
			...
			long maxSleepCount = lockWaitTimeout / LockPollInterval;
			long sleepCount = 0;
			while (!locked)
			{
				...
				try
				{
					System.Threading.Thread.Sleep(TimeSpan.FromMilliseconds(LockPollInterval));
				}
