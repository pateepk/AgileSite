...
namespace WorldNet.Net
{
	...
	public class Syns2Index
	{
		...
		public static void  Main(System.String[] args)
		{
			...
			Console.WriteLine("Opening Prolog file " + prologFilename);
			...
			var row = 1;
			// parse prolog file
			Console.WriteLine("[1/2] Parsing " + prologFilename);
			while ((line = br.ReadLine()) != null)
			{
				...
				{
					mod *= 2;
					Console.WriteLine("\t" + row + " " + line + " " + word2Nums.Count + " " + num2Words.Count + " ndecent=" + ndecent);
				}
			...
			// create the index
			Console.WriteLine("[2/2] Building index to store synonyms, " + " map sizes are " + word2Nums.Count + " and " + num2Words.Count);
			...
			using (var dir = FSDirectory.Open(new DirectoryInfo(indexDir)))
			{
				...
				while (i1.MoveNext())
				{
					...
					if (n > 0)
					{
						...
						if ((++row % mod) == 0)
						{
							Console.WriteLine("\trow=" + row + "/" + word2Nums.Count + " doc= " + doc);
							mod *= 2;
						}
