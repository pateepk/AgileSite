...
namespace CMS.Newsletters
{
    ...
    public static class EmailQueueManager
    {
        ...
        /// <param name="sendNew">Indicates if new emails should be sent.</param>
        ...
        public static void SendAllEmails(bool sendFailed = true, bool sendNew = true, int issueId = 0)
        {
            // Check license
        ...
        private static SubscriberInfo GetSubscriberMember(EmailQueueItemInfo email)
        {
            ...
            var member = SubscriberInfoProvider.CreateSubscriberFromContact(email.EmailContactID, source, "ContactFirstName", "ContactLastName");
            if (member != null)
            {
        ...
        internal static DataSet FetchEmailsToSend(bool fetchFailed, bool fetchNew, int firstEmailId, int issueId, int topN)
        {
            return FetchEmailsToSendInternal(fetchFailed, fetchNew, firstEmailId, Guid.Empty, issueId, topN);
        }
        ...
        /// <param name="issueId">If set, only e-mails of specified issue will be fetched</param>
        /// <param name="topN">Number of e-mails to fetch</param>
        internal static DataSet FetchVariantEmailsToSend(bool fetchFailed, bool fetchNew, Guid firstEmailGuid, int issueId, int topN)
        {
            return FetchEmailsToSendInternal(fetchFailed, fetchNew, -1, firstEmailGuid, issueId, topN);
        }
        ...
        /// <param name="issueId">If set, only e-mails of specified issue will be fetched</param>
        /// <param name="topN">Number of e-mails to fetch</param>
        private static DataSet FetchEmailsToSendInternal(bool fetchFailed, bool fetchNew, int firstEmailId, Guid firstEmailGuid, int issueId, int topN)
        {
            ...
            using (var h = NewsletterEvents.GenerateQueueItems.Start(issue, subscriber, emailAddressBlocker))
            {
                if (h.CanContinue())
                {
                    if (isContactSubscriber)
                    {
                        ...
                        var subscriberMember = SubscriberInfoProvider.CreateSubscriberFromContact(subscriber.SubscriberRelatedID, subscriber, "ContactEmail");
                    ...
                    else if (subscriber.SubscriberType.Equals(PredefinedObjectType.CONTACTGROUP, StringComparison.InvariantCultureIgnoreCase))
                    {
                        ...
                        FillContactGroupTable(data, issue, monitorBounces, bounceLimit, subscriber.SubscriberRelatedID, subscriber.SubscriberID, null, false, emailAddressBlocker);
        ...
        private static void GenerateContactEmails(IssueInfo issue, SiteInfo site, bool monitorBounces, int bounceLimit, HashSet<string> generatedEmails, IEmailAddressBlocker emailAddressBlocker)
        {
            ...
            {
                FillContactTable(data, issue, site.SiteID, monitorBounces, bounceLimit, generatedEmails, emailAddressBlocker);
            }
            else
            {
        ...
        /// <param name="emailAddressBlocker">Every email will be checked using this email address blocker before it is added to the email queue.</param>
        private static void FillContactTable(DataTable table, IssueInfo issue, int siteId, bool monitorBounces, int bounceLimit, HashSet<string> generatedEmails, IEmailAddressBlocker emailAddressBlocker)
        {
            var where = new WhereCondition().WhereEquals("NewsletterID", issue.IssueNewsletterID)
            ...
            // Get IDs of all subscribed contacts
            DataSet members = SubscriberNewsletterInfoProvider.GetSubscriptions(where.ToString(true), null, -1, "SubscriberID, SubscriberRelatedID AS ContactID");
            if (!DataHelper.DataSourceIsEmpty(members))
            {
                ...
                List<string> contactIds = DataHelper.GetUniqueValues(members.Tables[0], "ContactID", false);
                ...
                do
                {
                    ...
                    List<string> tempIds = contactIds.GetRange(0, maxCount);
                    ...
                    if (!DataHelper.DataSourceIsEmpty(contacts))
                    {
                        ...
                        DataTableReader reader = new DataTableReader(contacts.Tables[0]);
                ...
                foreach (DataRow contact in members.Tables[0].Rows)
                {
                    ...
                    int subscriberId = ValidationHelper.GetInteger(contact["SubscriberID"], 0);
                    int contactId = ValidationHelper.GetInteger(contact["ContactID"], 0);
                    string email = ValidationHelper.GetString(contact["ContactEmail"], string.Empty);
            ...
            members = SubscriberNewsletterInfoProvider.GetSubscriptions(string.Format("NewsletterID={0} AND (SubscriptionApproved=1 OR SubscriptionApproved IS NULL) AND SubscriberType='om.contactgroup'", issue.IssueNewsletterID), null, -1, "SubscriberID,SubscriberRelatedID");
            if (DataHelper.DataSourceIsEmpty(members))
            {
                return;
            }
            ...
            foreach (DataRow group in members.Tables[0].Rows)
            {
                ...
                int subscriberId = ValidationHelper.GetInteger(group[0], 0);
                int contactGroupId = ValidationHelper.GetInteger(group[1], 0);

                FillContactGroupTable(table, issue, monitorBounces, bounceLimit, contactGroupId, subscriberId, generatedEmails, true, emailAddressBlocker);
            }
        }
        ...
        /// <param name="emailAddressBlocker">Every email will be checked using this email address blocker before it is added to the email queue.</param>
        private static void FillContactGroupTable(DataTable table, IssueInfo issue, bool monitorBounces, int bounceLimit, int contactGroupId, int subscriberId, HashSet<string> currentEmails, bool setSending, IEmailAddressBlocker emailAddressBlocker)
        {
            ...
            int siteId = issue.IssueSiteID;
            int issueId = issue.IssueIsVariant ? issue.IssueVariantOfIssueID : issue.IssueID;

            // Get IDs and emails of all valid contact group members
            DataSet contacts = SubscriberInfoProvider.GetContactGroupMembers(contactGroupId, monitorBounces, bounceLimit, -1, "ContactID,ContactEmail");
            if (DataHelper.DataSourceIsEmpty(contacts))
            {
                return;
            }
            ...
            foreach (DataRow contact in contacts.Tables[0].Rows)
            {
                ...
                int contactId = ValidationHelper.GetInteger(contact[0], 0);
                string email = ValidationHelper.GetString(contact[1], string.Empty);
        ...
        public static int GenerateDynamicIssue(int newsletterId)
        {
            ...
            var resolvedUrl = GetDynamicNewsletterUrl(newsletter);
            ...
            {
                throw new Exception(string.Format("{0}(URL: {1}, RESOLVED URL: {2})", ex.Message, newsletter.NewsletterDynamicURL, resolvedUrl));
            }
        ...
        private static string GetDynamicNewsletterUrl(NewsletterInfo newsletter)
        {
            if (newsletter == null)
            {
