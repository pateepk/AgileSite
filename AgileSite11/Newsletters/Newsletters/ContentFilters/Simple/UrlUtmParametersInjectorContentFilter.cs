...
namespace CMS.Newsletters.Filters
{
    ...
    internal sealed class UrlUtmParametersInjectorContentFilter : IEmailContentFilter
    {
        ...
        private static readonly Regex hyperlinkRegex = RegexHelper.GetRegex(@"(?<prefix><a[\s|\S]*?href(\s)*?=(\s)*?)(?<quote>[""'])(?<url>.*?)\k<quote>(?<suffix>[\s|\S]*?>)");
        ...
        public string Apply(string text)
        {
            ...
            return hyperlinkRegex.Replace(text, (m) =>
            {
                ...
                if (IsDomainLink(link, baseLink, site.DomainName, aliases))
                {
        ...
        public bool IsDomainLink(Link link, Link baselink, string domainName, IList<string> domainAliases)
        {
            ...
            return baseUrlDomain == linkDomain || baselink.ToString() == linkDomain || domainName == linkDomain || domainAliases.Any(linkDomain.Equals);
        }
