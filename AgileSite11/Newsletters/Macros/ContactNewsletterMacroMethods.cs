        ...
        private static bool InteractedWithNewsletterIssue(object contact, Guid issueGuid, bool click, int lastXDays)
        {
            ...
            if (contactInfo != null)
            {
                var issueIds = IssueInfoProvider.GetIssues()
                                                ...
                                                .Or()
                                                .WhereIn("IssueVariantOfIssueID", IssueInfoProvider.GetIssues()
                                                    .Column("IssueID")
                                                    .WhereEquals("IssueGUID", issueGuid))
                                                .Column("IssueID");
                ...
                if (issueIds.Count > 0)
                {
                    var where = ActivityInfo.TYPEINFO.CreateWhereCondition().WhereIn("ActivityItemDetailID", issueIds).ToString(true);
                    var activityType = click ? PredefinedActivityType.NEWSLETTER_CLICKTHROUGH : PredefinedActivityType.NEWSLETTER_OPEN;

                    return ActivityInfoProvider.ContactDidActivity(contactInfo.ContactID, activityType, null, lastXDays, where);
                }
            }
