    ...
    public class OnSiteEditHelper
    {
        ...
        public static string GetOnSiteEditStartTag(OnSiteEditStartTagConfiguration tagConfig)
        {
            ...
            if (PortalContext.CurrentUserIsDesigner || (userIsDocumentEditor && !string.IsNullOrEmpty(tagConfig.EditUrl) && tagConfig.WebPartIsEditable))
            {
                ...
                {
                    // Portal engine mode
                    tagBuilder.Append(" id=\"OE_", pageInfo.DocumentID, "_", webPartInstance.ControlID, "\" data-title=\"", HTMLHelper.HTMLEncode(!string.IsNullOrEmpty(tagConfig.WebPartTitle) ? tagConfig.WebPartTitle : webPartInstance.ControlID), "\"");
                ...
                else
                {
                    ...
                    if (tagConfig.ControlObject is CMSAbstractEditableControl)
                    {
                        ...
                        tagBuilder.AppendFormat(@" id=""OE_{0}_{1}"" data-title=""{1}""", pageInfo.DocumentID, controlId);
