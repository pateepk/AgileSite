...
namespace CMS.DancingGoat.Samples
{
    ...
    public sealed class EcommerceSampleDataGenerator : ISampleDataGenerator
    {
        ...
        private void GenerateEcommerceData(int siteID)
        {
            ...
            var currency = CurrencyInfoProvider.GetCurrencies(siteID).Where("CurrencyIsMain", QueryOperator.Equals, 1).TopN(1).FirstObject;
            ...
            for (int i = 0; i <= 30; i++)
            {
                ...
                for (int j = 0; j < ((days / 2) + adjustNumberOfOrders); j++)
                {
                    ...
                    var oi = new OrderInfo
                    {
                        ...
                        OrderBillingAddress = GenerateOrderAddress(GetRandomCountryID()),
                        OrderShippingAddress = GenerateOrderAddress(GetRandomCountryID()),
                        OrderSiteID = siteID,
                    ...
                    // Order must be set before creating order items and setting order date
        ...
        private OrderAddressInfo GenerateOrderAddress(int countryID)
        {
