...
namespace CMS.DancingGoat.Samples
{
    ...
    internal class NewslettersDataGenerator
    {
        ...
        {
            var lesson1 = IssueInfoProvider.GetIssues().WhereEquals("IssueSubject", "Coffee 101 - Lesson 1").FirstOrDefault();
            var lesson2 = IssueInfoProvider.GetIssues().WhereEquals("IssueSubject", "Coffee 101 - Lesson 2").FirstOrDefault();
            var campaignEmail = IssueInfoProvider.GetIssues().WhereEquals("IssueSubject", "Get a free Colombia coffee sample today").FirstOrDefault();
