...
namespace CMS.Personas.Web.UI
{
    ...
    /// </summary>
    internal sealed class PersonasUniSelectorExtender : ControlExtender<UniSelector>
    {
        private readonly IPersonaPictureImgTagGenerator mPersonaPictureImgTagGenerator =
            PersonasFactory.GetPersonaPictureImgTagGenerator();
        ...
        private object SetPersonaImageAndDisplayName(object sender, string sourceName, object parameter, object value)
        {
            switch (sourceName.ToLowerCSafe())
            {
                ...
                case "itemname":
                {
                    ...
                    if (imgTag != null)
                    {
                        btn.Text = imgTag + "<span class=\"personas-table-persona-name\">" +
                                   HTMLHelper.HTMLEncode(persona.PersonaDisplayName) + "</span>";
                    }
                    return btn;
                }
            }
