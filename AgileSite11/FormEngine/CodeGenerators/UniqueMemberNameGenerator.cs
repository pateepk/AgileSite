...
namespace CMS.FormEngine
{
    ...
    internal class UniqueMemberNameGenerator
    {
        ...
        public string GetUniqueMemberName(string fieldName, bool removePrefix)
        {
            ...
            {
                var fieldNameWithoutPrefix = fieldName.Substring(mFieldNamePrefix.Length);

                // Keep the prefix if the name starts with a number, since member name must begin with an alphabetic character or an underscore
                if (!Char.IsNumber(fieldNameWithoutPrefix[0]))
                {
        ...
        {
            if (String.IsNullOrEmpty(fieldName) || String.IsNullOrEmpty(mFieldNamePrefix) || mFieldNamePrefix.Length >= fieldName.Length)
            {
                return false;
            }

            return fieldName.StartsWith(mFieldNamePrefix, StringComparison.InvariantCultureIgnoreCase);
        }
