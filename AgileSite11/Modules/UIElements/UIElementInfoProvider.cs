...
namespace CMS.Modules
{
    ...
    public class UIElementInfoProvider : AbstractInfoProvider<UIElementInfo, UIElementInfoProvider>, IFullNameInfoProvider, IRelatedObjectCountProvider
    {
        ...
        [Obsolete("Use object query via GetUIElements() instead.")]
        public static TypedDataSet GetUIElements(string where, string orderBy)
        {
            return GetUIElements(where, orderBy, 0, null);
        }
        ...
        [Obsolete("Use object query via GetUIElements() instead.")]
        public static TypedDataSet GetUIElements(string where, string orderBy, int topN, string columns)
        {
            return ProviderObject.GetUIElementsInternal(where, orderBy, topN, columns);
        }
        ...
        public static TypedDataSet GetUIMenuElements(string resourceName)
        {
            return ProviderObject.GetUIMenuElementsInternal(resourceName);
        }
        ...
        [Obsolete("Method was not intended for public use and will be removed in the next version.")]
        public static TypedDataSet GetUIMenuElements(string resourceName, string elementName)
        {
            return ProviderObject.GetUIMenuElementsInternal(resourceName, elementName);
        }
        ...
        /// <param name="columns">Data columns to return</param>
        [Obsolete("Method was not intended for public use and will be removed in the next version.")]
        public static ObjectQuery<UIElementInfo> GetChildUIElements(int parentElementId, string columns)
        {
            return ProviderObject.GetChildUIElementsInternal(parentElementId, columns);
        }
        ...
        [Obsolete("Method was not intended for public use and will be removed in the next version.")]
        public static UIElementInfo GetChildUIElement(int parentElementID, String additionalWhere)
        {
            return ProviderObject.GetChildUIElementInternal(parentElementID, additionalWhere);
        }
        ...
        [Obsolete("Method was not intended for public use and will be removed in the next version.")]
        public static TypedDataSet GetResourceUIElements(int resourceId)
        {
            return ProviderObject.GetResourceUIElementsInternal(resourceId);
        }
        ...
        /// <param name="elementID">Element ID</param>
        [Obsolete("Method was not intended for public use and will be removed in the next version.")]
        public static UIElementInfo GetFirstChildElement(int elementID)
        {
            return ProviderObject.GetFirstChildElementInternal(elementID);
        }
        ...
        /// <param name="elementID">Element ID</param>
        [Obsolete("Method was not intended for public use and will be removed in the next version.")]
        protected virtual UIElementInfo GetFirstChildElementInternal(int elementID)
        {
            return GetChildUIElements(elementID).TopN(1).FirstObject;
        }
        ...
        /// <param name="columns">Data columns to return</param>
        [Obsolete("Use object query via GetUIElements() instead.")]
        protected virtual TypedDataSet GetUIElementsInternal(string where, string orderBy, int topN, string columns)
        {
            return GetUIElements().Where(where).OrderBy(orderBy).TopN(topN).Columns(columns).BinaryData(true).TypedResult;
        }
        ...
        /// <param name="resourceName">Name of the resource (module)</param>
        [Obsolete("Method was not intended for public use and will be removed in the next version.")]
        protected virtual TypedDataSet GetUIMenuElementsInternal(string resourceName)
        {
            return GetUIMenuElementsInternal(resourceName, String.Empty);
        }
        ...
        /// <param name="elementName">Name of the element</param>
        [Obsolete("Method was not intended for public use and will be removed in the next version.")]
        protected virtual TypedDataSet GetUIMenuElementsInternal(string resourceName, string elementName)
        {
            // Default module level for UI 
            int level = 2;
            String parentWhere = String.Empty;
            String resourceWhere = String.Format(" ElementResourceID IN (SELECT ResourceID FROM CMS_Resource WHERE ResourceName = N'{0}') ", SqlHelper.GetSafeQueryString(resourceName, false));

            if (!String.IsNullOrEmpty(elementName))
            {
                UIElementInfo ui = GetUIElementInfo(resourceName, elementName);
                if (ui != null)
                {
                    // Add one from root element and one to skip groups
                    level = ui.ElementLevel + 2;
                    parentWhere = " AND (ElementParentID IN (SELECT ElementID FROM CMS_UIElement WHERE ElementParentID =" + ui.ElementID + "))";
                }
                resourceWhere = String.Empty;
            }

            string where = "(ElementLevel = " + level + ")" + parentWhere;
            where = SqlHelper.AddWhereCondition(where, resourceWhere);

            string columns = "* ,(SELECT PUI.ElementOrder FROM CMS_UIElement AS PUI WHERE PUI.ElementID = CMS_UIElement.ElementParentID) AS ParentOrder";

            return GetUIElements().Where(where).OrderBy("ParentOrder, ElementOrder").Columns(columns).BinaryData(true).TypedResult;
        }
        ...
        /// <param name="additionalWhere">Additional where condition</param>
        [Obsolete("Method was not intended for public use and will be removed in the next version.")]
        protected virtual UIElementInfo GetChildUIElementInternal(int parentElementID, String additionalWhere)
        {
            String where = (parentElementID > 0) ? "ElementParentID = " + parentElementID : "ElementParentID IS NULL";

            // Returns all records with base condition
            DataSet ds = GetUIElements().Where(where).OrderBy("ElementOrder").BinaryData(true).TypedResult;
            if (!DataHelper.DataSourceIsEmpty(ds))
            {
                // Apply additional condition
                if (!String.IsNullOrEmpty(additionalWhere))
                {
                    // If any record found with additional condition return it
                    DataRow[] dr = ds.Tables[0].Select(additionalWhere, "ElementOrder");
                    if (dr.Length > 0)
                    {
                        return new UIElementInfo(dr[0]);
                    }
                }

                // Otherwise return record with base condition
                return new UIElementInfo(ds.Tables[0].Rows[0]);
            }
        ...
        /// <param name="resourceId">ID of the resource</param>
        [Obsolete("Method was not intended for public use and will be removed in the next version.")]
        protected virtual TypedDataSet GetResourceUIElementsInternal(int resourceId)
        {
            string where = null;
            if (resourceId > 0)
            {
                where = "ElementResourceID = " + resourceId + " AND ElementLevel = 0";
            }
