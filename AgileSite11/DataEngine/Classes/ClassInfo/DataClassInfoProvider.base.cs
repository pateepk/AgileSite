...
namespace CMS.DataEngine
{
        ...
        where TProvider : DataClassInfoProviderBase<TProvider>, new()
    {
        ...
        internal override DataClassInfo GetInfoByColumn<T>(string columnName, T value)
        {
            ...
            // Base implementation uses the InfoDataSet implememtation which causes stack overflow exception for DataClassInfo
            var items = GetObjectQuery().WhereEquals(columnName, value);
            if (items.Count > 0)
            {
                dataClass = CreateInfo(items.Tables[0].Rows[0]);
            }
