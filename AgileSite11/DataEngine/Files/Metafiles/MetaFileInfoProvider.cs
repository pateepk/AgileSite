...
namespace CMS.DataEngine
{
    ...
    public class MetaFileInfoProvider : AbstractInfoProvider<MetaFileInfo, MetaFileInfoProvider>
    {
        ...
        public static bool UpdateMetaFiles(BaseInfo infoObj, DataTable filesDt, int filesObjectId, LoadDataEventHandler onLoadData = null, bool logSynchronization = true, bool parentIsNew = false, bool onlyAddNew = false)
        {
        ...
        public static string SaveFileToDisk(string siteName, string guid, string fileName, string fileExtension, BinaryData fileData, bool deleteOldFiles, bool synchronization)
        {
            if (!string.IsNullOrEmpty(fileName))
            {
                ...
                if (fileData != null)
                {
                    ...
                    if (!synchronization)
                    {
                        WebFarmHelper.CreateIOTask(DataTaskType.UpdateMetaFile, filePath, fileData, "metafileupload", siteName, guid, fileName, fileExtension, deleteOldFiles.ToString());
                    }
                ...
                if (Directory.Exists(directoryPath))
                {
                    ...
                    if (filesInfos != null)
                    {
                        ...
                        foreach (FileInfo file in filesInfos)
                        {
                            ...
                            if (!synchronization)
                            {
                                WebFarmHelper.CreateIOTask(DataTaskType.DeleteMetaFile, file.FullName, null, "FileDelete", siteName, fileName, deleteDirectory.ToString());
                            }
                        }
        ...
        protected override void SetInfo(MetaFileInfo info)
        {
            ...
            var oldChecksum = (oldInfo != null) ? oldInfo.BinaryDataChecksum : null;
            var newChecksum = (newBinaryData != null) ? newBinaryData.Checksum : null;
            ...
            if (dataChanged)
            {
                ...
                else
                {
                    ...
                    WebFarmHelper.CreateTask(DataTaskType.UpdateMetaFile, "metafileupload", siteName, guid, guid, info.MetaFileExtension, true.ToString());
                }
            }
            else
            {
