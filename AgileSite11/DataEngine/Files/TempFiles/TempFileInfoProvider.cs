...
{
    using TypedDataSet = InfoDataSet<TempFileInfo>;
    ...
    public class TempFileInfoProvider : AbstractInfoProvider<TempFileInfo, TempFileInfoProvider>
    {
        ...
        [Obsolete("Use method GetTempFiles() instead")]
        public static TypedDataSet GetTempFiles(string where, string orderBy, int topN, string columns)
        {
            return ProviderObject.GetTempFilesInternal(where, orderBy, topN, columns);
        }
        ...
        protected virtual TempFileInfo GetTempFileInfoInternal(int tempFileId)
        {
            ...
            return GetObjectQuery().BinaryData(true).WhereEquals("FileID", tempFileId).FirstObject;
        }
        ...
        /// <param name="columns">Columns to be selected</param>
        [Obsolete("Use method GetTempFiles() instead")]
        protected virtual TypedDataSet GetTempFilesInternal(string where, string orderBy, int topN, string columns)
        {
            return GetTempFiles().Where(where).OrderBy(orderBy).TopN(topN).Columns(columns).BinaryData(true).TypedResult;
        }
