    ...
    public class DatabaseSeparationHelper
    {
        ...
        public static bool CheckDBSeparation()
        {
            ...
            if (isSepConnString)
            {
                ...
                {
                    error = "The separated database was joined back to main database. Please remove connection string OMConnectionString from the web.config.";
                }
            }
            ...
            else if (!CheckCMDatabase(null))
            {
                ...
                error = "The contact management database is separated. Please add connection string OMConnectionString to the web.config.";
            }
        ...
        private void ModifyContent(List<TableAndClass> tableSchemas)
        {
            ...
            LogMessage(ResHelper.GetString("separationDB.tablescopy"), MessageTypeEnum.Info, false);
        ...
        {
            LogMessage(ResHelper.GetString("separationDB.proceduresviews"), MessageTypeEnum.Info, false);
        ...
        private void TestLinkedServer()
        {
            ...
            {
                throw new Exception(ResHelper.GetAPIString("separationDB.errorLinkedServer", "Creating link between SQL Servers failed. Please make sure that SQL Server enables remote connections, that specified login has appropriate permissions and that SQL connections are not blocked."));
            }
        }
...
FROM OPENQUERY([" + connectionStringSource.DataSource + "], 'SELECT " + tableColumns + @" FROM [" + connectionStringSource.InitialCatalog + "]..[" + tableName + @"]')";
            ...
            LogMessage(ResHelper.GetString("separationDB.contentcopy") + tableName, MessageTypeEnum.Info, false);
