...
namespace CMS.DataEngine.Generators
{
    ...
    /// </summary>
    public partial class BindingInfoTemplate
    {
        /// <summary>
        /// Module name
        /// </summary>
        ...
        /// Gets the info properties description.
        /// </summary>
        public IEnumerable<InfoTemplateProperty> References
        {
            get;
            private set;
        }
        ...
        public BindingInfoTemplate(DataClassInfo dataClass)
        {
            ...
            // Default value of SiteID column
            SiteIdColumn = "null";

            var references = new List<InfoTemplateProperty>();
            foreach (var p in form.GetFields<FieldInfo>())
            {
                if (!string.IsNullOrEmpty(p.ReferenceToObjectType))
                {
                    if (string.IsNullOrEmpty(ParentObjectType) && (p.ReferenceType == ObjectDependencyEnum.Binding))
                    {
                        ParentObjectType = p.ReferenceToObjectType;
                        ParentColumn = GetTemplateString(p.Name);
                    }
                    else
                    {
                        if (p.ReferenceToObjectType == PredefinedObjectType.SITE)
                        {
                            // If there is binding to Site, it should go to SiteID column, not to DependsOn
                            SiteIdColumn = GetTemplateString(p.Name);
                        }
                        else
                        {
                            references.Add(new InfoTemplateProperty(p));
                        }
                    }
                }
            }
        ...
        public class InfoTemplateProperty
        {
            ...
            public InfoTemplateProperty(FieldInfo field)
            {
                ...
                {
                    comment.Append(ResHelper.LocalizeString(fieldDescription));
                }
