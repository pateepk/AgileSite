    ...
    internal sealed class ObjectDependenciesRemover
    {
            ...
            else
            {
                ...
                var deleteQuery =
                    new DataQuery(typeInfo.ObjectClassName, QueryName.GENERALDELETE)
                        .Where(pathWhere)
                        .Where(
                            (typeInfo.TypeCondition != null) ?
                            typeInfo.TypeCondition.GetWhereCondition() :
                            null
                        );
        ...
        private IEnumerable<RemoveDependencyInfo> ProcessRemoveDependency(BaseInfo infoObj, bool getAll, IWhereCondition baseWhere, DependencyReference dependencyReference, HashSet<string> processed)
        {
            ...
            if (!deleteDependencies)
            {
                ...
                if (dependencyReference.IntegrityType == ObjectDependencyEnum.RequiredHasDefault)
                {
                    ...
                    if (defaultObj != null)
                    {
                        ...
                        {
                            throw new Exception(String.Format(@"The default object of object type ""{0}"" cannot be deleted.", dependencyObjTypeInfo.ObjectType));
                        }
            ...
            removeQuery.ApplySettings(q => q
                .Where(where)
                .Where(
                    (dependentObjTypeInfo.TypeCondition != null) ?
                    dependentObjTypeInfo.TypeCondition.GetWhereCondition().With(w => w.WhereIsComplex = true) :
                    null
                )
            );
        ...
        /// </remarks>
        /// <param name="infoObj">Info object</param>
        /// <param name="getAllRecursively">When true all dependent types in object hierarchy are returned, otherwise returns only the direct dependent types.</param>
        ...
        /// <param name="conditions">Collection containing already processed types. Used in recursive mode - <paramref name="getAllRecursively"/>.</param>
        /// <param name="processedObjectTypes">Set of object types which were processed in the call hierarchy of this method (when <paramref name="getAllRecursively"/> is true).</param>
        internal Dictionary<string, ICollection<WhereCondition>> GetDependentObjectsWhereConditions(BaseInfo infoObj, bool getAllRecursively, IWhereCondition baseWhere = null, Dictionary<string, ICollection<WhereCondition>> conditions = null, HashSet<string> processedObjectTypes = null)
        {
            baseWhere = baseWhere ?? new WhereCondition();
            ...
            foreach (var dependencyReference in dependencyReferences)
            {
                ...
                if (getAllRecursively)
                {
                    // Recursively process all dependencies of statically dependent objects
                    GetDependentObjectsWhereConditions(dependentObject, true, where, conditions, processedObjectTypes);
                    ...
                    if (pathWhere != null)
                    {
                        ...
                        if ((pathDependentWhere != null) && !pathDependentWhere.WhereIsEmpty)
                        {
                            ...
                            GetDependentObjectsWhereConditions(dependentObject, true, pathDependentWhere, conditions, processedObjectTypes);
                        }
                    }
                }
            }
            ...
            AddDynamicDependentWhereConditions(infoObj, baseWhere, conditions, getAllRecursively ? pathWhere : null);
        ...
        private static WhereCondition GetDependentObjectPathWhereCondition(BaseInfo infoObj, IWhereCondition baseWhere = null)
        {
            ...
            if (ti.IsSiteObject && (ti.SiteIDColumn != ObjectTypeInfo.COLUMN_NAME_UNKNOWN))
            {
                ...
                finalPathWhere.Where(sitePathWhere.WhereEquals(ti.SiteIDColumn, infoObj.GetIntegerValue(ti.SiteIDColumn, 0)));
            }
