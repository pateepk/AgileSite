...
namespace CMS.DataEngine
{
    ...
    internal class InfoCloneHelper
    {
        ...
        public static BaseInfo InsertAsClone(BaseInfo obj, CloneSettings settings, CloneResult result, bool isRoot)
        {
            ...
            using (new CMSActionContext { CreateVersion = false }.DisableLogging())
            {
            ...
            if (isRoot)
            {
                using (new CMSActionContext().DisableLogging())
                {
                    // Call set explicitly to ensure version (its disabled within above action context) and to ensure correct LastModifiedDate (if cloning of child objects take too long)
                    genCloneObj.SetObject();
                }

                // Log staging tasks for main objects with child objects and bindings (previously disabled)
                LogCloneSynchonization(cloneObj);
            }
        ...
        /// <summary>
        /// Logs the clone synchronization tasks
        /// </summary>
        private static void LogCloneSynchonization(BaseInfo cloneObj)
        {
            var typeInfo = cloneObj.TypeInfo;

            if (typeInfo.IsMainObject)
            {
                // Log the create task for the main cloned object
                typeInfo.RaiseOnLogObjectChange(cloneObj, TaskTypeEnum.CreateObject);
            }
            else
            {
                // Otherwise log update task for parent
                var parent = cloneObj.Parent;
                if (parent != null)
                {
                    parent.TypeInfo.RaiseOnLogObjectChange(parent, TaskTypeEnum.UpdateObject);
                }
            }

            // Log assign to site tasks for the site bindings
            if (typeInfo.SiteBinding != null)
            {
                var siteBindings = cloneObj.Bindings[typeInfo.SiteBinding];
                if (siteBindings != null)
                {
                    foreach (var binding in siteBindings)
                    {
                        binding.TypeInfo.RaiseOnLogObjectChange(binding, TaskTypeEnum.CreateObject);
                    }
                }
            }

            // Log extra tasks for main parents of other bindings
            foreach (var bindings in cloneObj.OtherBindings)
            {
        ...
        private static void LocalizeStringsInClone(BaseInfo obj, BaseInfo cloneObj)
        {
            ...
            foreach (string col in cloneObj.ColumnNames)
            {
                ...
                if (!string.IsNullOrEmpty(content))
                {
                    ...
                    {
                        cloneObj.SetValue(col, ResHelper.LocalizeString(content));
                    }
                }
            }
        }
        ...
        private static void CloneChildren(BaseInfo obj, CloneSettings settings, CloneResult result, BaseInfo cloneObj)
        {
            ...
            if (settings.IncludeChildren && (settings.MaxRelativeLevel != 0) && (obj.Children != null))
            {
                ...
                {
                    childContext.LogSynchronization = false;
                    ...
                    foreach (var childCollection in obj.Children)
                    {
                        ...
                        try
                        {
                            foreach (BaseInfo child in childCollection)
                            {
                                ...
                                InsertAsClone(child, settings, result, false);
                            }
                        }
        ...
        private static void CloneMetafiles(BaseInfo obj, CloneSettings settings, CloneResult result, BaseInfo cloneObj)
        {
            if (settings.IncludeMetafiles && (obj.MetaFiles != null))
            {
                ...
                {
                    // Clone metafiles
                    InsertAsClone(metafile, settings, result, false);
                }
                                ...
                                if (AllowCloneToOtherSite(obj, settings, bindingCollection.ObjectType, true))
                                {
                                    try
                                    {
                                        foreach (BaseInfo binding in bindingCollection)
                                        {
                                            ...
                                            InsertAsClone(bindingClone, settings, result, false);
                                        }
                                    }
                                    catch (LicenseException)
                                    {
                                ...
                                (settings.IncludeSiteBindings && bindingCollection.TypeInfo.IsSiteBinding))
                            {
                                ...
                                if (AllowCloneToOtherSite(obj, settings, bindingCollection.ObjectType, false))
                                {
                                    try
                                    {
                                        ...
                                        foreach (BaseInfo binding in bindingCollection)
                                        {
                                            InsertAsClone(binding, settings, result, false);
                                        }
                                    }
                                    catch (LicenseException)
                                    {
