    ...
    public class RentedUpdater
    {
        ...
        public string RenewLicenseKeys(string[] rentedKeys)
        {
            ...
            for (int i = 0; i < rentedKeys.Length; i++)
            {
                ...
                if (licenseRow != null)
                {
                    ...
                    {
                        throw new InvalidOperationException(String.Format(ResHelper.GetString("RentedLicense.PerpetualExists"), domains[i]));
                    }
                    else
                    {
            ...
            if (rentedKeys.Length > 0)
            {
                ...
                {
                    returnMessage = ResHelper.GetString("RentedLicense.NoLicenseNeeded");
                }
                ...
                else
                {
                    ...
                    returnMessage += String.Format(ResHelper.GetString("RentedLicense.LicenseUpdated"), updatedCount);
                    if (!String.IsNullOrEmpty(additionalInfo))
                    {
                        returnMessage += " " + ResHelper.GetString("RentedLicense.SomeProblem");
                    }
                }
            }
        ...
        private string[] GetDomains(string[] rentedKeys)
        {
            ...
            for (int i = 0; i < rentedKeys.Length; i++)
            {
                ...
                {
                    throw new InvalidOperationException(String.Format(ResHelper.GetString("RentedLicense.InvalidKeyCode"), rentedKeys[i]));
                }
        ...
        private void CheckForDuplicitDomains(string[] domains)
        {
            ...
            for (i = 0; i < domains.Length - 1; i++)
            {
                for (int j = i + 1; j < domains.Length; j++)
                {
                    ...
                    {
                        throw new InvalidOperationException(ResHelper.GetString("RentedLicense.MoreLicenses"));
                    }
                }
            }
        }
