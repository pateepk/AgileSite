...
namespace CMS.LicenseProvider
{
    ...
    public class LicenseKeyInfoProvider : AbstractInfoProvider<LicenseKeyInfo, LicenseKeyInfoProvider>
    {
        ...
        // Features enum with shortcut
        private static readonly string[,] items = new string[14, 3];

        // Feature enums in hash with shortcut key
        private static readonly SafeDictionary<string, string> E36 = new SafeDictionary<string, string>();
        ...
        {
            // remove protocol like http, https etc.
            if (url.Contains("://"))
            {
        ...
        public static void SetLicenseKeyInfo(LicenseKeyInfo licenseKey)
        {
            if (licenseKey == null)
            {
                throw new ArgumentNullException("licenseKey", Encoding.ASCII.GetString(Convert.FromBase64String("Tm8gTGljZW5zZUtleUluZm8gb2JqZWN0IHNldC4=")));
            }
        ...
        private static bool IsFeatureAvailable(ProductEditionEnum edition, FeatureEnum feature)
        {
            ...
            switch (edition)
            {
                ...
                case ProductEditionEnum.Free:
                    ...
                    // No feature here, would be in CommonFeatures
                    return false;
        ...
        public static bool IsFeatureAvailable(string domain, FeatureEnum feature)
        {
            ...
            // Prepare the key
            string key = String.Format("{0}|{1}", domain, feature);
            bool? result;

            if (!C.TryGetValue(key, out result))
            {
        ...
        public static int VersionLimitations(string domain, FeatureEnum feature, bool siteCheck = true)
        {
            ...
            if (lki != null)
            {
                if (siteCheck && (HttpContext.Current != null))
                {
                    string url = HttpContext.Current.Request.AppRelativeCurrentExecutionFilePath.ToLowerInvariant();
                    url = url.Remove(0, 2); // remove ~/

                    if (url.StartsWith("cmspages/logon", StringComparison.InvariantCultureIgnoreCase)
                        || url.StartsWith("cmsmodules/licenses", StringComparison.InvariantCultureIgnoreCase)
                        || url.StartsWith("cmsmodules/system", StringComparison.InvariantCultureIgnoreCase)
                        || url.StartsWith("admin/", StringComparison.InvariantCultureIgnoreCase))
                    {
                        return LicenseHelper.LIMITATIONS_UNLIMITED;
                    }

                    // CMSModules folder is not excluded if feature Editors or Global admins
                    if (url.StartsWith("cmsmodules/", StringComparison.InvariantCultureIgnoreCase) && (feature != FeatureEnum.Editors) && (feature != FeatureEnum.Administrators))
                    {
                        return LicenseHelper.LIMITATIONS_UNLIMITED;
                    }
                }
        ...
        /// Init custom modules.
        /// </summary>
        private static void GetServerVariables()
        {
            items[0, 0] = FeatureEnum.BizForms.ToString();
            items[0, 1] = "2";
            items[0, 2] = "BIZ";

            items[1, 0] = FeatureEnum.Blogs.ToString();
            items[1, 1] = "2";
            items[1, 2] = "BLO";

            items[2, 0] = FeatureEnum.DocumentLevelPermissions.ToString();
            items[2, 1] = "2";
            items[2, 2] = "DLP";

            items[3, 0] = FeatureEnum.Ecommerce.ToString();
            items[3, 1] = "3";
            items[3, 2] = "ECO";

            items[4, 0] = FeatureEnum.EventManager.ToString();
            items[4, 1] = "3";
            items[4, 2] = "EVM";

            items[5, 0] = FeatureEnum.Forums.ToString();
            items[5, 1] = "2";
            items[5, 2] = "FOR";

            items[6, 0] = FeatureEnum.Multilingual.ToString();
            items[6, 1] = "3";
            items[6, 2] = "MUL";

            items[7, 0] = FeatureEnum.Newsletters.ToString();
            items[7, 1] = "2";
            items[7, 2] = "NEW";

            items[8, 0] = FeatureEnum.Polls.ToString();
            items[8, 1] = "2";
            items[8, 2] = "POL";

            items[9, 0] = FeatureEnum.Staging.ToString();
            items[9, 1] = "3";
            items[9, 2] = "STA";

            items[10, 0] = FeatureEnum.WebAnalytics.ToString();
            items[10, 1] = "3";
            items[10, 2] = "WAN";

            items[11, 0] = FeatureEnum.Webfarm.ToString();
            items[11, 1] = "3";
            items[11, 2] = "WFA";

            items[12, 0] = FeatureEnum.WorkflowVersioning.ToString();
            items[12, 1] = "2";
            items[12, 2] = "WVE";

            items[13, 0] = FeatureEnum.IntegrationBus.ToString();
            items[13, 1] = "3";
            items[13, 2] = "INT";
