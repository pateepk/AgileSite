...
public partial class CMSModules_Membership_Pages_Users_User_Edit_Roles : CMSUsersPage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        var data = UserRoleInfoProvider.GetUserRoles().WhereEquals("UserID", userId).And().WhereIn("RoleID", roleIds).Columns("RoleID").TypedResult;
        if (data.Any())
        {
            currentValues = TextHelper.Join(";", data.Select(i => i.RoleID));
        }
    ...
    private void SaveData()
    {
        ...
        // Remove old items
        string items = DataHelper.GetNewItemsInList(selectorValues, currentValues);
        ...
        // Add new items
        items = DataHelper.GetNewItemsInList(currentValues, selectorValues);
        if (!String.IsNullOrEmpty(items))
        {
            string[] newItems = items.Split(new char[] { ';' }, StringSplitOptions.RemoveEmptyEntries);
            if (newItems != null)
