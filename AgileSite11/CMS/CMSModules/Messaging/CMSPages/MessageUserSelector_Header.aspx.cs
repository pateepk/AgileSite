...
public partial class CMSModules_Messaging_CMSPages_MessageUserSelector_Header : LivePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!MembershipContext.AuthenticatedUser.IsPublic())
        {
            ...
            var query = "?refresh=" + QueryHelper.GetText("refresh", String.Empty) + "&hidid=" + QueryHelper.GetText("hidid", String.Empty) + "&mid=" + QueryHelper.GetText("mid", String.Empty);
            query += "&hash=" + ValidationHelper.GetHashString(query, new HashSettings { HashSalt = "CMSPages|MessageUserSelector|Search" });
            // Search tab
            CurrentMaster.Tabs.AddTab(new UITabItem()
            {
                Text = GetString("general.search"),
                RedirectUrl = ApplicationUrlHelper.ResolveDialogUrl("~/CMSModules/Messaging/CMSPages/MessageUserSelector_Search.aspx") + query
            });
