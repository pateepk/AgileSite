...
public partial class CMSModules_Messaging_Dialogs_MessageUserSelector_Header : CMSPage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!MembershipContext.AuthenticatedUser.IsPublic())
        {
            ...
            query += "&hash=" + ValidationHelper.GetHashString(query, new HashSettings { HashSalt = "Dialogs|MessageUserSelector|Search" });
            
            // Search tab
            CurrentMaster.Tabs.AddTab(new UITabItem
            {
                Text = GetString("general.search"),
                RedirectUrl = ApplicationUrlHelper.ResolveDialogUrl("~/CMSModules/Messaging/Dialogs/MessageUserSelector_Search.aspx") + query
            });
