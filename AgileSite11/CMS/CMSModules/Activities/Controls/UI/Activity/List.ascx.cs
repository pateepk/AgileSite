...
public partial class CMSModules_Activities_Controls_UI_Activity_List : CMSAdminListControl, ICallbackEventHandler
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (ContactID > 0)
        {
            gridElem.ObjectType = PredefinedObjectType.ACTIVITY;
            gridElem.WhereCondition = new WhereCondition(gridElem.WhereCondition).And().WhereEquals(nameof(ActivityInfo.ActivityContactID), ContactID).ToString(true);
        }
        else
        {
            // Add contact column to the grid, if the contact for whom to retrieve activities is not specified
            gridElem.GridColumns.Columns.Insert(2, new Column { Source = "ContactFullNameJoined", Name = "contactname", Caption = "$om.activity.contactname$", Wrap = false });
            gridElem.Columns += ",ContactFullNameJoined";
            gridElem.ObjectType = "om.activitylist";
        }
