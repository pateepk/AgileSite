...
public partial class CMSModules_DataProtection_Pages_PersonalDataManagement : CMSPage
{
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        try
        {
            if (mDataSubjectIdentifiersFilterControl.IsValid())
            {
                ...
                mDataSubjectIdentifiersFilter = JsonConvert.SerializeObject(dataSubjectIdentifiersFilter);
            }
        }
        catch (Exception ex)
        {
    ...
    private void RegisterDeleteDataScripts()
    {
        ...
        var script = $@"
            ...
            {{
                modalDialog('{ScriptHelper.GetString(dialogUrl, false)}', 'SelectDataToDelete', '660', '590'); 
            }}";
