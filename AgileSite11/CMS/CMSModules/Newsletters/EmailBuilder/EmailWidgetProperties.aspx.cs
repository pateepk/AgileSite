...
public partial class CMSModules_Newsletters_EmailBuilder_EmailWidgetProperties : CMSNewsletterPage
{
    ...
    /// </summary>
    private IssueInfo Issue => mIssue ?? (mIssue = IssueInfoProvider.GetIssueInfo(QueryHelper.GetInteger("issueid", 0)));
    ...
    private void SetupForm()
    {
        if (widgetTypeFormInfo.ItemsList.Count > 0)
        {
            ...
            propertiesForm.FormType = FormTypeEnum.DataForm;
    ...
    private DataRowContainer GetWidgetProperties()
    {
        ...
        foreach (var property in widgetInstance.Properties)
        {
            if (property.Value != null)
            {
                ...
                if (formField != null)
                {
                    var fieldDataType = formField.DataType;
                    var value = DataTypeManager.ConvertToSystemType(TypeEnum.Field, fieldDataType, property.Value, CultureHelper.EnglishCulture);
