...
namespace NHG_C
{
    public partial class BlueKey_CMSWebParts_NeighborhoodSearch_AvaliableHomes_CH_WTE : CMSAbstractBaseFilterControl
    {
        #region "Properties"
        ...
        protected void PopulateAreas()
        {
            ...
            DataSet ds = CustomTableItemProvider.GetItems("customlocations.area", "", "AreaName ASC");
        ...
        protected void resetCitiesByArea(int area)
        {
            if (area != 0)
            {
                ...
                ds = CustomTableItemProvider.GetItems("customlocations.city", "CityCounty IN (SELECT ItemID FROM customlocations_county WHERE CountyArea = '" + Convert.ToString(area) + "') AND CityVisible = 1 ", "CityName ASC", 0, "CityName, MAX(ItemID) as ItemID").GroupBy("CityName");
        ...
        private string getLocationFilterQueryByArea(int area)
        {
            ...
            DataSet ds = CustomTableItemProvider.GetItems("customlocations.county", "CountyArea = '" + Convert.ToString(area) + "' ", null);
