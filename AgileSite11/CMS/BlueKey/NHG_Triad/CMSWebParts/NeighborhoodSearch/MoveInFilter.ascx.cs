...
namespace NHG_T
{
    public partial class BlueKey_CMSWebParts_NeighborhoodSearch_MoveInFilter : CMSAbstractBaseFilterControl
    {
        #region "Properties"
        ...
        private Tuple<string, string> _sortField = new Tuple<string, string>(string.Empty, string.Empty);
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (arg != null)
            {
                ...
                /*     if (divID == "jsSort")
                     {
                         string classAttr = Request.Form["__EVENTARGUMENT"];

                         // This should be sorter, sorter##COLUMN##, direction
                         string[] classes = classAttr.Split(' ');
                         //_sortField.Item1 = classes.Length > 1 ? classes[1] : string.Empty;
                         //_sortField.Item2 = classes.Length > 2 ? classes[2] : string.Empty;
                         _sortField = new Tuple<string, string>(classes.Length > 1 ? classes[1] : string.Empty, classes.Length > 2 ? classes[2] : string.Empty);
                     }            */
            }
        ...
        {
            /*  switch (_sortField.Item1)
              {
                  case "sorterPrice":
                      this.OrderBy = " ListingPrice";
                      break;
                  case "sorterName":
                      this.OrderBy = " NeighborhoodName";
                      break;
                  case "sorterCity":
                      this.OrderBy = " ListingCity";
                      break;
                  case "sorterPlan":
                      this.OrderBy = " ListingPlan";
                      break;
                  case "sorterBuilder":
                      this.OrderBy = " DeveloperName";
                      break;
                  case "sorterLot":
                      this.OrderBy = " ListingLotNumber";
                      break;
                  case "sorterStories":
                      this.OrderBy = " ListingStory";
                      break;
                  case "sorterBedBath":
                      this.OrderBy = " ListingBedrooms";
                      break;
                  case "sorterSqFt":
                      this.OrderBy = " ListingSquareFootage";
                      break;
                  default:
        ...
        private void Filter()
        {
            if (this.FilterByQuery)
            {
                ...
                if (!String.IsNullOrEmpty(_sortField.Item1))
                {
                    url = URLHelper.AddParameterToUrl(url, "sort", _sortField.Item1);
                }

                if (!String.IsNullOrEmpty(_sortField.Item2))
                {
                    url = URLHelper.AddParameterToUrl(url, "dir", _sortField.Item2);
                }
