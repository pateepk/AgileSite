...
public partial class sso_encrypt_test_old : System.Web.UI.Page
{
    ...
    protected void cmdCreateSSOLink_Click(object sender, EventArgs e)
    {
        ...
        if (isOK)
        {
            ...
            LiteralSSOLink.Text = String.Format("<a target=\"_blank\" href=\"{0}\">{0}</a>", String.Format("/sso/sso_encrypt_old.aspx?name={0}&email={1}&CompanyID={2}&hash={3}", HttpUtility.UrlEncode(txtFullName.Text), HttpUtility.UrlEncode(txtEmailAddress.Text), intCompanyID, hash));
        }
    }
}