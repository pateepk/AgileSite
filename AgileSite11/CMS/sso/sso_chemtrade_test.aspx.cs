...
public partial class sso_chemtrade_test : System.Web.UI.Page
{
    ...
    protected void cmdCreateSSOLink_Click(object sender, EventArgs e)
    {
        ...
        if (isOK)
        {
            int intRedirect = 4255;
            string hash = intCompanyID.ToString() + txtEmailAddress.Text + SharedKey;
            MD5 md5Hash = MD5.Create();
            hash = HttpUtility.UrlEncode(GetMd5Hash(md5Hash, hash));
            LiteralSSOLink.Text = String.Format("<a target=\"_blank\" href=\"{0}\">{0}</a>", String.Format("https://www.trainingnetworknow.com/sso/sso_chemtrade.aspx?name={0}&email={1}&CompanyID={2}&hash={3}&video={4}", HttpUtility.UrlEncode(txtFullName.Text), HttpUtility.UrlEncode(txtEmailAddress.Text), intCompanyID, hash, intRedirect));
        }
    }
}