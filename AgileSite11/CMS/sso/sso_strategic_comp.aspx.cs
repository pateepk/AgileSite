...
public partial class sso_strategic_comp : System.Web.UI.Page
{
    #region Global Vars

    private string _dbRedirectUrl = "";
    private const string SSOErrorURL = "http://learn.streamery.co/system/SSOError?SSOLogID=";
    private const string TestPageURL = "https://www.trainingnetworknow.com/sso/sso_strategic_comp_test.aspx";
    private string _X509Certificate2 = "";
    private string _pfxPassword = "TiR4DD3v0lp3R"; // fake password, not important
    ...
    private int[] allowed_companyID = new int[] { 1, 543, 5179 };
    private int DefaultCompanyID = 5179;
                        ...
                        if (cancontinue)
                        {
                            ...
                            if (cancontinue)
                            {
                                ...
                                if (userInfo != null)
                                {
                                    ...
                                    {
                                        lblDebug.Text += "<br>" + "Requested User was disabled manually...";
                                        SSOLogMessage = "Requested User was disabled manually...";
                                    }
                                }
                            }
                            else
    ...
    public bool CheckAndLogCert(XmlDocument samlXMLDoc, out string message)
    {
        ...
        try
        {
            ...
            {
                ret = VerifyXml(samlXMLDoc, GetX509Certificate2(_X509Certificate2)); // Verify XML and check cert
            }
            else
            {
                if (!_validateCert)
    ...
    protected bool GetCompanyX509Certificate(int CompanyID)
    {
        ...
        try
        {
            DataSet ds = ConnectionHelper.ExecuteQuery(string.Format("SELECT X509certificate, LandingPageUrl, SSOEnabled  FROM customtable_Customers WHERE Itemid = {0}", CompanyID), null, QueryTypeEnum.SQLQuery);
            if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                DataRow row = ds.Tables[0].Rows[0];
                _X509Certificate2 = row["X509certificate"].ToString();
