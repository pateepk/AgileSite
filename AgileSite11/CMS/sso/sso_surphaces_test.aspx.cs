...
public partial class sso_surphaces_test : System.Web.UI.Page
{
    ...
    protected void cmdCreateSSOLink_Click(object sender, EventArgs e)
    {
        ...
        if (isOK)
        {
            ...
            LiteralSSOLink.Text = String.Format("<a target=\"_blank\" href=\"{0}\">{0}</a>", String.Format("https://www.trainingnetworknow.com/sso/sso_surphaces.aspx?name={0}&email={1}&CompanyID={2}&hash={3}", HttpUtility.UrlEncode(txtFullName.Text), HttpUtility.UrlEncode(txtEmailAddress.Text), intCompanyID, hash));
        }
    }
}