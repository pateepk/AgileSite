...
namespace NHG_T.BlueKey
{
    ...
    public static class CsvParser
    {
        ...
        public static bool CreateCSVFile(DataTable dt, string m_fileName)
        {
            ...
            return CsvParser.CreateCSVFile(dt, m_fileName, columns); // move along, nothing to see here
        }

        public static bool CreateCSVFile(DataTable dt, string m_fileName, string[] columns)
        {
            bool result = true;

            StreamWriter sw = new StreamWriter(m_fileName, false);
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    if (columns.Length > 0)
                    {
                        ...
                        foreach (string name in columns)
                        {
                            ...
                            {
                                sw.Write(CsvParser.MakeCSVFriendly(dr[name]));
                            }
                            if (i < columns.Length)
                            {
                                sw.Write(",");
                    ...
                    else
                    {
                        ...
                        for (int i = 0; i < iColCount; i++)
                        {
                            ...
                            {
                                sw.Write(CsvParser.MakeCSVFriendly(dr[i]));
                            }
                            if (i < iColCount - 1)
                            {
                                sw.Write(",");
        ...
        private static string MakeCSVFriendly(object value)
        {
