...
{
    public partial class CUFMemberTaxYearFilter : CMSAbstractQueryFilterControl
    {
        #region "Properties"
        ...
        private void SetupControl()
        {
            ...
            else if (!RequestHelper.IsPostBack())
            {
                ...
                sql.Append(" CASE ");
                sql.Append(" WHEN N.NoticeTypeID = 10 AND N.Date2 is not null THEN DatePart(Year, N.Date2)");
                sql.Append(" ELSE DatePart(Year, N.Date)");
                sql.Append(" END TaxYear");
                sql.Append(" from ").Append(dataDB).Append(".dbo.Notice N");
                sql.Append(" INNER JOIN ").Append(dataDB).Append(".dbo.NoticeType NT ON N.NoticeTypeID = NT.NoticeTypeID and NT.NoticeTypeID in (14, 10, 3)");
                sql.Append(" INNER JOIN ").Append(dataDB).Append(".dbo.Batch B on N.BatchID = B.BatchID");
                if (ReplicatingAdmin == 0)
                {
                    sql.Append(" AND B.BatchStatusID = 5");
                }
                sql.Append(" WHERE N.MemberID = @MemberID ");
                sql.Append(" union");
                sql.Append(" select TaxYear from ").Append(dataDB).Append(".dbo.TaxInfo TI");
                sql.Append(" INNER JOIN ").Append(dataDB).Append(".dbo.Batch B on TI.BatchID = B.BatchID");
                if (ReplicatingAdmin == 0)
                {
                    sql.Append(" AND B.BatchStatusID = 5");
                }
                sql.Append(" where TI.MemberID = @MemberID");
                sql.Append(" ) TY");
                ...
                divYearDropDown.Visible = ddlTaxYear.Items.Count > 0;
            }
        }
