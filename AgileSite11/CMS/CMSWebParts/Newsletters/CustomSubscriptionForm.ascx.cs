...
public partial class CMSWebParts_Newsletters_CustomSubscriptionForm : CMSAbstractWebPart
{
    ...
    private bool Save(string newsletterName, ContactInfo contact)
    {
        ...
        if (newsletter != null)
        {
            try
            {
                ...
                if (!mSubscriptionService.IsSubscribed(contact, newsletter) || IsExceeded(contact, newsletter))
                {
                    toReturn = true;
    ...
    private bool IsExceeded(ContactInfo contact, NewsletterInfo newsletter)
    {
        ...
        var interval = SettingsKeyInfoProvider.GetDoubleValue(SiteContext.CurrentSiteName + ".CMSOptInInterval");
