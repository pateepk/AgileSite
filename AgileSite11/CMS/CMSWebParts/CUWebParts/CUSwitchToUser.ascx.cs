...
{
    public partial class CUSwitchToUser : CMSAbstractWebPart
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                {
                    string testID = QueryHelper.GetString("MemberIDToSwitch", "0");
                    long realID = 0;
                    MemberIDToSwitch = 0;
                    if (long.TryParse(testID, out realID))
                    {
                        MemberIDToSwitch = realID;
                    }

                    if (MemberIDToSwitch != 0)
                    {
                        switch (actionString)
                        {
                            case "switch":
                    ...
                    {
                        lblMsg.Text = "MemberIDToSwitch was not passed or was 0.";
                    }
                }
            }
        }
        ...
        {
            if (ActionRequested != ActionEnum.Unknown && MemberIDToSwitch != 0)
            {
                ...
                ObjectQuery<UserSettingsInfo> userSettingsSet = UserSettingsInfoProvider.GetUserSettings()
                    .WhereEquals("CUMemberNumber", MemberIDToSwitch);
                ...
                if (userInfo == null && sb.Length == 0)
                {
                    ...
                    qdp.Add("membernumber", MemberIDToSwitch);
                    ...
                    if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        userSettings.SetValue("CUMemberNumber", MemberIDToSwitch);
