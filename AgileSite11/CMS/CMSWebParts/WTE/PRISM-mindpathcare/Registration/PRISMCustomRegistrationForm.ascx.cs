...
public partial class CMSWebParts_WTE_PRISM_mindpathcare_Registration_PRISMCustomRegistrationForm : CMSAbstractWebPart
{
    ...
    //private string _RedirectURL = "https://referrals.mindpathcare.com/Register/Thank-you/";
    ...
    private void formUser_OnAfterSave(object sender, EventArgs e)
    {
        ...
        if (!ConfirmationRequired)
        {
            ...
            {
                ui.Enabled = true;
                ui.UserSettings.UserWaitingForApproval = false;
            }
        }
        else
        {
        ...
        else
        {
            ...
            {
                // UserInfo userInfo = UserInfoProvider.GetUserInfo(ui.UserName);
                // URLHelper.Redirect(_RedirectURL + userInfo.UserID);
    ...
    // Due to issues with custom registration, we will enable user on the form to properly set settings then diable here.. this is a hack...
    public void DisableUser(UserInfo ui)
    {
        DataSet ds2 = ConnectionHelper.ExecuteQuery(string.Format("UPDATE CMS_User SET UserEnabled = 0 WHERE UserID = {0}", ui.UserID), null, QueryTypeEnum.SQLQuery);
    }

    // Saves New user input from form to Referr [PRISM] sAdmin table
    private void SaveNewUserToPRISMData(UserInfo ui)
    {
        //{0} FirstName,{1} MiddleName, {2} LastName,{3} emailinput, {4} UserName, {5} GroupEmail, {6} ContactNumber,{SET TO 0} IsAdmin, {7} IsProvider, {set to 0 }ApprovedProvider,{8}  IsCoordinator
        try
        {
            FormEngineUserControl txtFirstName = formUser.FieldControls["FirstName"];
            FormEngineUserControl txtMiddleName = formUser.FieldControls["MiddleName"];
            FormEngineUserControl txtLastName = formUser.FieldControls["LastName"];

            FormEngineUserControl txtUserName = formUser.FieldControls["UserName"];
            FormEngineUserControl txtEmail = formUser.FieldControls["Email"];
            FormEngineUserControl txtGroupEmail = formUser.FieldControls["GroupEmail"];
            FormEngineUserControl txtContact_Number = formUser.FieldControls["Contact_Number"];
            FormEngineUserControl txtIsProvider = formUser.FieldControls["IsProvider"];
            FormEngineUserControl txtIsCoordinator = formUser.FieldControls["IsCoordinator"];

            DataSet ds = ConnectionHelper.ExecuteQuery(string.Format(
                    @"INSERT INTO Form_MindPathCheckIn_SAdmin_Register
(FormInserted, FormUpdated, FirstName, MiddleName, LastName, emailinput,  UserName, GroupEmail, ContactNumber,IsAdmin, IsProvider, ApprovedProvider, IsCoordinator)
VALUES
(GETDATE(), GETDATE(), {0}, {1}, {2}, {3}, {4}, {5}, {6}, 0, {7}, 0, {8})
"
                , "'" + txtFirstName.Value + "'" // 0
                , txtMiddleName.Value != "" ? "'" + txtMiddleName.Value + "'" : "'NMN'" // 1
                , "'" + txtLastName.Value + "'"// 2
                , "'" + txtEmail.Value + "'"// 3
                , "'" + txtUserName.Value + "'"// 4
                , "'" + txtGroupEmail.Value + "'"// 5
                , "'" + txtContact_Number.Value + "'"// 6
                , txtIsProvider.Value.ToString().Contains("T") ? 1 : 0 // 7
                , txtIsCoordinator.Value.ToString().Contains("T") ? 1 : 0 // 8
                )
        ...
        try
        {
            ...
            DataSet ds2 = ConnectionHelper.ExecuteQuery(string.Format(
            @"SELECT TOP 1 [RG].[PracticeEmail], [RG].[GroupName] FROM CMS_User [USR]
        LEFT JOIN Form_MindPathCheckIn_SAdmin_Register [SADMIN] ON [USR].[Email] = [SADMIN].emailinput
        LEFT JOIN Form_MindPathCheckIn_ReferralGroups [RG] ON [SADMIN].[GroupEmail] LIKE '%' + [RG].[DomainName] + '%'
        WHERE UserID = {0}", ui.UserID), null, QueryTypeEnum.SQLQuery);

            if (ds2 != null && ds2.Tables.Count > 0 && ds2.Tables[0].Rows.Count > 0)
            {
                DataRow row = ds2.Tables[0].Rows[0];
                cc = row["PracticeEmail"].ToString();
                groupName = row["GroupName"].ToString();
                // TODO: TESTING REMOVE THIS!
                cc = "";
                SendUserConfirmationEmail(ui, cc, groupName);
                SendTeamConfirmationEmail(ui, cc, groupName);
            }
            else
            {
                EventLogProvider.LogEvent(EventType.INFORMATION, "Registration Error", "CUSTOMREGESTRATION", eventDescription: "no group found");
            }
        }
        catch (Exception ex)
        {
    ...
    // Builder for new user confirmation email
    private void SendUserConfirmationEmail(UserInfo userInfo, string cc, string groupName)
    {
        string subject = "'MindPath Referral Account Request Received'";
        string body = "'<html><body><p>Thank you, your form submission to request a Referral Account for MindPath has been received.</p><p>You will receive follow-up information once the request is reviewed and the account is approved.</p>&nbsp;<p><b>Request Submitted by: </b><span>{{FULLNAME}} </span></p>&nbsp;<p><b>Referring Practice Group Name: </b><span>{{GROUPNAME}} </span></p>&nbsp;<p>If you have any questions or need assistance, please contact us at <a href=''mailto:info@mindpathcare.com''>info@mindpathcare.com</a>.</p></body></html>'";

        body = body.Replace("{{FULLNAME}}", userInfo.FullName);
        body = body.Replace("{{GROUPNAME}}", groupName);

        SendEmail(subject, body, userInfo.Email, cc);
    }

    // Builder for company & wte notification of a new user
    private void SendTeamConfirmationEmail(UserInfo userInfo, string cc, string groupName)
    {
        string subject = "'PRISM New Referral Account Request'";
        string body = "'<html><body><p>A request for a new Referral account has been submitted. Please login to the <a href=''https://referrals.mindpathcare.com/sAdmin''>PRISM Sadmin</a> to review the request:</p><p><b>Request Submitted by: </b><span>{{FULLNAME}} </span></p><p><b>Referring Practice Group Name: </b><span>{{GROUPNAME}} </span></p>&nbsp;</body></html>'";
        body = body.Replace("{{FULLNAME}}", userInfo.FullName);
        body = body.Replace("{{GROUPNAME}}", groupName);

        //SendEmail(subject, body, "Referrals@mindpathcare.com;darci@wte.net;egarrison@wte.net;raphael@fonseca.tech", cc);
        string cced = "darci@wte.net;eric.garrison@mindpathcare.com;raphael@wte.net";
        string to = "lin.nelson@mindpathcare.com";

        // For testing
        SendEmail(subject, body, to, cced);
        //SendEmail(subject, body, "raphael@fonseca.tech", cc);
    }

    // SQL that sends an email
    private void SendEmail(string subject, string body, string to, string cc)
    {
        try
        {
            DataSet ds = ConnectionHelper.ExecuteQuery(string.Format(
                "EXEC custom_Email_Send @Mode = 1, @Subject = {0}, @HtmlBody = {1}, @To = '{2}', @CC = '{3}', @From = 'Referrals@mindpathcare.com'"
                , subject, body, to, cc)
                , null, QueryTypeEnum.SQLQuery);
        }
        catch (Exception ex)
