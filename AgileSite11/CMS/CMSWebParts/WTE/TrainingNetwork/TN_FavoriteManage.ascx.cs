...
{
    /// <summary>
    /// Web part for managing a TN User Favorites (multiple row selection support)
    /// </summary>
    public partial class TN_FavoriteManage : CMSAbstractWebPart
    {
        ...
        /// </summary>
        public class FavoriteManageData
        {
            /// <summary>
            /// The action.
            /// </summary>
        ...
        public string SessionMessage
        {
            get
            {
                string ret = GetSession("TNFavoriteManageSessionMessage");

                if (String.IsNullOrWhiteSpace(ret))
                {
                    if (String.IsNullOrWhiteSpace(ret))
                    {
                        ret = GetStringProperty("TNFavoriteManageSessionMessage", "");
                    }
                }
            ...
            set
            {
                this.SetValue("TNFavoriteManageSessionMessage", value);
                SetSession("TNFavoriteManageSessionMessage", value);
            }
        }
        ...
        public bool? IsSessionError
        {
            ...
            {
                bool? ret = null;
                string retString = GetSession("TNFavoriteManageIsSessionError");

                if (String.IsNullOrWhiteSpace(retString))
                {
                    if (String.IsNullOrWhiteSpace(retString))
                    {
                        ret = GetNullableBoolProperty("TNFavoriteManageIsSessionError", null);
                    }
                }
                else
                {
            ...
            set
            {
                this.SetValue("TNFavoriteManageIsSessionError", value);
                SetSession("TNFavoriteManageIsSessionError", value);
            }
        }
        ...
        public bool ProcessFavorite()
        {
            ...
            FavoriteManageData data = GetJSONPostedData<FavoriteManageData>(null);
            ...
            if (data != null)
            {
                ...
                string sprocName = "[dbo].[Proc_TN_customtable_VideoUserFavorite_AddOrRemove]";


                GeneralConnection conn = ConnectionHelper.GetConnection();
                QueryDataParameters parameters = new QueryDataParameters();
                parameters.Add("@" + CompanyIDColumnKey, data.CompanyID);
                parameters.Add("@" + UserIDColumnKey, data.UserID);
                parameters.Add("@" + VideoIDColumnKey, data.VideoID);
                parameters.Add("@AddFavorite", addFavorite);
                ...
                if (ds != null && ds.Tables.Count > 0 && ds.Tables[0] != null && ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    if (row != null)
                    {
                        message += GetStringValueOrNull(row["Message"]);
                        success = GetBoolValueOrNull(row["Success"]).GetValueOrDefault(true);
                    }
                }
