...
{
    /// <summary>
    /// Bulk enable/disable and delete user.
    /// </summary>
    public partial class TN_UserManage : CMSAbstractWebPart
    {
        ...
        public class UserManageData
        {
            ...
            /// <summary>
            /// The list of user ID to perform operations
            /// </summary>
        ...
        public bool IsDebugMode
        {
            get
            {
                return GetBoolProperties("IsDebugMode", false);
            }
            set
            {
        ...
        public bool ShowGUI
        {
            get
            {
                return GetBoolProperties("ShowGUI", false);
            }
            set
            {
        ...
        /// <summary>
        /// Mode of operation
        /// "auto" = try to pull the mode from POST data.
        /// "manage" or "manage[type]" = enable/disable or delete
        ...
        public string SessionMessage
        {
            get
            {
                string ret = GetSession("TNUserManageSessionMessage");

                if (String.IsNullOrWhiteSpace(ret))
                {
                    if (String.IsNullOrWhiteSpace(ret))
                    {
                        ret = GetStringProperty("TNUserManageSessionMessage", "");
                    }
                }
            ...
            set
            {
                this.SetValue("TNUserManageSessionMessage", value);
                SetSession("TNUserManageSessionMessage", value);
            }
        }
        ...
        public bool? IsSessionError
        {
            get
            {
                ...
                if (String.IsNullOrWhiteSpace(retString))
                {
                    if (String.IsNullOrWhiteSpace(retString))
                    {
                        ret = GetNullableBoolProperties("TNCatalogManagerIsSessionError", null);
                    }
                }
                else
                {
                    ret = GetNullableBoolProperties(retString, null);
                }
        ...
        /// <summary>
        /// Process User Actions (disable, enable, delete, copy)
        /// </summary>
        /// <returns></returns>
        protected bool ProcessManageUser()
        {
            bool success = false;
            ...
            UserManageData data = GetJSONPostedData<UserManageData>(null);
            if (data != null)
            {
                action = data.Action;
                try
                {
                    ...
                    List<string> itemids = data.UserID;
                    foreach (string id in itemids)
                    {
                        UserInfo item = GetUser(id, out tempMessage);
                        debugMessage += tempMessage;

                        if (CanManageUser(item, out tempMessage))
                        {
                            switch (action)
                            {
                                case "delete":
                                    item = DeleteUser(item, out success, out tempMessage);
                                ...
                                case "copy":
                                    item = CopyUser(item, out success, out tempMessage);
                                    if (success && item != null)
                                    {
                                        if (!String.IsNullOrWhiteSpace(CopyRedirectURL))
                                        {
                                            redirectURL = CopyRedirectURL + "?UserID=" + item.UserID.ToString() + "&Action=Copy";
                                        }
                                    }
                                    break;

                                case "enable":
                                    item = EnableUser(item, true, out success, out tempMessage);
                                    if (success && item != null)
                                    {
                                ...
                                case "disable":
                                    item = EnableUser(item, false, out success, out tempMessage);
                                    if (success && item != null)
                                    {
                                        if (!String.IsNullOrWhiteSpace(RedirectURL))
                                        {
        ...
        /// <summary>
        /// Check to see if we can manage the user
        /// </summary>
        /// <param name="p_item"></param>
        /// <param name="p_message"></param>
        /// <returns></returns>
        protected bool CanManageUser(UserInfo p_item, out string p_message)
        {
            bool ret = false;
            ...
            if (p_item != null)
            {
                // allow edit of all user except when it's GlobalAdmin, Admin, Editor, or the same person
        ...
        /// <param name="p_message"></param>
        /// <returns></returns>
        protected UserInfo DeleteUser(UserInfo p_item, out bool p_success, out string p_message)
        {
            p_success = false;
        ...
        /// <summary>
        /// Copy the specific user
        /// </summary>
        /// <param name="p_item"></param>
        /// <param name="p_success"></param>
        /// <param name="p_message"></param>
        /// <returns></returns>
        protected UserInfo CopyUser(UserInfo p_item, out bool p_success, out string p_message)
        {
            p_success = false;
            p_message = String.Empty;
        ...
        /// <param name="p_redirectionURL"></param>
        /// <returns></returns>
        protected UserInfo EnableUser(UserInfo p_item, bool p_enable, out bool p_success, out string p_message)
        {
            p_success = false;
            p_message = String.Empty;
            ...
            if (p_item != null)
            {
                try
                {
                    p_item.Enabled = p_enable;
                    p_item.Update();
                    string enableString = p_enable ? "enabled" : "disabled";
                    p_message = String.Format("User {0}({1}) has been {2} <br/>", p_item.UserName, p_item.UserID, enableString);
                    p_success = true;
                }
                catch (Exception ex)
                {
        ...
        /// <param name="p_messsage"></param>
        /// <returns></returns>
        protected UserInfo GetUser(object p_itemid, out string p_message)
        {
            UserInfo item = null;
            p_message = String.Empty;
            ...
            if (key > 0)
            {
                ...
                item = UserInfoProvider.GetUserInfo(key);
                if (item == null)
                {
