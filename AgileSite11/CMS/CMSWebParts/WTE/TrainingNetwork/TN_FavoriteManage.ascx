<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="TN_FavoriteManage.ascx.cs" Inherits="CMSApp.CMSWebParts.WTE.TrainingNetwork.TN_FavoriteManage" %>
...
<div id="divtest" runat="server" visible="false">
    ...
    <script type="text/javascript">
        function btnTest_Clicked() {
            PerformMultipleManageAction('ddlManageActionTest', 'favoriteselecttest', 'hdnPostbackUrl')
        }
        ...
        function favoriteselectalltest_checkchanged() {
            doselectall('favoriteselecttest_all', 'favoriteselecttest');
        }
    </script>

    <div class="inline-form-group">
        1:
    <input type="checkbox" id="favoriteselecttest_1" checked="checked" value="1" />
        2:
    <input type="checkbox" id="favoriteselecttest_2" checked="checked" value="2" />
        3:
    <input type="checkbox" id="favoriteselecttest_3" checked="checked" value="3" />
    </div>
    ...
    <div class="inline-form-group">
        <label for="favoriteselecttest_all">select all:</label><input type="checkbox" id="favoriteselecttest_all" onchange="favoriteselectalltest_checkchanged();return false;" />
    </div>
...
<script type="text/javascript">
    ...
    function chkManageSelectAll_checkchanged() {
        doselectall('select_all', 'favoriteselect');
    }

    // button clicked
    function btnPerformMultipleManageAction_Clicked() {
        PerformMultipleManageAction('ddlManageAction', 'favoriteselect', 'hdnPostbackUrl')
    }
    ...
    if (typeof PerformMultipleManageAction !== 'function' || typeof PerformMultipleManageAction === 'undefined') {
        ...
        function PerformMultipleManageAction(actionDropdownName, checkboxName, postbackUrlName) {
            ...
            if (selectedid.length > 0) {
                var actionstring = 'Are you sure you want to ' + action + ' all selected favorites?'
                if (confirm(actionstring)) {
                    var payload = '{"Action":"' + action + '","FavoriteID":[' + selectedid + ']}';
    ...
    if (typeof PerformSingleManageAction !== 'function' || typeof PerformSingleManageAction === 'undefined') {
        ...
        function PerformSingleManageAction(actionName, id, postbackUrlName) {
            ...
            if (selectedid.length > 0) {
                var actionstring = 'Are you sure you want to ' + action + ' this favorite?'
                if (confirm(actionstring)) {
                    var payload = '{"Action":"' + action + '","FavoriteID":[' + selectedid + ']}';
                    var transResponse = JSON.parse(payload);
                    RedirectAndPostJSON(postbackUrl, transResponse);
...
<script type="text/javascript">
        ...
        function PerformMultipleProductAssignmentAction(actionDropdownName, checkboxName, postbackUrlName) {
            ...
            $('input[type=checkbox][id*=' + cbname + ']').each(function () {
                ...
                if (checked == true) {
                    ...
                    var favoriteid = getControlValue('hdnFavoriteID_' + id);

                    var assignmentdata = '{"AssignmentID":"' + assignmentid + '","FavoriteID":"' + favoriteid + '","FavoriteID":"' + nodeid + '","VideoID":"' + videoid + '"}';
            ...
            if (selectedid.length > 0) {
                var actionstring = 'Are you sure you want to ' + action + ' all selected products/category to favorite?'
                if (confirm(actionstring)) {
    ...
    if (typeof PerformSingleProductAssignmentAction !== 'function' || typeof PerformSingleProductAssignmentAction === 'undefined') {
        // perform the selected action
        function PerformSingleProductAssignmentAction(actionName, id, postbackUrlName) {
