...
namespace CMS.URLRewritingEngine
{
    ...
    public class URLRewriter
    {
        ...
        /// Cache item priority.
        /// </summary>
        public static CacheItemPriority CacheItemPriority = CacheItemPriority.NotRemovable;
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        else
                                                        {
                                                            ...
                                                            if (isCrawler || !RedirectToDocumentRedirect(pageInfo, originalQuery, action, viewMode.Value, siteName.Value))
                                                            {
                                                                ...
                                                                // Try to get a MVC handler for the current page info (successful only when there is a MVC route defined for the document)
                                                                IHttpHandler mvcHandler = CMSMvcHelper.GetMVCPageHandler(pageInfo, null, ref status);
                                                                ...
                                                                if (mvcHandler != null)
                                                                {
                                                                    URLRewritingContext.HttpHandler = mvcHandler;
        ...
        private static void RedirectToPageNotFound(SiteNameOnDemand siteName, ViewModeOnDemand viewMode)
        {
            ...
            if (String.IsNullOrEmpty(path))
            {
                ...
                if (pi != null)
                {
                    if (CMSMvcHelper.IsMVCPageInfo(pi))
                    {
                        RequestStatusEnum status = RequestStatusEnum.Unknown;
                        var mvcHandler = CMSMvcHelper.GetMVCPageHandler(pi, null, ref status);
                        if (mvcHandler != null)
                        {
                            ProcessRoutePageNotFound(mvcHandler);
                            return;
                        }
                    }
                ...
                {
                    // Try MVC/Route page not found
                    RewritePath(url);

                    // Ensure reload for AppRelativeCurrentExecutionFilePath property
                    RequestContext.CurrentRelativePath = null;
                    RequestContext.CurrentExcludedStatus = ExcludedSystemEnum.Unknown;

                    var data = RouteTable.Routes.GetRouteData(context);
                    if (data != null)
                    {
                        var requestContext = new System.Web.Routing.RequestContext(context, data);
                        IHttpHandler handler = Mvc.MvcAdapter.GetMvcHandler(requestContext);
                        ProcessRoutePageNotFound(handler);
                        return;
                    }
                }
            }
        ...
        {
            return RequestHelper.IsPostBack() && request.Form["__CMSCsrfToken"] == null;
        }
        ...
        /// <summary>
        /// Process route/MVC page not found page
        /// </summary>
        /// <param name="handler">IHttpHandler</param>
        private static void ProcessRoutePageNotFound(IHttpHandler handler)
        {
        ...
        {
            bool originalPrefferedCultureIsEmpty = String.IsNullOrEmpty(culture.Value);

            // Validate the culture
            if (originalPrefferedCultureIsEmpty || !CultureSiteInfoProvider.IsCultureAllowed(culture.Value, siteName.Value))
            {
                ...
                else
                {
                    ...
                    SetLanguage(siteName, viewMode, culture.Value, !originalPrefferedCultureIsEmpty);
                }
            }
            else
            {
        ...
        {
            // Use live URL inside Pages application for pages which use MVC (either directly or MVC template)
            PageTemplateInfo pti = PageTemplateInfoProvider.GetPageTemplateInfo(node.GetUsedPageTemplateId());
            if (((pti != null) && (pti.PageTemplateType == PageTemplateTypeEnum.MVC))
                || node.DocumentUrlPath.StartsWithCSafe(TreePathUtils.URL_PREFIX_MVC))
            {
        ...
        internal static bool RedirectToMainUrl(SiteNameOnDemand siteName, PageInfo pageInfo, PageInfoSource pageInfoSource, string wildCardQueryString, string originalQuery, UrlRewritingAction action, PreferredCultureOnDemand culture)
        {
            ...
            if ((action != null) && !String.IsNullOrEmpty(action.RedirectURL))
            {
                return false;
            }
        ...
        private static string UpdatePreviewHashForVirtualContext(string query, string path)
        {
            ...
            var url = VirtualContext.AddPreviewHash(path + query);
        ...
        public static bool SendOutputFromCache(string relativePath, ExcludedSystemEnum excludedEnum, ViewModeOnDemand viewMode, SiteNameOnDemand siteName)
        {
            ...
            if (!ResponseOutputFilter.SendOutputFromCache(viewMode, siteName, out output))
            {
                return false;
            }
        ...
        {
            if (!ResponseOutputFilter.UseOutputFilterCache || (excludedEnum != ExcludedSystemEnum.NotExcluded))
            {
                return false;
            }
