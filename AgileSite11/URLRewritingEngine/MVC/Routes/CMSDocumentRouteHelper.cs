...
namespace CMS.URLRewritingEngine
{
    ...
    public static class CMSDocumentRouteHelper
    {
        ...
        private static CMSStatic<SafeDictionary<string, List<RouteBase>>> mRoutes = new CMSStatic<SafeDictionary<string, List<RouteBase>>>(() => new SafeDictionary<string, List<RouteBase>>());

        // Table of the registers routes by the document [node key] -> [List of RouteBase]
        private static CMSStatic<SafeDictionary<string, List<RouteBase>>> mRoutesByDocuments = new CMSStatic<SafeDictionary<string, List<RouteBase>>>(() => new SafeDictionary<string, List<RouteBase>>());
        ...
        /// Table of the registered routes [siteName] -> [List of RouteBase]
        /// </summary>
        private static SafeDictionary<string, List<RouteBase>> Routes
        {
            get
            {
                return mRoutes;
        ...
        /// Table of the registers routes by the document [node key] -> [List of RouteBase]
        /// </summary>
        private static SafeDictionary<string, List<RouteBase>> RoutesByDocuments
        {
            get
            {
                return mRoutesByDocuments;
            }
            set
            {
                mRoutesByDocuments.Value = value;
            }
        }
        ...
        public static void DropAllRoutes(bool logWebFarmTask = true)
        {
            ...
            if (logWebFarmTask)
            {
                WebFarmHelper.CreateTask(RouteTableTaskType.DropAllRoutes);
            }
        }
        ...
        public static void DropRoutes(string siteName)
        {
            lock (routeLock)
            {
                string lowerSiteName = siteName.ToLowerCSafe();
        ...
        private static void RegisterDocumentRoutes(string where, string siteName, List<RouteBase> routes)
        {
            ...
            var query = tree.SelectNodes()
                           .All()
                           .Where(where)
                           .Where(new WhereCondition().WhereStartsWith("DocumentURLPath", TreePathUtils.URL_PREFIX_MVC).Or().WhereStartsWith("DocumentURLPath", TreePathUtils.URL_PREFIX_ROUTE))
        ...
        {
            where = SqlHelper.AddWhereCondition(where, String.Format("AliasURLPath LIKE '{0}%' OR AliasURLPath LIKE '{1}%'", TreePathUtils.URL_PREFIX_MVC, TreePathUtils.URL_PREFIX_ROUTE));
        ...
        private static Route RegisterRoute(string siteName, string aliasPath, string urlPath, int nodeId, string culture, bool alias)
        {
            ...
            if (separatorIndex > 0)
            {
                ...
                // Register the route
                string name = Guid.NewGuid() + ":" + siteName + ":" + aliasPath + ":" + culture;
                ...
                try
                {
                    ...
                    {
                        // MVC route
                        var constraint = new CMSMvcDocumentConstraint(siteName, aliasPath, nodeId, culture, urlPath, pageInfoSource);

                        // Register the routes
                        route = new CMSRoute(name, pattern, MvcAdapter.GetMvcRouteHandler());

                        route.Defaults = new RouteValueDictionary { { "DefaultValues", defaults } };
                        route.Constraints = new RouteValueDictionary { { "Document", constraint } };
                        route.DataTokens = new RouteValueDictionary();
                    ...
                    else if (prefix.StartsWithCSafe(TreePathUtils.URL_PREFIX_ROUTE, true))
                    {
                        ...
                        var handler = new CMSDocumentRouteHandler(siteName, aliasPath, nodeId, culture, null);
                    ...
                    {
                        key += "|culture|" + culture.ToLowerCSafe();
                    }
        ...
        /// <param name="nodeId">Node ID</param>
        /// <param name="culture">Document culture</param>
        public static void DropRoutes(int nodeId, string culture)
        {
            lock (routeLock)
            {
                // Find out if the route is registered
                string key = nodeId + "|culture|" + culture.ToLowerCSafe();
        ...
        /// </summary>
        /// <param name="nodeId">Node ID</param>
        public static void DropAliasRoutes(int nodeId)
        {
            lock (routeLock)
            {
                // Find out if the route is registered
        ...
        public static void RefreshRoutes(TreeNode node, bool logWebFarmTask = true)
        {
            ...
            if (logWebFarmTask)
            {
                WebFarmHelper.CreateTask(RouteTableTaskType.RefreshRoutes, null, node.DocumentID.ToString());
            }
        }
        ...
        public static void RefreshAliasRoutes(int nodeId, string siteName, bool logWebFarmTask = true)
        {
            ...
            if (logWebFarmTask)
            {
                WebFarmHelper.CreateTask(RouteTableTaskType.RefreshAliasRoutes, null, nodeId.ToString(), siteName);
            }
        }
        ...
        private static string ParseWildcardElement(Match m, RouteValueDictionary defaults)
        {
            ...
            bool hidden = name.StartsWithCSafe("*") && name.EndsWithCSafe("*");
            if (hidden)
            {
                name = name.Trim('*');
            }
            ...
            bool optional = name.StartsWithCSafe("?");
            if (optional)
            {
            ...
            {
                value = MvcAdapter.OptionalUrlParameter;
            }
    ...
    /// <summary>
    /// Web farm tasks for operations over the routing table
    /// </summary>
    internal static class RouteTableTaskType
    {
        /// <summary>
        /// Drop all routes.
        /// </summary>
        public const string DropAllRoutes = "DROPALLROUTES";


        /// <summary>
        /// Refresh page routes.
        /// </summary>
        public const string RefreshRoutes = "REFRESHROUTES";


        /// <summary>
        /// Refresh alias routes.
        /// </summary>
        public const string RefreshAliasRoutes = "REFRESHALIASROUTES";
    }
    ...
    internal static class RouteTableSynchronization
    {
        ...
        public static void Init()
        {
            ...
            {
                Type = RouteTableTaskType.DropAllRoutes,
                Execute = DropAllRoutes,
                IsMemoryTask = true
            });

            WebFarmHelper.RegisterTask(new WebFarmTask
            {
                Type = RouteTableTaskType.RefreshRoutes,
                Execute = RefreshRoutes,
                IsMemoryTask = true
            });

            WebFarmHelper.RegisterTask(new WebFarmTask
            {
                Type = RouteTableTaskType.RefreshAliasRoutes,
                Execute = RefreshAliasRoutes,
                IsMemoryTask = true
            });
        }
        ...
        /// <summary>
        /// Drops all CMS routes from the route table.
        /// </summary>
        /// <param name="target">Task target</param>
        /// <param name="data">Task data</param>
        /// <param name="binaryData">Task binary data</param>
        private static void DropAllRoutes(string target, string[] data, BinaryData binaryData)
        {
        ...
        /// <summary>
        /// Refreshes routes for a particular document
        /// </summary>
        private static void RefreshRoutes(string target, string[] data, BinaryData binaryData)
        {
            if (data.Length != 1)
            {
                throw new ArgumentException(String.Format("[RouteTableSynchronization.RefreshRoutes]: Expected 1 data argument, but received {0}", data.Length));
            }
        ...
        /// <summary>
        /// Refreshes routes for particular document aliases
        /// </summary>
        /// <param name="target">Task target</param>
        /// <param name="data">Task data</param>
        /// <param name="binaryData">Task binary data</param>
        private static void RefreshAliasRoutes(string target, string[] data, BinaryData binaryData)
        {
            if (data.Length != 2)
            {
                throw new ArgumentException(String.Format("[RouteTableSynchronization.RefreshAliasRoutes]: Expected 2 data argument, but received {0}", data.Length));
            }

            int nodeId = ValidationHelper.GetInteger(data[0], 0);
