...
namespace CMS.FormEngine.Web.UI
{
    ...
    public class UIContextHelper
    {
        ...
        public static bool CheckElementVisibilityCondition(UIElementInfo ui)
        {
            ...
            if (MacroProcessor.ContainsMacro(ui.ElementVisibilityCondition))
            {
                // Resolve task condition and if is not match -> do not show element
                if (!ValidationHelper.GetBoolean(MacroResolver.Resolve(ui.ElementVisibilityCondition), false))
                {
                    return false;
                }
            }
