...
namespace CMS.FormEngine.Web.UI
{
    ...
    public class SpecialFieldsDefinition : List<SpecialField>
    {
        ...
        public void FillItems(ListItemCollection items)
        {
            if (SortItems)
            {
            ...
            for (int i = Count - 1; i >= 0; --i)
            {
                ...
                InsertResolvedItem(this[i], items, resolver);
            }
        }
        ...
        private void InsertResolvedItem(SpecialField field, ListItemCollection items, MacroResolver resolver)
        {
            ...
            if (visible && insertAllowed)
            {
                ...
                if (SetUniqueIDs)
                {
                    item.Attributes.Add(DATA_HASH_ATTRIBUTE, ValidationHelper.GetHashString(item.Value));
                }
                items.Insert(0, item);
            }
        }
