...
namespace CMS.WebFarmSync
{
    ...
    internal sealed class WebFarmTaskProcessor : ThreadWorker<WebFarmTaskProcessor>
    {
        ...
        /// <summary>
        /// Creates <see cref="ThreadSettings"/> object and sets <see cref="ThreadSettings.IsBackground"/> to <c>true</c> and <see cref="ThreadSettings.UseEmptyContext"/> to <c>false</c>.
        /// </summary>
        protected override ThreadSettings CreateThreadSettings()
        {
            var originalSettings = base.CreateThreadSettings();
            originalSettings.UseEmptyContext = false;
        ...
        /// <summary>
        /// Returns all system tasks for the given server.
        /// </summary>
        private ObjectQuery<WebFarmTaskInfo> GetSystemTasks()
        {
            return GetServerTasks()
                .Where(w => w
                    ...
                    .Or()
                    .WhereContains("TaskTarget", SystemTaskType.RestartApplication)
                    .Or()
                    ...
                    .Or()
                    .Where(w2 => w2
                        .WhereEquals("TaskType", CacheTaskType.TouchCacheItem)
                        .And()
                        .Where(w3 => w3
        ...
        internal bool ProcessTasks(DataSet tasks, Action<List<int>> deleteTasks)
        {
            ...
            foreach (DataRow task in tasks.Tables[0].Rows)
            {
                ...
                catch (Exception e)
                {
                    ...
                    {
                        EventLogProvider.LogException("WebFarmTaskProcessor", "ProcessTasks", e);
                    }
                }
            }
        ...
        private void ProcessTask(string taskType, string target, string[] data, int taskId, bool hasData)
        {
            ...
            {
                // Log the operation
                DataRow ldr = WebFarmDebug.LogWebFarmOperation("DO: " + taskType, data.Join(WebFarmTaskInfo.TASK_DATA_SEPARATOR), null, target);
        ...
        {
            var parsedTarget = new string[] { null };
            var parsedData = new[] { new string[] { } };
            ...
            if (data != null)
            {
                parsedData = data.Split(new[] { WebFarmTaskInfo.MULTIPLE_TASK_DATA_SEPARATOR }, StringSplitOptions.None)
                    .Select(d => d.Split(new[] { WebFarmTaskInfo.TASK_DATA_SEPARATOR }, StringSplitOptions.None)).ToArray();
            }
