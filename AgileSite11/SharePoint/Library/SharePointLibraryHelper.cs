        ...
        protected virtual void UploadFileInternal(SharePointLibraryInfo sharePointLibrary, SystemIO.Stream contentStream, string libraryRelativeUrl)
        {
            ...
            if (fileToBeCreatedDataRow != null)
            {
                using (CMSTransactionScope transactionScope = new CMSTransactionScope())
                {
                    ...
                    var oldFile = SharePointFileInfoProvider.GetSharePointFiles(sharePointLibrary.SharePointLibraryID).WhereEquals("SharePointFileServerRelativeURL", serverRelativeUrl).FirstObject;
                    if (oldFile != null)
                    {
            ...
            if (fileToBeCreatedDataRow != null)
            {
                using (CMSTransactionScope transactionScope = new CMSTransactionScope())
                {
                    if (sharePointFile.SharePointFileServerRelativeURL != newServerRelativeUrl)
                    {
                        ...
                        var oldFile = SharePointFileInfoProvider.GetSharePointFiles(libraryInfo.SharePointLibraryID).WhereEquals("SharePointFileServerRelativeURL", newServerRelativeUrl).FirstObject;
                        if (oldFile != null)
                        {
                            SharePointFileInfoProvider.DeleteSharePointFileInfo(oldFile);
                        }
