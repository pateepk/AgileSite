...
namespace CMS.DocumentEngine
{
    ...
    public class DocumentSearchIndexer : SearchIndexer
    {
        ...
        public override void PartialRebuild(SearchTaskInfo taskInfo)
        {
            ...
            {
                var searchables = DocumentSearchablesRetriever.GetSearchablesObjects(index, nodeAliasPath, siteName);
        ...
        {
            var tree = new TreeProvider(null);
        ...
        protected override void DeleteItem(SearchTaskInfo taskInfo, SearchIndexInfo indexInfo)
        {
            ...
            if (taskInfo.SearchTaskField == SearchFieldsConstants.PARTIAL_REBUILD)
            {
                ...
                {
                    var searcher = indexInfo.Provider.GetSearcher();
                    if ((searcher != null) && (searcher.NumberOfDocuments() > 0))
                    {
        ...
        public override bool FillSearchResult(DataRow dr, DataRow resultDr, ILuceneSearchDocument doc)
        {
            ...
            if (result)
            {
                dr["documentExtensions"] = DataHelper.GetStringValue(resultDr, "DocumentExtensions");

                ValidatePreviewImage(dr, resultDr);
                ...
                if (String.Equals(dci.ClassSearchContentColumn, "documentcontent", StringComparison.InvariantCultureIgnoreCase))
                {
                    ...
                    if (!string.IsNullOrEmpty(content))
                    {
                        ...
                        dr["content"] = editableContentProvider.GetSearchContent();
                    }
                }
        ...
        private static void ValidatePreviewImage(DataRow dr, DataRow resultDr)
        {
            var siteId = ValidationHelper.GetInteger(resultDr["NodeSiteID"], 0);
            var attachmentGuid = ValidationHelper.GetGuid(dr["image"], Guid.Empty);
        ...
        public override string GetSearchImageUrl(string id, string type, string imageUrl)
        {
            // Get base image URL
            var result = base.GetSearchImageUrl(id, type, imageUrl);
            ...
            // Attachment guid
            var guid = ValidationHelper.GetGuid(imageUrl, Guid.Empty);
            if (guid != Guid.Empty)
            {
            ...
            {
                string siteName = ValidationHelper.GetString(GetSearchValue(id, "SiteName"), String.Empty);

                result = DocumentURLProvider.GetUrl(imageUrl, null, siteName);
            }
