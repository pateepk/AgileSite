...
namespace CMS.DocumentEngine
{
    ...
    internal static class DocumentImport
    {
        ...
        private static void ImportDocumentsData(SiteImportSettings settings, TranslationHelper th)
        {
            CancelImportIfCanceled(settings);

            try
            {
                if (settings.IsLowerVersion("9.0"))
                {
                    // Backward compatibility with older packages. Objects that reference DocumentCultureDataInfo used to reference TreeNode.
                    CreateDocumentCultureDataRecords(th);
                }
        ...
        /// <summary>
        /// Creates translation records for type <see cref="DocumentCultureDataInfo.OBJECT_TYPE"/> in translation table by 
        /// duplicating all records of type <see cref="TreeNode.OBJECT_TYPE"/>.
        /// </summary>
        /// <remarks>
        /// Method ensures backward compatibility with older import packages. Objects that reference <see cref="DocumentCultureDataInfo"/> used to reference
        /// <see cref="TreeNode"/> in previous versions. 
        /// </remarks>
        private static void CreateDocumentCultureDataRecords(TranslationHelper th)
        {
            var documentRows = th.TranslationTable.AsEnumerable()
                                 .Where(row => String.Equals(
                                     row.Field<string>(TranslationHelper.RECORD_CLASS_NAME_COLUMN),
                                     TranslationHelper.GetSafeClassName(TreeNode.OBJECT_TYPE),
                                     StringComparison.InvariantCultureIgnoreCase))
                                 .Select(row =>
                                 {
                                     var newRow = th.TranslationTable.NewRow();
                                     newRow.ItemArray = row.ItemArray;
                                     newRow.SetField(TranslationHelper.RECORD_CLASS_NAME_COLUMN, TranslationHelper.GetSafeClassName(DocumentCultureDataInfo.OBJECT_TYPE));
                                     newRow.SetField(TranslationHelper.RECORD_OBJECT_TYPE_COLUMN, DocumentCultureDataInfo.OBJECT_TYPE);
                                     return newRow;
                                 })
                                 .ToList();

            documentRows.ForEach(th.TranslationTable.Rows.Add);
        }
        ...
        private static HashSet<int> ImportACLs(SiteImportSettings settings, TranslationHelper th, DataSet aclDS, Dictionary<int, int> importACLs, int currentRootACLID, int originalRootACLID)
        {
            ...
            foreach (DataRow dr in aclsDT.Rows)
            {
                ...
                try
                {
                    ...
                    if (settings.IsLowerVersion("9.0"))
                    {
                        // Store original ACL owners
                        aclOwners.Add(Convert.ToInt32(dr["ACLOwnerNodeID"]));
                    }
                            ...
                            if (change)
                            {
                                ...
                                if (node != null)
                                {
                                    ...
                                    if (node.SetIntegerValue("NodeACLID", newACLId, false))
                                    {
                                        ...
                                        if (settings.IsLowerVersion("9.0"))
                                        {
                                            // Mark document as ACL owner
                                            if (backwardCompatibilityACLOwners.Contains(oldNodeId))
                                            {
