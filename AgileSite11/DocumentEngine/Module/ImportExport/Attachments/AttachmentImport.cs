...
namespace CMS.DocumentEngine
{
    ...
    internal static class AttachmentImport
    {
        ...
        public static void Init()
        {
            ...
            ImportExportEvents.ImportObjectType.Before += CleanTemporaryFilesLocationSetting;
            ImportExportEvents.ImportObjectType.After += CleanTemporaryFilesLocationSetting;
        }


        private static void CleanTemporaryFilesLocationSetting(object sender, ImportDataEventArgs e)
        {
            if (e.Settings.IsLowerVersion("9.0"))
            {
                var objectType = e.ObjectType;
                if (objectType == SettingsKeyInfo.OBJECT_TYPE)
                {
                    // Delete global temporary setting
                    var tempLocationType = SettingsKeyInfoProvider.GetSettingsKeyInfo(TEMP_FILES_LOCATION_TYPE_SETTING_NAME);
                    if (tempLocationType != null)
                    {
                        tempLocationType.Delete();
                    }
                }
            }
        }
        ...
        private static void ProcessMainObject_Before(object sender, ImportEventArgs e)
        {
            if (e.Settings.IsLowerVersion("9.0"))
            {
                ...
                if (objectType == SettingsKeyInfo.OBJECT_TYPE)
                {
                    var settingKey = (SettingsKeyInfo)e.Object;
                    if (keysToRemove.Contains(settingKey.KeyName))
                    {
                        ConvertSettingsForFilesLocationType(settingKey);
