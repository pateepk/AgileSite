...
namespace CMS.DocumentEngine
{
    ...
    public static class DocumentSynchronizationHelper
    {
        ...
        private static DataSet GetDocumentDataSet(TreeNode node, TranslationHelper th, TaskTypeEnum taskType, TaskDataTypeEnum dataType, TaskParameters taskParams, string siteName)
        {
            ...
            string originalSiteName = SiteInfoProvider.GetSiteName(node.OriginalNodeSiteID);
            ...
            if (translationsNotPresent)
            {
                ...
                th.RegisterRecord(node.NodeSiteID, new TranslationParameters(SiteInfo.OBJECT_TYPE) { CodeName = node.NodeSiteName });
                if (node.IsLink)
                {
                    th.RegisterRecord(node.OriginalNodeSiteID, new TranslationParameters(SiteInfo.OBJECT_TYPE) { CodeName = originalSiteName });
                }
            }
                ...
                case TaskTypeEnum.UpdateDocument:
                    {
                        ...
                        if (includeChildren)
                        {
                            ...
                            if (!DataHelper.DataSourceIsEmpty(attachmentsDS))
                            {
                                ...
                                EnsureAttachmentsData(attData, originalSiteName);
                            ...
                            if (aclItemsTable != null)
                            {
                                foreach (DataRow dr in aclItemsTable.Rows)
                                {
                                    ...
                                    if ((userId > 0) && translationsNotPresent && !th.RecordExists(UserInfo.OBJECT_TYPE, userId))
                                    {
                                        ...
                                        {
                                            th.RegisterRecord(userId, new TranslationParameters(UserInfo.OBJECT_TYPE) { CodeName = userName });
                                        }
                                    }
                                    ...
                                    if ((roleId > 0) && translationsNotPresent && !th.RecordExists(RoleInfo.OBJECT_TYPE, roleId))
                                    {
                                        ...
                                        if (role != null)
                                        {
                                            th.RegisterRecord(roleId, new TranslationParameters(role.TypeInfo) { CodeName = role.RoleName, SiteName = siteName, GroupId = role.RoleGroupID });
                                        }
                                    }
                                    ...
                                    {
                                        string userName = UserInfoProvider.GetUserNameById(lastModifiedByUserId);
                                        if (!string.IsNullOrEmpty(userName))
                                        {
                                            th.RegisterRecord(lastModifiedByUserId, new TranslationParameters(UserInfo.OBJECT_TYPE) { CodeName = userName });
                                        }
                                    }
                                }
                            }
            ...
            if (includeBindings && (includeBindingData || includeBindingTranslations))
            {
                if (includeBindingTranslations && translationsNotPresent)
                {
                    ...
                    th.RegisterRecord((int)node.GetValue("NodeClassID"), new TranslationParameters(DataClassInfo.OBJECT_TYPE) { CodeName = node.NodeClassName }, DocumentTypeInfo.OBJECT_TYPE_DOCUMENTTYPE);
                }

                // Register page template translation and data
                RegisterDocumentPageTemplate(th, originalSiteName, data, translationsNotPresent, includeBindingData, includeBindingTranslations, node.NodeTemplateID);
                if (node.DocumentPageTemplateID != node.NodeTemplateID)
                {
                    RegisterDocumentPageTemplate(th, originalSiteName, data, translationsNotPresent, includeBindingData, includeBindingTranslations, node.DocumentPageTemplateID);
                }                
                ...
                if (includeBindingTranslations && translationsNotPresent)
                {
                    ...
                    if (node.DocumentStylesheetID > 0)
                    {
                        ...
                        if (csi != null)
                        {
                            th.RegisterRecord(node.DocumentStylesheetID, new TranslationParameters(csi.TypeInfo) { CodeName = csi.StylesheetName });
                        }
                    }
                    ...
                    if (linkedNodeId > 0)
                    {
                        ...
                        if (linkedNodeGuid != Guid.Empty)
                        {
                            ...
                            // Site record
                            th.RegisterRecord(linkedSi.SiteID, new TranslationParameters(linkedSi.TypeInfo) { CodeName = linkedSi.SiteName });
                        }
                    }
                }
                ...
                {
                    // Get the object
                    GeneralizedInfo skuInfo = ProviderHelper.GetInfoById(PredefinedObjectType.SKU, skuId);
                    if (skuInfo != null)
                    {
                        if (includeBindingTranslations && translationsNotPresent)
                        {
                            th.RegisterRecord(skuId, new TranslationParameters(PredefinedObjectType.SKU) { CodeName = skuInfo.ObjectCodeName });
                        }
                ...
                {
                    // Get the object
                    GeneralizedInfo groupInfo = ModuleCommands.CommunityGetGroupInfo(groupId);
                    if (groupInfo != null)
                    {
                        if (includeBindingTranslations && translationsNotPresent)
                        {
                            th.RegisterRecord(groupId, new TranslationParameters(groupInfo.TypeInfo) { CodeName = groupInfo.ObjectCodeName, SiteName = siteName });
                        }
                ...
                {
                    // Get the object
                    GeneralizedInfo tagGroupInfo = TagGroupInfoProvider.GetTagGroupInfo(tagGroupId);
                    if (tagGroupInfo != null)
                    {
                        if (includeBindingTranslations && translationsNotPresent)
                        {
                            th.RegisterRecord(tagGroupId, new TranslationParameters(tagGroupInfo.TypeInfo) { CodeName = tagGroupInfo.ObjectCodeName, SiteName = originalSiteName });
                        }
            ...
            {
                // Get the object
                GeneralizedInfo wfStepInfo = WorkflowStepInfoProvider.GetWorkflowStepInfo(stepId);
                if (wfStepInfo != null)
                {
                    if (translationsNotPresent)
                    {
                        th.RegisterRecord(stepId, new TranslationParameters(wfStepInfo.TypeInfo) { CodeName = wfStepInfo.ObjectCodeName, SiteName = originalSiteName });
                    }
                }
            }
        ...
        private static void RegisterDocumentPageTemplate(TranslationHelper th, string originalSiteName, DataSet ds, bool translationsNotPresent, bool includeBindingData, bool includeBindingTranslations, int templateId)
        {
            if (templateId > 0)
            {
                ...
                if (pti != null)
                {
                    if (includeBindingTranslations && translationsNotPresent)
                    {
                        ...
                        string ptSiteName = (pti.IsReusable ? null : originalSiteName);
                        th.RegisterRecord(templateId, new TranslationParameters(pti.TypeInfo) { CodeName = pti.CodeName, SiteName = ptSiteName });
                    }
        ...
        private static void RegisterUserTranslation(TreeNode node, TranslationHelper th, string columnName)
        {
            ...
            if (userId > 0)
            {
                if (!th.RecordExists(UserInfo.OBJECT_TYPE, userId))
                {
                    string userName = UserInfoProvider.GetUserNameById(userId);
                    if (!string.IsNullOrEmpty(userName))
                    {
                ...
                if (h.CanContinue())
                {
                    ...
                    if (CMSActionContext.LogDocumentChange())
                    {
                        ...
                        if (settings.LogStaging || settings.LogIntegration)
                        {
                            ...
                            if (!settings.WorkerCall)
                            {
                                settings.Node = settings.Node.Clone();
                            }
