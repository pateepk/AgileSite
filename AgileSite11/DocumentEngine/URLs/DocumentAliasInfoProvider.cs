...
namespace CMS.DocumentEngine
{
    ...
    public class DocumentAliasInfoProvider : AbstractInfoProvider<DocumentAliasInfo, DocumentAliasInfoProvider>
    {
        ...
        public static string CreateWildcardRule(string wildcardUrl, out int wildcardCount)
        {
            return ProviderObject.CreateWildcardRuleInternal(wildcardUrl, out wildcardCount);
        }
        ...
        {
            int wildcardSectionsCount;
            documentAlias.AliasWildcardRule = CreateWildcardRule(documentAlias.AliasURLPath, out wildcardSectionsCount);
        ...
        internal static DocumentAliasInfo GetDocumentAliasInternal(string path, string siteName, string currentCultureCode, bool combineWithDefaultCulture)
        {
            ...
            var query = GetDocumentAliases()
                ...
                .OrderByDescending(DocumentQueryColumnBuilder.GetURLSectionsCountPriority())
                .OrderByDescending(DocumentQueryColumnBuilder.GetWildcardCountPriority())
        ...
        /// <param name="wildcardUrl">Wildcard URL</param>
        /// <param name="wildcardCount">Number of matches</param>
        protected virtual string CreateWildcardRuleInternal(string wildcardUrl, out int wildcardCount)
        {
            wildcardCount = 0;
            ...
            if (re.Matches(wildcardUrl).Count <= 0)
            {
                return String.Empty;
            }

            var wildcardRule = re.Replace(wildcardUrl, "%");
            wildcardCount = GetWildcardSectionsCount(wildcardRule);
        ...
        {
            var ruleRegex = RegexHelper.GetRegex(@"((?<!\[)%[^/\]]*)", RegexOptions.None);
            return ruleRegex.Matches(wildcardRule).Count;
        }
        ...
        protected virtual string GetNormalizedUrlPathInternal(string urlPath)
        {
            ...
            if (urlPath.StartsWith(TreePathUtils.URL_PREFIX_ROUTE, StringComparison.InvariantCultureIgnoreCase) 
                || urlPath.StartsWith(TreePathUtils.URL_PREFIX_MVC, StringComparison.InvariantCultureIgnoreCase))
            {
        ...
        internal static WhereCondition GetMatchPathWhereCondition(string columnName, string wildcardRuleColumnName, string aliasUrlPath, bool containsWildcard)
        {
            ...
            {
                int wildcardCount;
                var wildcardRule = CreateWildcardRule(aliasUrlPath, out wildcardCount);
