    ...
    internal class FileSystemRestoreAllJob : AbstractFileSystemAllJob
    {
        ...
        protected override void RunInternal(CancellationToken cancellationToken)
        {
            using (new RepositoryActionContext { IsRestoreOperationRunning = true })
            {
                ...
                var orderedObjectTypes = objectTypeAnalyzer
                    .GetSequence()
                ...
                try
                {
                    ...
                    // Try restore custom table items of newly restored custom tables
                    RepositoryConfiguration.ReloadCustomTableObjectTypes();
                    var newCustomTableObjectTypes = RepositoryConfiguration.ObjectTypes.Except(orderedObjectTypes);

                    CancellableForEach(cancellationToken, newCustomTableObjectTypes, objectType =>
                    {
                        TryRun(
                            CreateAndUpsert,
                            objectType,
                    ...
                    // Try delete or dispose objects in reverse order 
                    var reversedOrderedObjectTypes = orderedObjectTypes.Reverse();

                    CancellableForEach(cancellationToken, reversedOrderedObjectTypes, objectType =>
                    {
                        TryRun(
                            DeleteAndDispose,
                            objectType,
