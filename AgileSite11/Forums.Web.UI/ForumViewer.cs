...
namespace CMS.Forums.Web.UI
{
    ...
    public class ForumViewer : CMSUserControl, IPostBackEventHandler
    {
        ...
        // Indicates whether friendship request is enabled
        private bool mEnableFriendship;

        // Indicates whether private messaging is enabled
        private bool mEnableMessaging;
        ...
        public bool EnableFriendship
        {
            get
            {
                return mEnableFriendship;
            }
            set
            {
                if (AuthenticationHelper.IsAuthenticated())
                {
                    mEnableFriendship = value;
                }
            }
        }
        ...
        public string GetURL(object data, ForumActionType type)
        {
            ...
            switch (type)
            {
                ...
                case ForumActionType.Forum:
                    url = URLCreator("ForumID", ValidationHelper.GetString(GetData(data, "ForumID"), ""), false, data);
                    url = URLHelper.RemoveParameterFromUrl(url, "threadid");
                    url = URLHelper.RemoveParameterFromUrl(url, "thread");
        ...
        public void CopyValues(ForumViewer frmv)
        {
            if (frmv != null)
            {
                ...
                frmv.EnableFriendship = EnableFriendship;
                frmv.EnableMessaging = EnableMessaging;
        ...
        {
            if (GenerateActionScripts && (EnableAvatars || EnableFriendship))
            {
                string script = "";

                // Friendship request
                script += "function FriendshipRequest(id) { \n" +
                          "modalDialog('" + ResolveUrl("~/CMSModules/Friends/CMSPages/Friends_Request.aspx") + "?userid=" + MembershipContext.AuthenticatedUser.UserID + "&requestid=' + id,'requestFriend', 670, 440); \n" +
                          " } \n";

                var query = "?userid=" + MembershipContext.AuthenticatedUser.UserID;
                query += "&hash=" + ValidationHelper.GetHashString(query, new HashSettings { HashSalt = "CMSPages|Messaging|SendMessage" });

                // Send private message
                script += "function PrivateMessage(id) { \n" +
                          "modalDialog('" + ResolveUrl("~/CMSModules/Messaging/CMSPages/SendMessage.aspx") + query + "&requestid=' + id , 'sendMessage', 750, 680); \n" +
                          " } \n";

                ScriptHelper.RegisterDialogScript(Page);
                ScriptHelper.RegisterClientScriptBlock(this, typeof(string), "PostActionScripts", ScriptHelper.GetScript(script));
            }
