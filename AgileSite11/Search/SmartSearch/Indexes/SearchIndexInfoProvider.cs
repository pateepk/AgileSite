...
{
    using TypedDataSet = InfoDataSet<SearchIndexInfo>;
    ...
    public class SearchIndexInfoProvider : AbstractInfoProvider<SearchIndexInfo, SearchIndexInfoProvider>
    {
        ...
        [Obsolete("Use method GetSearchIndexes() instead.")]
        public static TypedDataSet GetSearchIndexes(string where, string orderBy, int topN, string columns)
        {
            return ProviderObject.GetSearchIndexesInternal(where, orderBy, topN, columns);
        }
        ...
        /// <param name="columns">Selected columns</param>
        [Obsolete("Use method GetSearchIndexes() instead.")]
        protected virtual TypedDataSet GetSearchIndexesInternal(string where, string orderBy, int topN, string columns)
        {
            return GetSearchIndexes().Where(where).OrderBy(orderBy).TopN(topN).Columns(columns).BinaryData(true).TypedResult;
        }
        ...
        protected virtual void DeleteSearchIndexInfoInternal(SearchIndexInfo infoObj)
        {
            ...
            if (infoObj != null)
            {
                ...
                SearchHelper.InvalidateSearcher(infoObj.IndexGUID);
