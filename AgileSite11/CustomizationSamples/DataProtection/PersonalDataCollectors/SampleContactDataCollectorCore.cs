...
namespace DataProtection
{
    ...
    internal class SampleContactDataCollectorCore
    {
        ...
        private readonly List<CollectedColumn> newsletterInfoColumns = new List<CollectedColumn> {
            new CollectedColumn("NewsletterDisplayName", "Display name"),
            new CollectedColumn("NewsletterSenderName", "Sender name"),
            new CollectedColumn("NewsletterSenderEmail", "Sender email"),
            new CollectedColumn("NewsletterGUID", "GUID"),
            new CollectedColumn("NewsletterUnsubscribeUrl", "Unsubscribe URL"),
            new CollectedColumn("NewsletterTrackOpenEmails", "Track open emails"),
            new CollectedColumn("NewsletterTrackClickedLinks", "Track clicked links"),
        };
        ...
        public string CollectData(IEnumerable<BaseInfo> identities)
        {
            ...
            var contactActivities = ActivityInfoProvider.GetActivities()
            ...
            WriteNewsletters(contactEmails);
        ...
        private void WriteContacts(IEnumerable<ContactInfo> contacts)
        {
            foreach (var contactInfo in contacts)
            {
                ...
                if (countryID != 0)
                {
                    writer.WriteBaseInfo(CountryInfoProvider.GetCountryInfo(countryID), countryInfoColumns);
                }
                if (stateID != 0)
                {
                    writer.WriteBaseInfo(StateInfoProvider.GetStateInfo(stateID), stateInfoColumns);
                }
        ...
        {
            var consentsData = ConsentAgreementInfoProvider.GetConsentAgreements()
        ...
        private void WriteNewsletters(ICollection<string> contactEmails)
        {
            var contactSubscribedToNewsletters = NewsletterInfoProvider.GetNewsletters()
                   .Columns(newsletterInfoColumns.Select(t => t.Name))
                   .WhereIn("NewsletterId", SubscriberNewsletterInfoProvider.GetSubscriberNewsletters().Column("SubscriberID")
                                           .WhereIn("SubscriberID", SubscriberInfoProvider.GetSubscribers()
                                                                                          .WhereIn("SubscriberEmail", contactEmails)
                                                                                          .Column("SubscriberId")
                                                                                          .Distinct()));
            foreach (var newsletterInfo in contactSubscribedToNewsletters)
            {
                writer.WriteStartSection(NewsletterInfo.OBJECT_TYPE, "Newsletter");

                writer.WriteBaseInfo(newsletterInfo, newsletterInfoColumns);
        ...
        {
            var accountIDs = AccountContactInfoProvider.GetRelationships()
                .WhereIn("ContactID", contactIDs)
                .Column("AccountID")
                .Distinct();
            var accountInfos = AccountInfoProvider.GetAccounts()
                .Columns(accountInfoColumns.Select(t => t.Name))
                .WhereIn("AccountID", accountIDs)
                .ToList();

            var countryInfos = CountryInfoProvider.GetCountries()
                .WhereIn("CountryID", accountInfos.Select(r => r.AccountCountryID).ToList())
                .ToDictionary(ci => ci.CountryID);
            var stateInfos = StateInfoProvider.GetStates()
