...
{
    using TypedDataSet = InfoDataSet<ReportSubscriptionInfo>;
    ...
    public class ReportSubscriptionInfoProvider : AbstractInfoProvider<ReportSubscriptionInfo, ReportSubscriptionInfoProvider>
    {
        ...
        /// <summary>
        /// Returns dataset of all subscriptions matching the specified parameters.
        /// </summary>
        /// <param name="where">Where condition.</param>
        /// <param name="orderBy">Order by expression.</param>
        /// <param name="topN">Number of records to be selected.</param>        
        /// <param name="columns">Columns to be selected.</param>
        [Obsolete("Use method GetSubscriptions() instead")]
        public static TypedDataSet GetSubscriptions(string where, string orderBy, int topN, string columns)
        {
            return GetSubscriptions().Where(where).OrderBy(orderBy).TopN(topN).Columns(columns).TypedResult;
        }
        ...
        /// <param name="orderBy">Order by expression.</param>
        [Obsolete("Use method GetSubscriptions() instead")]
        public static TypedDataSet GetSubscriptions(string where, string orderBy)
        {
            return GetSubscriptions(where, orderBy, -1, null);
        }
