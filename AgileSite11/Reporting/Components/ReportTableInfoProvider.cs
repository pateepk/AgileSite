...
{
    using TypedDataSet = InfoDataSet<ReportTableInfo>;
    ...
    public class ReportTableInfoProvider : AbstractInfoProvider<ReportTableInfo, ReportTableInfoProvider>
    {
        ...
        [Obsolete("Use method GetReportTables() instead")]
        public static TypedDataSet GetTables(string where, string orderBy)
        {
            return GetTables(where, orderBy, -1, null);
        }
        ...
        [Obsolete("Use method GetReportTables() instead")]
        public static TypedDataSet GetTables(string where, string orderBy, int topN, string columns)
        {
            return GetReportTables().Where(where).OrderBy(orderBy).TopN(topN).Columns(columns).BinaryData(true).TypedResult;
        }
