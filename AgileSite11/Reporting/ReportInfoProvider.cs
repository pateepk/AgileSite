...
{
    using TypedDataSet = InfoDataSet<ReportInfo>;
    ...
    public class ReportInfoProvider : AbstractInfoProvider<ReportInfo, ReportInfoProvider>
    {
        ...
        [Obsolete("Use member GetReports() instead")]
        public static TypedDataSet GetReports(string where)
        {
            return GetReports().Where(where).TypedResult;
        }
        ...
        [Obsolete("Use member GetReports() instead")]
        public static TypedDataSet GetReports(string where, string orderBy, int topN, string columns)
        {
            return GetReports().Where(where).OrderBy(orderBy).TopN(topN).Columns(columns).BinaryData(true).TypedResult;
        }
