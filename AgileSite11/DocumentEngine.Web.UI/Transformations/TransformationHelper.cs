...
namespace CMS.DocumentEngine.Web.UI
{
    ...
    public class TransformationHelper : AbstractHelper<TransformationHelper>
    {
        ...
        /// <param name="maxSideSize">Max. side size</param>
        public string GetSearchImageUrl(string id, string type, string image, string noImageUrl, int maxSideSize)
        {
            string result;

            if (SearchHelper.CUSTOM_SEARCH_INDEX.Equals(type, StringComparison.OrdinalIgnoreCase))
            {
            ...
            {
                // Let indexer decide
                result = SearchIndexers.GetIndexer(type).GetSearchImageUrl(id, type, image);
            }
            ...
            if (!String.IsNullOrEmpty(result))
            {
                ...
                int siteId = ValidationHelper.GetInteger(GetSearchValue(id, "NodeSiteID"), 0);
        ...
        {
            return SearchHelper.Highlight(text, startTag, endTag);
        }
        ...
        public string GetSearchedContent(string id, string type, string content)
        {
            ...
            switch (type.ToLowerInvariant())
            {
                ...
                case PredefinedObjectType.FORUM:
                    if (content != null)
                    {
                        int forumId = ValidationHelper.GetInteger(GetSearchValue(id, "PostForumID"), 0);
        ...
        public string SearchResultUrl(string id, string type, bool absolute, bool addLangParameter = true)
        {
            ...
            switch (type.ToLowerInvariant())
            {
                ...
                case TreeNode.OBJECT_TYPE:
                    ...
                    // Get url path
                    string urlPath = Convert.ToString(GetSearchValue(id, "DocumentUrlPath"));

                    // If node is linked document, do not use url path
                    if (ValidationHelper.GetInteger(GetSearchValue(id, "NodeLinkedNodeID"), 0) > 0)
                    {
                        urlPath = null;
                    }

                    // Get node alias path
                    string nodeAlias = Convert.ToString(GetSearchValue(id, "NodeAliasPath"));
                    ...
                    bool nodeIsContentOnly = ValidationHelper.GetBoolean(GetSearchValue(id, "NodeIsContentOnly"), false);
                    if (nodeIsContentOnly)
                    {
                        var searchResult = SearchContext.CurrentSearchResults[id];
                    ...
                    if (SearchContext.MoreCultures && addLangParameter)
                    {
                        // Get current result item culture
                        string culture = GetSearchValue(id, "DocumentCulture") as string;
                ...
                case PredefinedObjectType.FORUM:
                    ...
                    if (forumId > 0)
                    {
                        ...
                        // Get post id path
                        string path = ValidationHelper.GetString(GetSearchValue(id, "PostIdPath"), String.Empty);
                ...
                case UserInfo.OBJECT_TYPE:
                    string userUrl = GetUserProfileURL(GetSearchValue(id, "UserName"));
            ...
            if (SearchHelper.CUSTOM_SEARCH_INDEX.Equals(type, StringComparison.OrdinalIgnoreCase))
            {
                string customUrl = Convert.ToString(GetSearchValue(id, SearchFieldsConstants.CUSTOM_URL));
