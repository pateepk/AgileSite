...
namespace CMS.Membership
{
    ...
    public class AvatarInfoProvider : AbstractInfoProvider<AvatarInfo, AvatarInfoProvider>
    {
            ...
            if (Directory.Exists(directoryPath))
            {
                ...
                if (filesInfos.Length > 0)
                {
                    ...
                    foreach (FileInfo file in filesInfos)
                    {
                        ...
                        if (!synchronization)
                        {
                            WebFarmHelper.CreateIOTask(MembershipTaskType.DeleteAvatar, file.FullName, null, "AvatarDelete", avatarGuid, fileExtension, deleteDirectory.ToString());
                        }
                    }
        ...
        public static void SaveAvatarFileToDisk(string guid, string fileName, string fileExtension, BinaryData fileData, bool synchronization)
        {
            if (!String.IsNullOrEmpty(fileName))
            {
                ...
                if (fileData != null)
                {
                    ...
                    if (!synchronization)
                    {
                        WebFarmHelper.CreateIOTask(MembershipTaskType.UpdateAvatar, filePath, fileData, "AvatarUpdate", guid, fileName, fileExtension);
                    }
        ...
        public AvatarInfo GetDefaultAvatarInternal(DefaultAvatarTypeEnum type)
        {
            ...
            var avatar = GetAvatars().Where(where).TopN(1).BinaryData(false).FirstObject;
            if (avatar != null)
            {
                // Store in cache
        ...
        {
            return GetObjectQuery().WhereEquals("AvatarName", avatarName).TopN(1).BinaryData(true).FirstObject;
        }
        ...
        {
            return GetObjectQuery().WhereEquals("AvatarID", avatarId).TopN(1).BinaryData(false).FirstObject;
        }
        ...
        {
            return GetObjectQuery().WhereEquals("AvatarGUID", guid).TopN(1).BinaryData(false).FirstObject;
        }
        ...
        {
            return GetObjectQuery().WhereEquals("AvatarName", avatarName).TopN(1).BinaryData(false).FirstObject;
        }
