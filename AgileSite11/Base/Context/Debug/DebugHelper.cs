...
namespace CMS.Base
{
    ...
    public static class DebugHelper
    {
        ...
        public static void ResetDebugSettings(bool createWebFarmTask = true)
        {
            ...
            {
                CoreServices.WebFarm.CreateTask(BaseSynchronization.ResetDebugSettingsTaskType);
            }
        }
        ...
        public static string GetRequestUrl(bool absolute = false)
        {
            string url;
            if (HttpContext.Current != null)
            {
#pragma warning disable BH1006 // 'Request.Url' should not be used. Use 'RequestContext.URL' instead.
                url = absolute ? HttpContext.Current.Request.Url.AbsoluteUri : HttpContext.Current.Request.RawUrl;
#pragma warning restore BH1006 // 'Request.Url' should not be used. Use 'RequestContext.URL' instead.
        ...
        private static bool CheckStackTrace(Func<MethodBase, bool> condition)
        {
        ...
        public static bool CheckStackTrace(params MethodInfo[] methods)
        {
            var items = methods.ToArray();

            return CheckStackTrace(m => CheckNext(items, m.Equals));
        }
        ...
        /// </summary>
        /// <param name="methods">Methods to check</param>
        public static bool CheckStackTrace(params string[] methods)
        {
            var items = methods.ToArray();

            return CheckStackTrace(m => CheckNext(items, item => GetMethodName(m).IndexOfCSafe(item, true) >= 0));
        }
        ...
        /// <param name="condition">Condition to check</param>
        private static bool CheckNext<T>(T[] items, Func<T, bool> condition)
            where T : class
        {
            var current = items[0];
            if (current == null)
            {
                // Checked last item, chain matches
                return true;
            }

            if (condition(current))
            {
                // Shift all array left
