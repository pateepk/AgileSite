...
namespace CMS.SocialMarketing
{
    ...
    internal class LinkedInInsightsHelper
    {
        ...
        private static bool ProcessLinkedInAccountInsights(LinkedInAccountInfo account, LinkedInAccountInsightsState accountInsightsState)
        {
            ...
            LinkedInAuthorization authorization = null;
            try
            {
                authorization = LinkedInAuthorizationHelper.GetAuthorizationByAccountInfo(account);
                if (authorization == null)
                {
                    throw new ArgumentException(String.Format("LinkedInAccountLinkedInApplicationID of LinkedIn account with ID {0} does not seem to be valid.", account.LinkedInAccountID));
                }
            }
            catch (ArgumentException ex)
            {
                // Catches both ArgumentNullException and ArgumentException
                EventLogProvider.LogWarning("Social marketing - LinkedIn account Insights", "ACCOUNTINSIGHTS", ex, account.LinkedInAccountSiteID,
                            String.Format("LinkedIn account Insights for account with ID {0} couldn't be retrieved from LinkedIn API. The OAuth tokens are not valid.", account.LinkedInAccountID));
            ...
            try
            {
                ...
                DateTime? recentlyCollected = CollectLinkedInAccountInsights(authorization, account.LinkedInAccountProfileID, accountInsightsState.LastCollectedDateTime);
            ...
            catch (LinkedInApiUnauthorizedException ex)
            {
                ...
                EventLogProvider.LogWarning("Social marketing - LinkedIn account Insights", "ACCOUNTINSIGHTS", ex, account.LinkedInAccountSiteID,
                        String.Format("LinkedIn API authorization (OAuth keys) invalid for account with ID {0} ({1}). Occurred while processing account insights.",
                                account.LinkedInAccountID, account.LinkedInAccountDisplayName));
            ...
            catch (LinkedInApiThrottleLimitException ex)
            {
                // Terminate collection of stats for given authorization tuple (company profile)
                EventLogProvider.LogWarning("Social marketing - LinkedIn account Insights", "ACCOUNTINSIGHTS", ex, account.LinkedInAccountSiteID,
                        String.Format("LinkedIn API throttle limit overdrown for account with ID {0} ({1}). Occurred while processing account insights.",
                                account.LinkedInAccountID, account.LinkedInAccountDisplayName));
            ...
            {
                EventLogProvider.LogWarning("Social marketing - LinkedIn account Insights", "ACCOUNTINSIGHTS", ex, account.LinkedInAccountSiteID,
                        String.Format("LinkedIn account Insights data for account with ID {0} ({1}) couldn't be retrieved from LinkedIn API.", account.LinkedInAccountID, account.LinkedInAccountDisplayName));
        ...
        private static bool ProcessLinkedInPostsInsights(LinkedInAccountInfo account)
        {
            ...
            catch (ArgumentException ex)
            {
                ...
                EventLogProvider.LogWarning("Social marketing - LinkedIn post Insights", "POSTINSIGHTS", ex, account.LinkedInAccountSiteID,
                            String.Format("LinkedIn post Insights for account with ID {0} couldn't be retrieved from LinkedIn API. The OAuth tokens are not valid.", account.LinkedInAccountID));
            ...
            while (postsToUpdate.Any())
            {
                ...
                try
                {
                    UpdateLinkedInPostInsights(authorization, account.LinkedInAccountProfileID, postToUpdate);
                }
                catch (LinkedInApiUnauthorizedException ex)
                {
                    // Terminate collection of stats for given authorization tuple (company profile)
                    EventLogProvider.LogWarning("Social marketing - LinkedIn post Insights", "POSTINSIGHTS", ex, account.LinkedInAccountSiteID,
                            String.Format("LinkedIn API authorization (OAuth keys) invalid for account with ID {0} ({1}). Occurred while processing post with ID {2}.",
                                    account.LinkedInAccountID, account.LinkedInAccountDisplayName, postToUpdate.LinkedInPostID));
                ...
                catch (LinkedInApiThrottleLimitException ex)
                {
                    // Terminate collection of stats for given authorization tuple (company profile)
                    EventLogProvider.LogWarning("Social marketing - LinkedIn post Insights", "POSTINSIGHTS", ex, account.LinkedInAccountSiteID,
                            String.Format("LinkedIn API throttle limit overdrown for account with ID {0} ({1}) while processing post with ID {2}.",
                                    account.LinkedInAccountID, account.LinkedInAccountDisplayName, postToUpdate.LinkedInPostID));
                ...
                {
                    EventLogProvider.LogWarning("Social marketing - LinkedIn post Insights", "POSTINSIGHTS", ex, account.LinkedInAccountSiteID,
                            String.Format("LinkedIn post Insights data for post with ID {0} couldn't be retrieved from LinkedIn API.", postToUpdate.LinkedInPostID));
        ...
        /// </summary>
        /// <param name="authorization">Authorization for API requests.</param>
        ...
        private static DateTime? CollectLinkedInAccountInsights(LinkedInAuthorization authorization, string companyId, DateTime? lastCollectedDateTime)
        {
            ...
            HistoricalStatusUpdateStatistics statusUpdateAccountStats = GetLinkedInHistoricalStatusUpdateStatistics(authorization, companyId, sinceWhen);
            HistoricalFollowStatistics followerAccountStats = GetLinkedInHistoricalFollowerStatistics(authorization, companyId, sinceWhen);
        ...
        private static DateTime? ProcessLinkedInAccountStatusUpdateInsights(string companyId, HistoricalStatusUpdateStatistics statusUpdateAccountStats)
        {
            ...
            statusUpdateAccountStats.Statistics.OrderBy(it => it.Time).ToList().ForEach(it =>
            {
                ...
                for (int i = 0; i < accountHistoricalUpdateInsights.Length; ++i)
                {
                    ...
                    long value = 0;
                    switch (accountHistoricalUpdateInsights[i][0].ToLowerCSafe())
                    {
                        case "click-count":
                            value = it.ClickCount;
                            break;
        ...
        private static DateTime? ProcessLinkedInAccountFollowerInsights(string companyId, HistoricalFollowStatistics followerAccountStats)
        {
            ...
            followerAccountStats.Statistics.OrderBy(it => it.Time).ToList().ForEach(it =>
            {
                ...
                for (int i = 0; i < accountHistoricalFollowerInsights.Length; ++i)
                {
                    ...
                    long value = 0;
                    switch (accountHistoricalFollowerInsights[i][0].ToLowerCSafe())
                    {
        ...
        /// </summary>
        /// <param name="authorization">Access token for posts.</param>
        /// <param name="companyId">ID of the company on LinkedIn</param>
        ...
        /// <exception cref="LinkedInApiThrottleLimitException">Thrown when protocol error occurs because the limit of API requests was overdrown.</exception>
        /// <exception cref="LinkedInApiException">Thrown when protocol error occurs.</exception>
        private static void UpdateLinkedInPostInsights(LinkedInAuthorization authorization, string companyId, LinkedInPostInfo post)
        {
            HistoricalStatusUpdateStatistics postStats = GetLinkedInHistoricalStatusUpdateStatistics(authorization, companyId, post.LinkedInPostUpdateKey, post.LinkedInPostPublishedDateTime.Value);

            if ((postStats != null) && (postStats.Statistics != null) && postStats.Statistics.Any())
            {
                int clickCount = 0;
                int commentCount = 0;
        ...
        /// </summary>
        /// <param name="authorization">Authorization to be used for API querying.</param>
        ...
        /// </remarks>
        public static HistoricalFollowStatistics GetLinkedInHistoricalFollowerStatistics(LinkedInAuthorization authorization, string companyId, DateTime? sinceWhen = null)
        {
            ...
            ApiResponse response = LinkedInHelper.ProcessGetRequest(authorization, requestUri);
        ...
        /// Can be used to collect statistics for whole days only. This means the latest sinceWhen must represent UTC's yesterday (LinkedIn uses UTC).
        /// </remarks>
        public static HistoricalStatusUpdateStatistics GetLinkedInHistoricalStatusUpdateStatistics(LinkedInAuthorization authorization, string companyId, DateTime? sinceWhen = null)
        {
            // Compute UNIX timestamp for yesterday. The company stats are always collected for whole days. Today's stats must be collected when the day ends.
            DateTime utcYesterday = UtcToday().AddDays(-1);
            DateTime startTime = (sinceWhen == null || sinceWhen > utcYesterday) ? utcYesterday : sinceWhen.Value;
        ...
        /// <exception cref="LinkedInApiThrottleLimitException">Thrown when protocol error occurs because the limit of API requests was overdrown.</exception>
        /// <exception cref="LinkedInApiException">Thrown when protocol error occurs.</exception>
        public static HistoricalStatusUpdateStatistics GetLinkedInHistoricalStatusUpdateStatistics(LinkedInAuthorization authorization, string companyId, string updateKey, DateTime postPublishedDateTime)
        {
