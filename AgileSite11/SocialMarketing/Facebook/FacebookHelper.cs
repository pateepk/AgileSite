...
namespace CMS.SocialMarketing
{
    ...
    public class FacebookHelper : AbstractHelper<FacebookHelper>
    {
        ...
        public static string PublishPostOnFacebookPage(string pageId, string pageAccessToken, string postText)
        {
            return HelperObject.PublishPostOnFacebookPageInternal(pageId, pageAccessToken, postText, String.Empty);
        }
        ...
        public static void DeletePostOnFacebookPage(string postId, string pageAccessToken)
        {
            HelperObject.DeletePostOnFacebookPageInternal(postId, pageAccessToken, String.Empty);
        }
        ...
        /// </summary>
        /// <param name="pageUrl">The URL to get the Facebook page identifier from.</param>
        /// <param name="appInfo">Facebook application associated with given <paramref name="pageUrl"/>.</param>
        /// <param name="pageId">The Facebook page identifier, if the specified URL is a valid Facebook page URL; otherwise, null.</param>
        /// <returns>True, if the specified URL is a valid Facebook page URL and <paramref name="appInfo"/> is correctly set; otherwise, false.</returns>
        [Obsolete("Method will be removed in the next version. The Facebook page ID has to be set manually.")]
        public static bool TryGetFacebookPageId(string pageUrl, FacebookApplicationInfo appInfo, out string pageId)
        {
            pageId = null;
            if (!ValidationHelper.IsURL(pageUrl) || appInfo == null)
            {
                return false;
            }

            pageUrl = URLHelper.RemoveQuery(pageUrl);
            pageUrl = URLHelper.AddParameterToUrl(pageUrl, "metadata", "1");

            var token = Service.Resolve<IFacebookAccessTokenService>().GetAccessToken(appInfo.FacebookApplicationConsumerKey, appInfo.FacebookApplicationConsumerSecret);
            var key = Encoding.UTF8.GetBytes(appInfo.FacebookApplicationConsumerSecret);

            FacebookClient client = new FacebookClient(token);
            var page = client.Get(pageUrl, new { appsecret_proof = SecurityHelper.GetHMACSHA2Hash(token, key) }) as IDictionary<string, object>;

            if ((page != null) && page.ContainsKey("metadata"))
            {
                IDictionary<string, object> metadata = page["metadata"] as IDictionary<string, object>;
                if ((metadata != null) && metadata.ContainsKey("type"))
                {
                    string type = (string)metadata["type"] ?? String.Empty;
                    if (type.ToLowerInvariant() == "page")
                    {
        ...
        {
            return HelperObject.GetPageAccessTokenInternal(accessToken, pageId, String.Empty);
        }
