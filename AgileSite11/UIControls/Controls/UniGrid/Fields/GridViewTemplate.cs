...
namespace CMS.UIControls
{
    ...
    public class GridViewTemplate : WebControl, ITemplate
    {
        ...
        public HashSettings HashSettings
        {
            get
            {
                return mHashSettings ?? (mHashSettings = new HashSettings());
            }
        }
        ...
        private string ObjectType
        {
            get
            {
                return mObjectType ?? (mObjectType = GetObjectType());
            }
        }
        ...
        /// </summary>
        private Dictionary<string, bool> ActionsAuthorization
        {
            get
            {
                return mActionsAuthorization ?? (mActionsAuthorization = new Dictionary<string, bool>(StringComparer.OrdinalIgnoreCase));
            }
        }
        ...
        private void BindData(object sender, EventArgs e)
        {
            ...
            var iconButton = sender as CMSGridActionButton;
            var imageButton = sender as ImageButton;
            IButtonControl buttonControl = (IButtonControl)iconButton ?? imageButton;
            ...
            if (buttonControl != null)
            {
                ...
                {
                    onclick = String.Format("if ({0}) {{ {1} }} return false;", confirmation, onclick);
            ...
            else
            {
                ...
                var menu = sender as ContextMenuContainer;
                if (menu != null)
                {
            ...
            if (container.RowType == DataControlRowType.DataRow)
            {
                if (container.DataItem != null)
                {
                    ...
                    if (uniGrid != null)
                    {
                        ...
                        {
                            onclick += String.Format("{0}('{1}', this.checked);", uniGrid.SelectionJavascript, ScriptHelper.GetString(argument, false));
                        }
        ...
        private void CreateButtonAction(Action action, Control container)
        {
            if (action == null)
            {
            ...
            if (action.ContextMenu != null)
            {
                ...
                container.Controls.Add(CreateActionContextMenu(action, button));
            }
            else
            {
                // Simple action
        ...
        private void CreateImageAction(Action action, Control container)
        {
            ...
            image.AddCssClass(String.Format("js-{0}", safeName));

            image.ID = "a" + safeName;
            image.EnableViewState = false;

            SetDataBindAttributes(image, action);
            image.DataBinding += BindData;

            // Context menu
            if (action.ContextMenu != null)
            {
                // Create the context menu container
                container.Controls.Add(CreateActionContextMenu(action, image));
            }
            else
            {
                // Simple action
        ...
        /// <param name="action">Unigrid action</param>
        private void SetDataBindAttributes(WebControl control, Action action)
        {
            // Confirmation
            if (action.Confirmation != null)
            {
                ...
                string confirmScript = String.Format("function UG_Confirm_{0}() {{ return confirm({1}); }}", actionIdentifier, ScriptHelper.GetString(confirmString));
                ...
                control.Attributes.Add("confirmation", String.Format("UG_Confirm_{0}()", actionIdentifier));
            }
        ...
        /// <param name="control">Action control (CMSAccessibleButton or CMSImageButton)</param>
        /// <returns></returns>
        private ContextMenuContainer CreateActionContextMenu(Action action, WebControl control)
        {
        ...
        public void InstantiateIn(Control container)
        {
            switch (templateType)
            {
                case ListItemType.Header:
                    ...
                    if (mHeaderControl != null)
                    {
                        ...
                        if (isCheckbox)
                        {
                            ...
                            lblHeader.Display = false;
                            lblHeader.AssociatedControlID = mHeaderControl.ID;
                            lblHeader.ResourceString = "general.selectall";
                    ...
                    {
                        // Create context menu
                        if ((mActions != null) && !string.IsNullOrEmpty(mActions.ContextMenu))
                        {
                ...
                case ListItemType.Item:
                    ...
                    {
                        // Process all actions
                        foreach (AbstractAction item in mActions.Actions)
                        {
                            ...
                            if (action != null)
                            {
                                if (action.HideIfNotAuthorized)
                                {
                                    ...
                                    if (!ActionsAuthorization.TryGetValue(action.Name, out actionAuthorized))
                                    {
                                ...
                                // Action
                                if (!String.IsNullOrEmpty(action.FontIconClass))
                                {
                                    CreateButtonAction(action, container);
                                }
                                else
                                {
                                    CreateImageAction(action, container);
                                }
                            }
                        }
                    }
