...
namespace CMS.UIControls
{
    ...
    public abstract class UniGrid : CMSUserControl, IUniGrid, IExtensibleUniGrid, ICallbackEventHandler, IPostBackEventHandler, IUniPageable
    {
        ...
        public List<string> SelectedItems
        {
            get
            {
                return mSelectedItems ?? (mSelectedItems = GetHiddenValues(SelectionHiddenField, SelectionHashHiddenField));
            }
            set
            {
                SetHiddenValues(value, SelectionHiddenField, SelectionHashHiddenField);
        ...
        public List<string> DeselectedItems
        {
            get
            {
                return ValidateHiddenValues(SelectionHiddenField, SelectionHashHiddenField) ? OriginallySelected.Except(SelectedItems).ToList() : new List<string>();
            }
        }
        ...
        public List<string> NewlySelectedItems
        {
            get
            {
                return ValidateHiddenValues(SelectionHiddenField, SelectionHashHiddenField) ? SelectedItems.Except(OriginallySelected).ToList() : new List<string>();
            }
        }
        ...
        public void RaiseAction(string cmdName, string cmdValue)
        {
            if (OnAction != null)
            {
                if ((ActionsHidden != null) && (ActionsHashHidden != null))
                {
                    ...
                    if (actions.Contains(String.Format("|{0}|", cmdValue)) && ValidationHelper.ValidateHash(actions, ActionsHashHidden.Value))
                    {
                        onActionUsed = true;
        ...
        protected void LoadActionsDefinition(UniGridActions actions)
        {
            ...
            if (actions.Actions.Count > 0 && !(actions.Actions.FirstOrDefault() is EmptyAction) && ShowObjectMenu && UniGridFunctions.ShowUniGridObjectContextMenu(emptyObject, SiteContext.CurrentSiteName))
            {
                ...
                var menus = from action in actions.Actions.OfType<Action>()
                            where (action.Name.Equals("#objectmenu", StringComparison.OrdinalIgnoreCase)) || (!String.IsNullOrEmpty(action.ContextMenu))
                            select action;

                // Add object menu of necessary
                if (!menus.Any() && !IsLiveSite)
                {
        ...
        public string GetCallbackResult()
        {
            if (!string.IsNullOrEmpty(callbackArguments))
            {
                ...
                if (fractions.Length >= 2)
                {
                    ...
                    // Validate the hashes
                    bool isValid = ValidationHelper.ValidateHashes(items.Values, items.Hashes, new HashSettings { Redirect = false });
                    if (isValid)
                    {
        ...
        protected void SetActionsHash()
        {
            ...
            {
                SetHiddenValues(ActionsID, ActionsHidden, ActionsHashHidden);
            }
        }
        ...
        private void ProcessDestroyAction(BaseInfo infoObj, string cmdValue)
        {
            try
            {
                ...
                using (var context = new CMSActionContext())
                {
                    ...
                    var action = GridActions.GetAction("#delete");
                    if (action != null)
                    {
                    ...
                    {
                        action = GridActions.GetAction("delete");
                        if (action != null)
                        {
        ...
        private void CheckActionAndRedirect(string actionName)
        {
            ...
            // Get the action
            var action = GridActions.GetAction(actionName);
            if (action != null)
            {
        ...
        {
            return ValidationHelper.ValidateHash(field.Value, hashField.Value, new HashSettings { Redirect = false });
        }
        ...
        protected void RegisterCmdScripts()
        {
            ...
            var allowRowMove = (GridActions != null) && GridActions.Actions.OfType<Action>().Any(action => action.Name.Equals("#move", StringComparison.OrdinalIgnoreCase));
