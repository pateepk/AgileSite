...
namespace CMS.UIControls
{
    ...
    public abstract class SelectionDialog : CMSUserControl, IObjectTypeDriven, ICallbackEventHandler
    {
                ...
                case "itemname":
                    {
                        ...
                        return "<div " + (!disabled ? "class=\"SelectableItem\" " : null) + (onclick != null ? "onclick=\"" + onclick + "\"" : null) + ">" + HTMLHelper.HTMLEncode(TextHelper.LimitLength(itemName, 100)) + "</div>";
                    }
            }
        ...
        private void UpdateHiddenFields(bool isSelectAllAction)
        {
            ...
            if (!DataHelper.DataSourceIsEmpty(ds))
            {
                var values = ItemsField.Value.Split(new[] { ValueSeparator }, StringSplitOptions.RemoveEmptyEntries).ToHashSet();
...
", ItemsField.ClientID, scriptValuesSeparator, regexEscapedValuesSeparator);
...
";
        ...
        private string GetItemCheckBoxHtml(string itemId)
        {
            ...
            string checkBox = $"<span class=\"checkbox\"><input id=\"chk{itemId}\" type=\"checkbox\" onclick=\"ProcessItem(this, false); UpdateCheckboxAllElement();\" class=\"chckbox\" ";
            ...
            checkBox += $"/><label for=\"chk{itemId}\">&nbsp;</label></span>";
        ...
        private string GetItemLinkClickScript(string itemId, string itemName)
        {
            ...
            switch (selectionMode)
            {
                ...
                case SelectionModeEnum.MultipleTextBox:
                case SelectionModeEnum.MultipleButton:
                    clickScript = string.Format("ProcessItem(document.getElementById('chk{0}'), true); UpdateCheckboxAllElement(); return false;", ScriptHelper.GetString(itemId).Trim('\''));
                    break;
