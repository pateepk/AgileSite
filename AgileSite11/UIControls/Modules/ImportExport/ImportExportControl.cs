            ...
            using (var context = new CMSActionContext())
            {
                ...
                foreach (var objectType in objectsToImport)
                {
                    ...
                    if (count > 0)
                    {
                        ...
                        {
                            return String.Format(LocalizationHelper.GetString("import.objecttypenotregistered"), objectType);
                        }
                        ...
                        using (new CMSActionContext { LogLicenseWarnings = false })
                        {
                            ...
                            if (!importAllowed)
                            {
                                return String.Format(LocalizationHelper.GetString("import.checklicensefailed"), readOnlyObject.TypeInfo.GetNiceObjectTypeName()) + " " + exceptionMessage;
                            }
                        }
                    }
                }
