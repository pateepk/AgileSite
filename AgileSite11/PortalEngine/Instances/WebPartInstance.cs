...
namespace CMS.PortalEngine
{
    ...
    public class WebPartInstance : ISimpleDataContainer
    {
        ...
        /// </summary>
        public WebPartInstance CurrentVariantInstance
        {
            get;
            set;
        }
        ...
        private static Regex PropertyRegEx
        {
            get
            {
                return mPropertyRegEx ?? (mPropertyRegEx = RegexHelper.GetRegex("\\{%(\\w+)%}"));
            }
        }
        ...
        /// </summary>
        public WebPartZoneInstance ParentZone
        {
            get;
            set;
        }
        ...
        public bool HasVariants
        {
            get
            {
                return ((PartInstanceVariants != null) && (PartInstanceVariants.Count > 0));
            }
        }
        ...
        /// </summary>
        public virtual List<WebPartInstance> PartInstanceVariants
        {
            get;
            set;
        }
        ...
        /// </summary>
        public StringSafeDictionary<WebPartInstance> NestedWebParts
        {
            get;
            set;
        }
        ...
        public WebPartInstance()
        {
            ...
            NestedWebParts = new StringSafeDictionary<WebPartInstance>();
        }
        ...
        public WebPartInstance Clone(bool copyMacroTable = false)
        {
            ...
            {
                // Clone nested web parts
                foreach (string key in NestedWebParts.Keys)
                {
                    wpi.NestedWebParts[key] = NestedWebParts[key].Clone(copyMacroTable);
                }
            }
        ...
        private void LoadFromXmlNode(XmlNode partNode)
        {
            ...
            // Get the control ID
            if ((attrs == null) || (attrs["controlid"] == null) || (attrs["controlid"].Value.Trim() == ""))
            {
            ...
            XmlNode nestedWebParts = partNode.SelectSingleNode("nestedwebparts");
            if (nestedWebParts != null)
            {
                // Load the properties
                foreach (XmlNode nested in nestedWebParts.ChildNodes)
                {
                    if (nested.NodeType == XmlNodeType.Element)
                    {
                        NestedWebParts[nested.Name] = new WebPartInstance(nested);
                    }
                }
            }
        }
        ...
        public XmlElement GetXmlNode(XmlDocument doc = null, string nodeName = "webpart")
        {
            ...
            // Nested web parts
            if (!NestedWebParts.IsEmpty)
            {
                var nestedNode = doc.CreateElement("nestedwebparts");
                partNode.AppendChild(nestedNode);

                foreach (var key in NestedWebParts.TypedKeys)
                {
                    var dsNode = NestedWebParts[key].GetXmlNode(doc, key);
                    nestedNode.AppendChild(dsNode);
                }
            }
