...
namespace CMS.PortalEngine
{
    ...
    public class DeviceProfileLayoutInfoProvider : AbstractInfoProvider<DeviceProfileLayoutInfo, DeviceProfileLayoutInfoProvider>
    {
        ...
        [Obsolete("Use method GetDeviceProfileLayouts() instead")]
        protected virtual BindingDataSet GetDeviceProfileLayoutsInternal(string where, string orderBy, int topN, string columns)
        {
            return GetDeviceProfileLayouts().Where(where).OrderBy(orderBy).TopN(topN).Columns(columns).BinaryData(true).TypedResult;
        }
