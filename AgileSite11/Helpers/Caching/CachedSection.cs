...
namespace CMS.Helpers
{
    ...
    public class CachedSection<TData> : CachedSection
    {
        ...
        public override void Dispose()
        {
            if (!mRemoved && Cached && LoadData)
            {
                ...
                if (mDataWasSet)
                {
                    var absoluteExpiration = Settings.UseSlidingExpiration ? Cache.NoAbsoluteExpiration : dateTimeNowService.GetDateTimeNow().AddMinutes(CacheMinutes);
                    var slidingExpiration = Settings.UseSlidingExpiration ? TimeSpan.FromMinutes(CacheMinutes) : Cache.NoSlidingExpiration;
    ...
    public abstract class CachedSection : Trackable<CachedSection>
    {
        ...
        /// Cache item priority.
        /// </summary>
        internal CacheItemPriority CacheItemPriority
        {
            get
            {
        ...
        {
            if ((Exception == null) && SystemContext.DevelopmentMode)
            {
