...
namespace CMS.Helpers
{
    ...
    public static class HTMLHelper
    {
        ...
        public static string UnResolveUrls(string mHtml, string applicationPath)
        {
            ...
            const string QUOTES = "[\"']?";
            var doubleSlashFix = applicationPath.Equals("/", StringComparison.Ordinal) ? "(?!/)" : "";
            var uri = "(" + applicationPath.TrimEnd('/') + "/)" + doubleSlashFix + "([^\"'>\\s]*)";
            var re = RegexHelper.GetRegex(
                string.Format(
                    @"((\s(background|src|href)={1}){0}({1}[\s>]))|((url\({1}){0}({1}\)))",
                    uri,
                    QUOTES),
                RegexHelper.DefaultOptions | RegexOptions.IgnoreCase | RegexOptions.Compiled);

            // Unresolve the URLs
            mHtml = re.Replace(mHtml, UnResolveUri);
        ...
        /// <summary>
        /// Unresolves the given URL.
        /// </summary>
        /// <param name="m">URL match</param>
        private static string UnResolveUri(Match m)
        {
            // Attribute is assembled of following groups:
            // (2|8) = name with equals sign and eventually a quotation mark
            // (5|10) = part of URI following the application path (see UnResolveUrls)
            // (6|11) = an eventual quotation mark and terminating symbol (6: white space or '>', 11: ')')
            string attributeWithUri;
            if (m.Groups[1].Success)
            {
                // 'background=' or 'src=' or 'href='
                attributeWithUri = m.Groups[2] + "~/" + m.Groups[5].Value.TrimStart('/') + m.Groups[6];
            }
            else if (m.Groups[7].Success)
            {
                // 'url('
                attributeWithUri = m.Groups[8] + "~/" + m.Groups[10].Value.TrimStart('/') + m.Groups[11];
            }
            else
            {
        ...
        public static string EnsureURLPrefixes(string mHtml, string applicationPath, string urlPrefix, URLHelper.PathModifierHandler urlModifier = null)
        {
            if (mHtml != null)
            {
                ...
                if (re == null)
                {
                    ...
                    re = RegexHelper.GetRegex("(\\sbackground=[\"']|\\ssrc=[\"']|\\shref=[\"']|url\\()(https://[^/\"']*|http://[^/\"']*|)(" + applicationPath.TrimEnd('/') + "/[^\"']*)([\"'])", RegexHelper.DefaultOptions | RegexOptions.IgnoreCase);
            ...
            if (HttpContext.Current != null)
            {
                ...
                if (standardForm)
                {
                    ...
                    if (formIndex >= 0)
                    {
                        ...
                        result = RegExFixFormAction.Replace(result, "$1$2" + HttpContext.Current.Server.UrlPathEncode(requestUrl) + "$3", 1, formIndex);
                    }
                }
                if (ajaxForm)
                {
                    ...
                    if (formIndex >= 0)
                    {
                        ...
                        result = RegExFixAJAXFormAction.Replace(result, "|" + requestUrl.Length + "|$2" + HttpContext.Current.Server.UrlPathEncode(requestUrl) + "$3", 1, formIndex);
                    }
                }
                return result;
            }
