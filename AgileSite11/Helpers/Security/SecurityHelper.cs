...
namespace CMS.Helpers
{
    ...
    public static class SecurityHelper
    {
        ...
        public static bool TryParseBasicAuthorizationHeader(string authorizationHeader, out string username, out string password)
        {
            ...
            {
                string[] credentialParts = Encoding.ASCII.GetString(Convert.FromBase64String(authorizationParts[1])).Split(':');
        ...
        {
            if (SystemContext.IsIntegratedMode && !IsXFrameOptionsExcluded(RequestContext.CurrentRelativePath, RequestContext.URL))
            {
