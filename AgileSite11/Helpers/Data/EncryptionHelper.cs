...
namespace CMS.Helpers
{
    ...
    public static class EncryptionHelper
    {
        ...
        public static bool VerifyRSA(string inputText, string publicKey)
        {
            try
            {
                ...
                using (RSACryptoServiceProvider myRsaProvider = new RSACryptoServiceProvider())
                {
                    ...
                    // Get input key
                    string inputKey = inputText.Substring(0, inputText.IndexOfCSafe("\r\n", inputText.IndexOfCSafe("SERVERS:")));

                    // Check cutom section
                    if (inputText.Contains("CUSTOM:"))
                    {
                        inputKey = inputText.Substring(0, inputText.IndexOfCSafe("\r\n", inputText.IndexOfCSafe("CUSTOM:")));
                    }

                    // Add end of line in default
                    if (!inputKey.EndsWithCSafe("\r\n"))
                    {
                        inputKey += "\r\n";
                    }
                    ...
                    // Get input signature
                    string inputSignature = inputText.Substring(inputText.IndexOfCSafe("\r\n", inputText.IndexOfCSafe("SERVERS:")) + 2);

                    //Check custom section
                    if (inputText.Contains("CUSTOM:"))
                    {
                        inputSignature = inputText.Substring(inputText.IndexOfCSafe("\r\n", inputText.IndexOfCSafe("CUSTOM:")) + 2);
                    }
