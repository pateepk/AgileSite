...
namespace CMS.Helpers
{
    ...
    public class URLHelper
    {
        ...
        public static string RemovePort(string domain)
        {
            ...
            if (!domain.EndsWithCSafe("]"))
            {
                int portIndex = domain.LastIndexOfCSafe(":");
                if (portIndex >= 0)
                {
        ...
        /// </summary>
        /// <param name="domainName">String to correct</param>
        /// <returns>Corrected string</returns>
        [Obsolete("Method will be removed use custom logic instead.")]
        public static string CorrectDomainName(string domainName)
        {
            string result = domainName.Trim();
            // Remove the protocol if present
            int protocolIndex = result.IndexOfCSafe("://");
            if (protocolIndex >= 0)
            {
            ...
            while (result.StartsWith("www.", StringComparison.OrdinalIgnoreCase))
            {
        ...
        [Obsolete("Method will be removed. Use custom logic instead.")]
        public static string RemovePathInfo(string url, string pathInfo)
        {
            // No URL, return null
            if (String.IsNullOrEmpty(url))
            {
        ...
        public static string ResolveUrl(string url, bool ensurePrefix = false)
        {
            ...
            // Use URL without query string because VirtualPathUtility doesn't exclude it from modifying
            urlWithoutQuery = VirtualPathUtility.ToAbsolute(urlWithoutQuery);

            return AppendQuery(urlWithoutQuery, query);
        }


        private static bool IsUrlRooted(string url)
        {
            return  url[0] == '/' || url[0] == '\\';
        }
        ...
        public static string ResolveClientUrl(string baseUrl, string relativeUrl)
        {
            ...
            try
            {
                ...
                return VirtualPathUtility.Combine(builder.Uri.AbsolutePath, relativeUrl);
            }
            catch
            {
                return relativeUrl;
        ...
        [Obsolete("Method will be removed. Use EnsureURLPrefix(string, string, string, PathModifierHandler) instead.")]
        public static string EnsureURLPrefix(string url, string applicationPath, string prefix)
        {
            return EnsureURLPrefix(url, applicationPath, prefix, null);
        }
        ...
        /// <param name="url">URL to modify</param>
        [Obsolete("Method will be removed. Use custom logic instead.")]
        public static string AddPrefixToUrl(string url)
        {
            if (url != null)
            {
                string prefix = RequestContext.CurrentURLPathPrefix;
                if (!String.IsNullOrEmpty(prefix))
                {
                    prefix = "/" + prefix;

                    EnsureURLPrefix(url, SystemContext.ApplicationPath, prefix);
                }
            }
        ...
        [Obsolete("Method will be removed. Use custom logic instead.")]
        public static string RemoveEmptyQueryParameters(string query)
        {
            string result = null;
            ...
            foreach (string param in parameters)
            {
                ...
                {
                    result = AddUrlParameter(result, param, HttpUtility.UrlEncode(parameters[param]));
                }
            }
            return result;
        }
...
#pragma warning restore BH1008 // 'Response.Redirect' should not be used.
        ...
        /// </summary>
        [Obsolete("Method will be removed. Use RequestHelper.IsPostBack() instead.")]
        ...
        public static string EnsureHashToQueryParameters(string url)
        {
            ...
            if (!String.IsNullOrEmpty(url) && url.Contains("?"))
            {
                ...
                url = url.Replace("{hash}", "hash=" + ValidationHelper.GetHashString(hashString));
            }
        ...
        [Obsolete("Method will be removed. Use HttpUtility.UrlEncode instead.")]
        ...
        [Obsolete("Method will be removed. Use HttpUtility.UrlDecode instead.")]
        ...
        /// </summary>
        /// <param name="address">IPv6 address to translate to IPv4</param>
        [Obsolete("Method will be removed. Use custom logic instead.")]
        public static string GetIPv4Address(string address)
        {
            // Check if address contains at least 2 ':' colons making it IPv6 address
            if (address.Split(':').Length > 2)
            {
                // Check if we have equivalent IPv4 address available 
