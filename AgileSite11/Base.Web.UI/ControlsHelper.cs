...
namespace CMS.Base.Web.UI
{
    ...
    public static class ControlsHelper
    {
        ...
        /// </summary>
        [Obsolete("Use custom code instead. Will be removed in next major release.")]
        public static List<ListItem> GetDropDownListSource(IEnumerable<string> items, string resourcePrefix = null)
        {
            var result = new List<ListItem>();

            bool localize = !String.IsNullOrEmpty(resourcePrefix);

            foreach (var item in items)
            {
                var text = localize ? ResHelper.GetString(String.Format("{0}.{1}", resourcePrefix, item)) : item;

                result.Add(new ListItem(text, item));
            }

            // Sort the items
            result.Sort((a, b) => String.Compare(a.Text, b.Text, StringComparison.CurrentCulture));
        ...
        [Obsolete("Use CMS.PortalEngine.Web.UI.PortalHelper.GetWebPartControl(Control) method instead. Will be removed in next major release.")]
        ...
        /// <param name="dr">DataRowView containing data that will be used for replacements</param>    
        ...
        [Obsolete("Use ResolveMacros(string, DataRow, bool) method instead. Will be removed in next major release.")]
        public static string ResolveMacros(string sourceText, DataRowView dr, bool encode)
        {
            return ResolveMacros(sourceText, dr.Row, encode);
        }
        ...
        [Obsolete("Method will be removed in next major release.")]
        public static void AddControlsToHashtable(ArrayList controls, Hashtable table)
        {
            if (controls != null)
            {
        ...
        public static string GetUniqueID(Control parent, string baseId)
        {
            return GetUniqueID(parent, baseId, null);
        }
        ...
        public static string GetUniqueID(Control parent, string baseId, Control control)
        {
