...
function CloseDialog(refreshPage) {
	...
	return false; 
        ...
        {
            string script = String.Format("setTimeout({0}, 0)", GetString(GetAlertScript(message, false), true));
...
<script type=""text/javascript"">
	{0}{1}{2}
        ...
        {
            return GetString(text, encapsulate, true);
        }
        ...
        public static string GetString(string text, bool encapsulate, bool encodeNewLine)
        {
            if (text != null)
            {
...
",
                    GetString(ResHelper.LocalizeString(newName), true)
                );
