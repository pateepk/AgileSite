                    ...
                    if (handler.CanContinue() && !handler.EventArguments.EventHandled)
                    {
                        ...
                        if (pageInfo != null)
                        {
                            ...
                            if (node != null)
                            {
                                ...
                                string mediaType = ValidationHelper.GetString(node.GetValue("documenttype"), "");
                                mediaSource.Extension = mediaType;

                                if (node.IsFile())
                                {
                                    Guid attachmentsGuid = ValidationHelper.GetGuid(node.GetValue("FileAttachment"), Guid.Empty);

                                    if (attachmentsGuid != Guid.Empty)
                                    {
                                        // Get the attachment
                                        var ai = DocumentHelper.GetAttachment(node, attachmentsGuid, false);
                                        if (ai != null)
                                        {
                                            mediaSource.AttachmentGuid = ai.AttachmentGUID;
                                            mediaSource.MediaWidth = ai.AttachmentImageWidth;
