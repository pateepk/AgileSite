...
namespace ThoughtWorks.QRCode.Codec
{
    public class QRCodeEncoder
    {
        ...
        {
            // Get the file physical path
            fileName = "~/App_Data/CMSModules/QRCode/[Resources.zip]/" + fileName + ".dat";
            fileName = CMS.IO.StorageHelper.GetFullFilePhysicalPath(fileName, null);

            var str = CMS.IO.File.OpenRead(fileName);
            return str;
        }
        /* CMS end */
        ...
        public virtual Bitmap Encode(String content, Encoding encoding)
        {
            ...
            {
                var textBytes = encoding.GetBytes(content);
                var preamble = encoding.GetPreamble();

                bytes = new byte[textBytes.Length + preamble.Length];
