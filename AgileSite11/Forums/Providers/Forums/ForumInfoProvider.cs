...
namespace CMS.Forums
{
    ...
    public class ForumInfoProvider : AbstractInfoProvider<ForumInfo, ForumInfoProvider>
    {
        ...
        [Obsolete("Use CMS.DataEngine.ObjectQuery<ForumInfo> instead")]
        public static InfoDataSet<ForumInfo> GetForumsByGroupID(int groupId, bool onlyOpen)
        {
            string where = "ForumGroupID = " + groupId;

            if (onlyOpen)
            {
                where = SqlHelper.AddWhereCondition(where, "ForumOpen = 1");
            }

            return GetForums(where, null, -1, null);
        }
        ...
        /// <param name="groupId">Forum group ID</param>
        [Obsolete("Use CMS.DataEngine.ObjectQuery<ForumInfo> instead")]
        public static InfoDataSet<ForumInfo> GetForumsByGroupID(int groupId)
        {
            return GetForumsByGroupID(groupId, false);
        }
        ...
        public static string GetRoleNamesForForum(int forumId)
        {
            return ProviderObject.GetRoleNamesForForumInternal(forumId);
        }
        ...
        /// </summary>
        [Obsolete("Use method GetForums() instead")]
        public static InfoDataSet<ForumInfo> GetAllForums(string where, string orderBy)
        {
            return GetForums(where, orderBy, -1, null);
        }
        ...
        [Obsolete("Use method GetForums() instead")]
        public static InfoDataSet<ForumInfo> GetForums(string where, string orderBy, int topN, string columns)
        {
            return ProviderObject.GetForumsInternal(where, orderBy, topN, columns);
        }
...
#pragma warning restore BH2000 // Method 'WhereLike()' or 'WhereNotLike()' should not be used used.
        ...
        /// <param name="columns">Columns</param>
        [Obsolete("Use method GetForums() instead")]
        protected InfoDataSet<ForumInfo> GetForumsInternal(string where, string orderBy, int topN, string columns)
        {
            return GetForums().Where(where).OrderBy(orderBy).TopN(topN).Columns(columns).BinaryData(true).TypedResult;
        }
        ...
        private bool IsUserAuthorizedPerForum(int forumId, string permissionName, UserInfo userInfo, int siteId)
        {
            ...
            Hashtable userForumsPerm = (Hashtable)AbstractStockHelper<RequestStockHelper>.GetItem("ForumPermission_" + userInfo.UserID);
            if (userForumsPerm?[forumId] == null)
            {
                ...
                {
                    AbstractStockHelper<RequestStockHelper>.Add("ForumPermission_" + userInfo.UserID, userForumsPerm);
                }
                else
                {
                    return false;
