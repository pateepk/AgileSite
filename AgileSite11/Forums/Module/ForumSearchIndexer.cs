...
namespace CMS.Forums
{
    ...
    public class ForumSearchIndexer : SearchIndexer
    {
                ...
                if ((allowedForums.Count > 0) || (siteIds.Count > 0))
                {
                    ...
                    try
                    {
                        if (iw != null)
                        {
                            ...
                            using (var where = new WhereCondition() { DisposeUsedObjects = true })
                            {
        ...
        {
            return AvatarInfoProvider.GetUserAvatarImageUrl(GetSearchValue(id, "AvatarID"), GetSearchValue(id, "PostUserID"), ValidationHelper.GetString(GetSearchValue(id, "PostUserMail"), String.Empty), 0, 0, 0);
        }
