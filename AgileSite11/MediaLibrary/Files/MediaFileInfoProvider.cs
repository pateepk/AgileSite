...
namespace CMS.MediaLibrary
{
    ...
    public class MediaFileInfoProvider : AbstractInfoProvider<MediaFileInfo, MediaFileInfoProvider>
    {
            ...
            if (mediaFile != null)
            {
                ...
                if (WebFarmHelper.WebFarmEnabled)
                {
                    ...
                    if (mli != null)
                    {
                        ...
                        WebFarmHelper.CreateIOTask(MediaTaskType.UpdateMediaFile, path, fileBinary, "mediafileupload", SiteContext.CurrentSiteName, mli.LibraryFolder, Path.GetDirectoryName(mediaFile.FilePath), mediaFile.FileName, mediaFile.FileExtension, mediaFile.FileGUID.ToString());
                    }
                }
            }
        }
        ...
        protected virtual void DeleteMediaFileInternal(int siteID, int libraryID, string filePath, bool onlyFile, bool synchronization)
        {
            ...
            if (!synchronization)
            {
                WebFarmHelper.CreateTask(MediaTaskType.DeleteMediaFile, "mediafiledelete", siteID.ToString(), libraryID.ToString(), filePath, onlyFile.ToString());
            }
        }
        ...
        protected virtual void DeleteMediaFilePreviewInternal(string siteName, int libraryID, string filePath, bool synchronization)
        {
            ...
            if (!synchronization)
            {
                WebFarmHelper.CreateTask(MediaTaskType.DeleteMediaFilePreview, "mediafilepreviewdelete", siteName, libraryID.ToString(), filePath);
            }
        }
        ...
        protected virtual string SaveFileToDiskInternal(string siteName, string libraryFolder, string librarySubFolderPath, string fileName, string fileExtension, Guid fileGuid, BinaryData fileData, bool synchronization, bool ensureUniqueFileName, bool skipChecks, string filePath, string fileSubFolderPath)
        {
            ...
            if (fileData != null)
            {
                ...
                {
                    WebFarmHelper.CreateIOTask(MediaTaskType.UpdateMediaFile, filePath, fileData, "mediafileupload", siteName, libraryFolder, librarySubFolderPath, fileName, fileExtension, fileGuid.ToString());
                }
        ...
        protected virtual void MoveMediaFileInternal(string siteName, int originalLibraryID, int newLibraryID, string origPath, string newPath, bool synchronization)
        {
            ...
            if (!synchronization)
            {
                WebFarmHelper.CreateTask(MediaTaskType.MoveMediaFile, "movemediafile", siteName, originalLibraryID.ToString(), newLibraryID.ToString(), origPath, newPath);
            }
        }
        ...
        protected virtual void CopyMediaFileInternal(string siteName, int libraryID, string origPath, string newPath, bool synchronization, int userId)
        {
            ...
            if (!synchronization)
            {
                WebFarmHelper.CreateTask(MediaTaskType.CopyMediaFile, "copymediafile", siteName, libraryID.ToString(), origPath, newPath);
            }
        }
        ...
        protected virtual string GetMediaFileAbsoluteUrlInternal(string siteName, string libraryFolder, string filePath)
        {
            ...
            {
                MediaFileInfo mfi = GetMediaFileInfo(siteName, filePath, libraryFolder);
                if (mfi != null)
                {
                    return GetMediaFileAbsoluteUrl(siteName, mfi.FileGUID, mfi.FileName);
                }
            }
