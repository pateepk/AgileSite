...
[assembly: RegisterHttpHandler("CMSPages/GetMediaFile.aspx", typeof(GetMediaHandler), Order=1)]
[assembly: RegisterHttpHandler("getmedia/{fileguid:guid}/{filename}", typeof(GetMediaHandler), Order=2)]
...
namespace CMS.MediaLibrary
{
    ...
    /// </summary>
    internal class GetMediaHandler : AdvancedGetFileHandler
    {
        ...
        /// <summary>
        /// Processes the media library file request.
        /// </summary>
        ...
        protected override void ProcessRequestInternal(HttpContextBase context)
        {
            ...
            using (var cs = new CachedSection<CMSOutputMediaFile>(ref outputFile, cacheMinutes, true, null, "getmediafile", CurrentSiteName, GetBaseCacheKey(), QueryHelper.GetParameterString()))
            {
                if (cs.LoadData)
                {
                    // Process the file
                    ...
                    if (cs.Cached)
                    {
                        ...
                        if ((cd == null) && (cacheMinutes > 0))
                        {
                            ...
                            {
                                "mediafile|" + fileGuid.ToString().ToLowerCSafe(),
                                "mediafilepreview|" + fileGuid.ToString().ToLowerCSafe()
                            });
                        }
        ...
        protected void SendFile(CMSOutputMediaFile file)
        {
            ...
            if (file != null)
            {
                ...
                if (checkPermissions)
                {
                    ...
                    if (!MediaLibraryInfoProvider.IsUserAuthorizedPerLibrary(mli, "LibraryAccess", MembershipContext.AuthenticatedUser))
                    {
                        // Using PageSecurityHelper.AccessDeniedPageURL(CurrentSiteName) would require using reference on DocumentEngine
                        string url = SettingsKeyInfoProvider.GetValue(CurrentSiteName + ".CMSAccessDeniedPageURL");
                        if (url == "")
                        {
