...
namespace CMS.EventLog
{
    ...
    public class EventLogHelper : AbstractHelper<EventLogHelper>
    {
        ...
        {
            HelperObject.LogLastApplicationErrorInternal();
        }
        ...
        protected virtual void LogInsertInternal(GeneralizedInfo infoObj)
        {
            ...
            LogAction(infoObj, "CREATEOBJ", ResHelper.GetAPIString("TaskTitle.CreateObject", "Create {0}"));
        }
        ...
        protected virtual void LogUpdateInternal(GeneralizedInfo infoObj)
        {
            ...
            LogAction(infoObj, "UPDATEOBJ", ResHelper.GetAPIString("TaskTitle.UpdateObject", "Update {0}"), true);
        }
        ...
        protected virtual void LogDeleteInternal(GeneralizedInfo infoObj)
        {
            ...
            LogAction(infoObj, "DELETEOBJ", ResHelper.GetAPIString("TaskTitle.DeleteObject", "Delete {0}"));
        }
        ...
        /// Logs the last application error.
        /// </summary>
        protected virtual void LogLastApplicationErrorInternal()
        {
            if ((HttpContext.Current != null) && RequestContext.LogCurrentError)
            {
                ...
                if (ex != null)
                {
                    const string eventCode = "EXCEPTION";
                    const string eventType = EventType.ERROR;

                    // Log request operation
                    RequestDebug.LogRequestOperation("OnError", ex.Message, 0);

                    // Page not found was already manually logged
                    bool log = RequestContext.CurrentStatus != RequestStatusEnum.PageNotFound;
                    ...
                    if (log)
                    {
                        ...
                        try
                        {
                            ...
                            if (logException)
                            {
                                ...
                                try
                                {
                                    EventLogProvider.LogEvent(eventType, "Application_Error", eventCode, EventLogProvider.GetExceptionLogMessage(ex));
                                }
                                catch
                                {
        ...
        private void LogAction(GeneralizedInfo infoObj, string eventCode, string eventDescription, bool changedFieldsOnly = false)
        {
            ...
            if (infoObj.LogEvents && EventLogProvider.LogMetadata(infoObj.ObjectSiteName))
            {
                ...
                string name = objectName + " '" + ResHelper.LocalizeString(infoObj.ObjectDisplayName) + "'";
