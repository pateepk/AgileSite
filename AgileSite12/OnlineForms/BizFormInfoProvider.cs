...
namespace CMS.OnlineForms
{
    ...
    public class BizFormInfoProvider : AbstractInfoProvider<BizFormInfo, BizFormInfoProvider>
    {
        ...
        internal const string CLEAR_CACHED_BIZFORMS_COUNT_ACTION_NAME = "ClearCachedBizFormsCount";
        ...
        {
            ClearBizHashInternal();

            ProviderObject.CreateWebFarmTask(CLEAR_CACHED_BIZFORMS_COUNT_ACTION_NAME, null);
        }


        private static void ClearBizHashInternal()
        {
            licBizForms.Clear();
            BizFormItemProvider.ClearLicensesCount();
        }
        ...
        /// <param name="formDevelopmentModel">Development model of the form whose file columns are retreived.</param>
        public static string[] GetBizFormFileColumns(string classFormDefinition, FormDevelopmentModelEnum formDevelopmentModel = FormDevelopmentModelEnum.WebForms)
        {
            string[] result = null;

            FormInfo fi = new FormInfo(classFormDefinition);
            List<FormFieldInfo> fileColumns = null;

            // Get fields with uploader control type
            if (formDevelopmentModel == FormDevelopmentModelEnum.Mvc)
            {
                fileColumns = fi.GetFields(BizFormUploadFile.DATATYPE_FORMFILE);
            }
            // Get fields with BizForm file data type
            else
            {
                fileColumns = fi.GetFields(FormFieldControlTypeEnum.UploadControl);
            }

            if ((fileColumns != null) && (fileColumns.Any()))
            {
                result = fileColumns.Select(x => x.Name).ToArray();
            }
        ...
        {
            if (String.Equals(actionName, CLEAR_CACHED_BIZFORMS_COUNT_ACTION_NAME, StringComparison.OrdinalIgnoreCase))
            {
                ClearBizHashInternal();
            }
            else
