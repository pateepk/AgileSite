...
{
    /// <summary>
    /// Web part for Managing a single row in a custom table (insert/delete/update a single row on a custom table)
    /// </summary>
    public partial class WTE_CustomTableManage : CMSAbstractWebPart
    {
        #region "Properties"
        ...
        {
            bool success = true;
            success = ProcessManageCustomTable();
            return success;
        }
        ...
        /// </summary>
        /// <returns></returns>
        protected bool ProcessManageCustomTable()
        {
            bool success = false;
            ...
            if (!String.IsNullOrEmpty(action))
            {
                try
                {
                    ...
                    CustomTableItem item = GetCustomTableItem(out tempMessage);
                    ...
                    if (item != null)
                    {
                        ...
                        {
                            case "delete":
                                item = DeleteItem(item, out success, out message);
                            ...
                            case "copy":
                                item = CopyItem(item, out success, out message);
                                if (success && item != null)
                                {
                            ...
                            case "enable":
                                item = EnableItem(item, true, out success, out message);
                                if (success && item != null)
                                {
                                    ...
                                    //else
                                    //{
                                    //    redirectURL = RequestContext.CurrentURL; // redirect back to the current page?
                                    //}
                                }
                                break;

                            case "disable":
                                item = EnableItem(item, false, out success, out message);
                                if (success && item != null)
                                {
                                    if (!String.IsNullOrWhiteSpace(RedirectURL))
                                    {
        ...
        /// <param name="p_obj"></param>
        /// <returns></returns>
        public static object DeserializeJSON(string jsonString, Type p_objectType)
        {
            Object obj = null;
        ...
        public static void RedirectAndPOSTData(Page page, string p_token, string postUrl)
        {
            try
            {
                ...
                string strForm = PreparePOSTForm(postUrl, data);
        ...
        /// <returns></returns>
        private static String PreparePOSTForm(string p_postUrl, NameValueCollection data)
        {
