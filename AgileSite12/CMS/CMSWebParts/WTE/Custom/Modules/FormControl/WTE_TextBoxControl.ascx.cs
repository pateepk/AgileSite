...
{
    public partial class WTE_TextBoxControl : FormEngineUserControl
    {
        #region "Constants"
        ...
        public string DisabledStyle
        {
            get
            {
                return ValidationHelper.GetString(GetValue("DisabledStyle"), "");
            }
            set
            {
        ...
        public string DisabledCSSClass
        {
            get
            {
                return ValidationHelper.GetString(GetValue("DisbledCSSClass"), "");
            }
            set
            {
                SetValue("DisbledCSSClass", value);
            }
        }
        ...
        public bool LockTextBox
        {
            get
            {
                ...
                try
                {
                    if (!String.IsNullOrWhiteSpace(Request.QueryString[PrefillDataKey]))
                    {
        ...
        /// <summary>
        /// Session/Querystring key to look for prefill data.
        /// </summary>
        public string PrefillDataKey
        {
            get
            {
                return ValidationHelper.GetString(GetValue("PrefillDataKey"), String.Empty);
            }
            set
            {
                SetValue("PrefillDataKey", value);
            }
        }
        ...
        public bool AutoDisable
        {
            get
            {
                return ValidationHelper.GetBoolean(GetValue("AutoDisable"), false);
            }
            set
            {
                SetValue("AutoDisable", value);
            }
        }
        ...
        public override object Value
        {
            ...
            set
            {
                ...
                string setval = ValidationHelper.GetString(value, null);
                if (String.IsNullOrWhiteSpace(setval))
                {
                    if (!String.IsNullOrWhiteSpace(Request.QueryString[PrefillDataKey]))
                    {
                        setval = Request.QueryString[PrefillDataKey];
                        disabletextBox = true;
                    }
                }
                // Convert the value to a proper type
                value = ConvertInputValue(setval);
                TextBox.Text = ValidationHelper.GetString(value, null);
                TextBox.Enabled = !disabletextBox;
            }
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (AutoDisable)
            {
                TextBox.Enabled = !LockTextBox;

                if (!TextBox.Enabled)
                {
                    if (!String.IsNullOrWhiteSpace(DisabledStyle))
                    {
                        // override the CSS to make it look disabled
                        TextBox.Attributes["style"] = DisabledStyle;
                    }

                    if (!String.IsNullOrWhiteSpace(DisabledCSSClass))
                    {
                        //TextBox.Attributes["class"] = DisabledCSSClass + " " + TextBox.Attributes["Class"];
                        TextBox.CssClass = DisabledCSSClass + " " + TextBox.CssClass;
                    }
                }
            }
        ...
        public override bool IsValid()
        {
            if (IsTextArea)
            {
                ...
                bool valid = CheckLength(0, maxControlSize, textLength, ref error);
