            ...
            else if (RegistrationInfo.IsEmailLogin || !String.IsNullOrWhiteSpace(p_email))
            {
                ...
                if (ret)
                {
                    ...
                    if (!UserInfoProvider.IsEmailUnique(p_email, p_siteList, 0))
                    {
                        if (p_compareUser != null)
                        {
                            UserInfo existingUser = existingUser = UserInfoProvider.GetUsersDataWithSettings()
                                .WhereEquals("Email", p_email)
                                .TopN(1)
                                .FirstOrDefault();
                                ...
                                if (!hasError)
                                {
                                    ...
                                    if (!String.IsNullOrWhiteSpace(ui.Email) && ui.Email != DefaultPasswordResetEmail)
                                    {
                                        ...
                                        if (template != null)
                                        {
                                            ...
                                            var resolver = MembershipResolvers.GetForgottenPasswordResolver(ui, txtPassword.Text, "https://www.trainingnetworknow.com/");
