...
namespace CMS.Ecommerce
{
    ...
    public class CMSAuthorizeNetProvider : CMSPaymentGatewayProvider, IDirectPaymentGatewayProvider, IDelayedPaymentGatewayProvider
    {
        ...
        private const string MESSAGE_CODE_AUTHENTICATION_FAILED = "E00007";
        ...
        protected virtual void ProcessPaymentInternal(IDictionary<string, object> paymentData)
        {
            ...
            // Error occurred while processing request on gateway
        ...
        protected virtual void UpdatePaymentInfo(CreateTransactionResponse response)
        {
            ...
            AuthorizeNetPaymentResult.PaymentStatusValue = ValidationHelper.GetString(response.TransactionResponse.ResponseCode, "", System.Globalization.CultureInfo.InvariantCulture);
        ...
        /// <remarks>
        /// Any occurred error is logged to event log and stored in <see cref="AuthorizeNetPaymentResult"/>.
        /// </remarks>
        protected virtual ResponseResult ProcessRequest(CreateTransactionRequest request)
        {
            ...
            try
            {
                ...
                // If specific kind of error occurs, ErrorResponse is returned
        ...
        protected virtual bool TryProcessError(ResponseResult response)
        {
            ...
            if (transactionResponse != null)
            {
                ...
                // If the error is about invalid credentials, we should log it into event log.
                // We are going to rely on the following error code to identify such response.
                if (MESSAGE_CODE_AUTHENTICATION_FAILED.Equals(transactionResponse.MainMessages.MainMessage.Code, StringComparison.OrdinalIgnoreCase))
                {
                    LogEvent(sbDescription.ToString(), ERROR_CODE);
                }
            }
        ...
        protected virtual CreateTransactionRequest GetPaymentRequest(IDictionary<string, object> paymentData, TransactionType transactionType)
        {
            ...
            request.RefId = ValidationHelper.GetString(Order.OrderID, "", System.Globalization.CultureInfo.InvariantCulture);
            ...
            request.TransactionRequest.Shipping = GetPartialAmount(shipAmount, shipOption?.ShippingOptionDisplayName, shipOption?.ShippingOptionDescription);

            request.TransactionRequest.PONumber = ValidationHelper.GetString(Order.OrderID, "", System.Globalization.CultureInfo.InvariantCulture);
        ...
        /// <exception cref="InvalidOperationException">Thrown when merchant login ID or merchant transaction key exceeds maximal length.</exception>
        protected virtual MerchantAuthentication GetMerchantAuthentication()
        {
            var settingService = mSettingFactory.GetSettingService(ShoppingCartInfoObj.ShoppingCartSiteID);

            var merchantName = settingService.GetStringValue(API_LOGINID_SETTING_NAME);
            if (merchantName.Length > 25)
            {
                throw new InvalidOperationException("Merchant login ID exceeds maximal length of 25, defined by Authorize.NET API.");
            }

            var merchantTransactionKey = settingService.GetStringValue(API_TRANSACTIONKEY_SETTING_NAME);
            if (merchantTransactionKey.Length > 16)
            {
                throw new InvalidOperationException("Merchant transaction key exceeds maximal length of 16, defined by Authorize.NET API.");
            }
        ...
        protected virtual Customer GetCustomer()
        {
            ...
            if (customer != null)
            {
                result.Id = ValidationHelper.GetString(customer.CustomerID, "", System.Globalization.CultureInfo.InvariantCulture);
                result.Email = LimitTextLength(customer.CustomerEmail, 255);
            }
        ...
        protected virtual Order GetOrder()
        {
            return new Order()
            {
                InvoiceNumber = LimitTextLength(Order.OrderInvoiceNumber, 20),
                Description = LimitTextLength(Order.OrderNote, 255)
            };
        ...
        protected virtual PartialAmount GetPartialAmount(string amount, string name = null, string description = null)
        {
            ...
            result.Name = LimitTextLength(ResHelper.LocalizeString(name), 31);
            result.Description = LimitTextLength(ResHelper.LocalizeString(description), 255);
        ...
        protected virtual Address GetAddress(OrderAddressInfo address)
        {
            if (address == null)
            {
                return null;
            ...
            result.FirstName = LimitTextLength(address.AddressPersonalName, 50);
            ...
            result.AddressLine = LimitTextLength(result.AddressLine, 60);

            result.City = LimitTextLength(address.AddressCity, 40);
            result.Zip = LimitTextLength(address.AddressZip, 20);
            result.Country = CountryInfoProvider.GetCountryInfo(address.AddressCountryID)?.CountryThreeLetterCode.ToNullIfEmpty();
            result.State = LimitTextLength(StateInfoProvider.GetStateInfo(address.AddressStateID)?.StateCode, 40);
        ...
        {
            return TextHelper.LimitLength(text, maxLength, "").ToNullIfEmpty();
        }
    }
}