...
namespace CMS.Ecommerce
{
    ...
    public class CMSPayPalProvider : CMSPaymentGatewayProvider, IDirectPaymentGatewayProvider, IDelayedPaymentGatewayProvider
    {
        ...
        protected internal virtual Payment GetPayment(string paymentIntent = AUTHORIZE_INTENT)
        {
            var payment = new Payment
            {
        ...
        protected virtual void ValidatePayment(Payment payment)
        {
            ...
            if (total != calculatedTotal)
            {
                ...
                {
                    name = ResHelper.GetString("PaymentGateway.PayPal.RoundingCorrection"),
        ...
        protected virtual void AddOrderDiscountItem(List<Item> items)
        {
            ...
            if (orderDiscount > 0m)
            {
                ...
                items.Add(GetItem(ResHelper.GetString("PaymentGateway.PayPal.OrderDiscount"), 1, NEGATIVE_MULTIPLIER * orderDiscount));
            }
        }
        ...
        protected virtual void AddOtherPaymentItem(List<Item> items)
        {
            ...
            if (otherPayments > 0m)
            {
                items.Add(GetItem(ResHelper.GetString("PaymentGateway.PayPal.OtherPayments"), 1, NEGATIVE_MULTIPLIER * otherPayments));
            }
        }
        ...
        {
            return GetAddress<Address>(Order.OrderBillingAddress);
        }
        ...
        {
            return GetAddress<ShippingAddress>(Order.OrderShippingAddress);
        }


        private static T GetAddress<T>(OrderAddressInfo orderAddress) where T : Address, new()
        {
            ...
            var address = new T
            {
                city = orderAddress.AddressCity.ToNullIfEmpty(),
                line1 = orderAddress.AddressLine1.ToNullIfEmpty(),
                line2 = orderAddress.AddressLine2.ToNullIfEmpty(),
                country_code = CountryInfoProvider.GetCountryInfo(orderAddress.AddressCountryID)?.CountryTwoLetterCode.ToNullIfEmpty(),
                state = StateInfoProvider.GetStateInfo(orderAddress.AddressStateID)?.StateCode.ToNullIfEmpty(),
                postal_code = orderAddress.AddressZip.ToNullIfEmpty(),
                phone = orderAddress.AddressPhone.ToNullIfEmpty()
            };
