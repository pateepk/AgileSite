...
namespace CMS.Ecommerce
{
    ...
    public class ECommerceContext : AbstractContext<ECommerceContext>
    {
        ...
        /// <summary>
        /// <para>
        /// Gets or sets the current shopping cart. The cart is retrieved using the <see cref="IShoppingService.GetCurrentShoppingCart"/> method
        /// while setting the cart is done by <see cref="ICurrentShoppingCartService.SetCurrentShoppingCart"/> method.
        /// </para>
        /// <para>
        ...
        /// </para>
        /// </summary>
        /// <seealso cref="IShoppingService"/>
        /// <seealso cref="ICurrentShoppingCartService"/>
        public static ShoppingCartInfo CurrentShoppingCart
        {
            get
            {
                if ((CMSHttpContext.Current != null) && (Current.mCurrentShoppingCart == null))
                {
                    DebugHelper.SetContext("CurrentShoppingCart");

                    Current.mCurrentShoppingCart = Service.Resolve<IShoppingService>().GetCurrentShoppingCart();
            ...
            set
            {
                if ((CMSHttpContext.Current != null) && (CMSHttpContext.Current.Session != null))
                {
                    Service.Resolve<ICurrentShoppingCartService>().SetCurrentShoppingCart(value);

                    // Context must be set after SetCurrentShoppingCart call, which invalidates it
        ...
        /// <summary>
        /// Invalidates the current shopping cart in a per request cache used by the <see cref="CurrentShoppingCart"/> member.
        /// </summary>
        /// <seealso cref="CurrentShoppingCart"/>
        /// <remarks>
        /// The cache is automatically invalidated when calling the default implementation of <see cref="ICurrentShoppingCartService.SetCurrentShoppingCart"/> method.
        /// </remarks>
        public static void InvalidateCurrentShoppingCartCache()
        {
            if ((CMSHttpContext.Current != null) && (CMSHttpContext.Current.Session != null))
            {
                Current.mCurrentShoppingCart = null;
            }
        }
