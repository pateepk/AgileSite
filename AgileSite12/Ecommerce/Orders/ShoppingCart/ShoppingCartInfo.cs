...
namespace CMS.Ecommerce
{
    ...
    public class ShoppingCartInfo : AbstractInfo<ShoppingCartInfo>
    {
        ...
        private AddressInfo mShoppingCartBillingAddress;
        private AddressInfo mShoppingCartShippingAddress;
        private AddressInfo mShoppingCartCompanyAddress;
        ...
        /// </summary>
        [RegisterProperty]
        public AddressInfo ShoppingCartBillingAddress
        {
            get
            {
            ...
            set
            {
                ShoppingCartBillingAddressID = value?.AddressID ?? 0;
        ...
        /// </summary>
        [RegisterProperty]
        public AddressInfo ShoppingCartShippingAddress
        {
            get
            {
            ...
            set
            {
                ShoppingCartShippingAddressID = value?.AddressID ?? 0;
        ...
        /// </summary>
        [RegisterProperty]
        public AddressInfo ShoppingCartCompanyAddress
        {
            get
            {
            ...
            set
            {
                ShoppingCartCompanyAddressID = value?.AddressID ?? 0;
        ...
        {
            if (ShoppingCartShippingOptionID == 0 && ECommerceContext.OnlyOneShippingOptionAvailableOnSite && IsShippingNeeded)
            {
                // Set the only one available shipping option as selected
                var option = ShippingOptionInfoProvider.GetShippingOptions(ShoppingCartSiteID, true).TopN(1).FirstOrDefault();
                ShoppingCartShippingOptionID = option.ShippingOptionID;

                return option;
            }

            return ShippingOptionInfoProvider.GetShippingOptionInfo(ShoppingCartShippingOptionID);
        }


        private PaymentOptionInfo GetPaymentOption()
        {
            if (ShoppingCartPaymentOptionID == 0 && ECommerceContext.OnlyOnePaymentOptionAvailableOnSite)
            {
                // Set the only one available payment option as selected
                var option = PaymentOptionInfoProvider.GetPaymentOptions(ShoppingCartSiteID, true).TopN(1).FirstOrDefault();
                ShoppingCartPaymentOptionID = option.PaymentOptionID;

                return option;
            }

            return PaymentOptionInfoProvider.GetPaymentOptionInfo(ShoppingCartPaymentOptionID);
        }
            ...
            : base(info, context, TYPEINFO)
        {
            ...
            ShoppingCartBillingAddress = (AddressInfo)info.GetValue(nameof(ShoppingCartBillingAddress), typeof(AddressInfo));
            ShoppingCartShippingAddress = (AddressInfo)info.GetValue(nameof(ShoppingCartShippingAddress), typeof(AddressInfo));
            ShoppingCartCompanyAddress = (AddressInfo)info.GetValue(nameof(ShoppingCartCompanyAddress), typeof(AddressInfo));
