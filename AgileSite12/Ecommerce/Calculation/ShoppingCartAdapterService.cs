    ...
    public class ShoppingCartAdapterService : IShoppingCartAdapterService
    {
        ...
        public virtual CalculationRequest GetCalculationRequest(ShoppingCartInfo cartInfo)
        {
            ...
            request.Items = SelectCartItems(cartInfo.CartItems)
                .Select(item => SelectItem(item, cartInfo.Currency))
                .ToList()
                .AsReadOnly();
        ...
        private CalculationRequestItem SelectItem(ShoppingCartItemInfo item, CurrencyInfo currency)
        {
            ...
            if (item.OrderItem != null)
            {
                ...
                var prices = new ProductPrices(unitPrice, unitPrice + discounts.Sum(d => d.Value), currency, discounts);
