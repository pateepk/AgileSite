...
namespace CMS.Ecommerce.ExchangeRates
{
    internal class DefaultCurrencyConverter : ICurrencyConverter
    {
        ...
        /// <exception cref="ArgumentException">Thrown when currency with given code does not exist in the system.</exception>
        /// <exception cref="InvalidCurrencyConversionException">Thrown when conversion fails (e.g. exchange rate not found).</exception>
        ...
        {
            var fromCurrency = GetCurrency(fromCurrencyCode) ?? throw new ArgumentException($"Currency with code '{fromCurrencyCode}' not found.", nameof(fromCurrencyCode));
            var toCurrency = GetCurrency(toCurrencyCode) ?? throw new ArgumentException($"Currency with code '{toCurrencyCode}' not found.", nameof(toCurrencyCode));
