...
namespace CMS.DocumentEngine.Internal
{
    ...
    public class DocumentSearchablesRetriever : SearchablesRetriever
    {
        ...
        internal override IEnumerable<ISearchable> GetSearchableObjects(SearchIndexInfo index, string nodeAliasPath, string siteName = null)
        {
            List<int> sites;
            List<string> cultures;
        ...
        private void LogBatchStart(int batchNumber, int lastID, string documentTypeCodeName, string siteDisplayName, bool linkedNodes)
        {
            if (batchNumber == 1)
            {
                ...
                Logger.LogMessage(ResHelper.GetStringFormat(message, documentTypeDisplayName, siteDisplayName));
            }
            else
            {
