...
namespace CMS.DocumentEngine
{
    ...
    internal class GetAttachmentHandler : AdvancedGetFileHandler, IGetAttachmentHandler
    {
        ...
        private void CheckPermissions(CMSOutputAttachment attachment)
        {
            node = attachment.FileNode;
            if (node == null)
            {
                return;
            }
            ...
            {                
                ...
                if ((MembershipContext.AuthenticatedUser.IsAuthorizedPerDocument(node, NodePermissionsEnum.Read) == AuthorizationResultEnum.Denied) && (node.NodeOwner != MembershipContext.AuthenticatedUser.UserID))
                {
        ...
        protected void ProcessFile(Guid attachmentGuid)
        {
            ...
            {
                secured = node.IsSecuredNode.HasValue && node.IsSecuredNode.Value;
            }
        ...
        private void CheckNodeSecurity()
        {
