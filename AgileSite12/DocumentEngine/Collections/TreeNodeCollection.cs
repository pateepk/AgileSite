...
namespace CMS.DocumentEngine
{
    ...
    public class TreeNodeCollection : InfoObjectCollection<TreeNode>
    {
        ...
        private MultiDocumentQuery BuildQuery(IWhereCondition where)
        {
            ...
            var includeCoupledData = SelectAllData ? IncludeCoupledDataEnum.Complete : IncludeCoupledDataEnum.None;

            // Ensure ClassName column and only required coupled data columns if columns are explicitly specified
            var columns = Columns;
            if (!string.IsNullOrEmpty(columns) && SelectAllData)
            {
                columns = SqlHelper.MergeColumns(columns, "ClassName");
                includeCoupledData = IncludeCoupledDataEnum.InnerQueryOnly;
            }
            ...
            var query =
                DocumentHelper.GetDocuments()
                              ...
                              .TopN(TopN)
                              .Columns(columns)
                              .WithCoupledColumns(includeCoupledData)
