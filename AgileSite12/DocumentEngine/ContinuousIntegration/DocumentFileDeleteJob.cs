    ...
    public class DocumentFileDeleteJob : FileSystemDeleteJob
    {
        ...
        protected override IEnumerable<string> SelectFilesToDelete(BaseInfo baseInfo)
        {
            ...
            if (documentCollection != null)
            {
                if (deleteMainFile)
                {
                    relativePaths.AddRangeToSet(documentCollection);
                }
                else
                {
                    ...
                    relativePaths.AddRangeToSet(structuredPaths.Select(location => location.MainLocation));

                    // Add separated field's file parts
                    relativePaths.AddRangeToSet(structuredPaths.SelectMany(location => location));
                }
            }
            else if (deleteMainFile)
            {
                relativePaths.Add(nodeMainPath);
                relativePaths.AddRangeToSet(locationCollections.SelectMany(collection => collection.StructuredLocations)
                                                          .Where(location => location.MainLocation.EqualsCSafe(nodeMainPath, true))
