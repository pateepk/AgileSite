...
namespace CMS.DocumentEngine
{
    internal class FileSystemDeleteDocumentsByTypeJob : FileSystemDeleteObjectsByTypeJob
    {
        ...
        {
            var conditions = RepositoryConfiguration.ObjectTypesFilterConditions;

            if (conditions != null && conditions.TryGetValue(objectType, out var whereCondition))
            {
                return new GetDocumentsEnumerator(objectType, whereCondition);
            }
