...
namespace CMS.DocumentEngine
{
    ...
    public static class DocumentHelper
    {
        ...
        /// </summary>
        public const string DOCUMENT_PREFIX = PredefinedObjectType.DOCUMENT_ITEM_PREFIX;
        ...
        public static bool DeleteDocument(DeleteDocumentSettings settings)
        {
            ...
            if (node.IsLink)
            {
                EnsureAlternatingDocument(node, settings, true);
        ...
        public static void MoveDocument(MoveDocumentSettings settings)
        {
            ...
            // Check the alternative URLs on the target site to prevent duplicity
            if (moveAcrossSite && AlternativeUrlHelper.IsAlternativeUrlsSiteConflict(node, targetSiteId))
            {
                throw new InvalidOperationException("[DocumentHelper.MoveDocument]: The action was canceled due to conflicts between alternative URLs of the moved pages and alternative URLs on the target site.");
            }
        ...
        public static TreeNode EnsureBlogPostHierarchy(TreeNode node, TreeNode parent, TreeProvider tree)
        {
            ...
            var cultureInfo = CultureHelper.GetCultureInfo(node.DocumentCulture);
        ...
        private static void EnsureAlternatingDocument(TreeNode deletedDocument, DeleteDocumentSettings settings, bool lastCulture)
        {
            ...
            if (deletedDocument.IsLink)
            {
                CreateDocumentAlias(deletedDocument, settings.AlternatingDocument.NodeID, deletedDocument.NodeAliasPath, string.Empty);

                if (settings.AlternatingDocumentCopyAllPaths)
                {
                    DocumentAliasInfoProvider.MoveAliases(deletedDocument.NodeID, settings.AlternatingDocument.NodeID, string.Empty);
                }
            }
            else
            {
                ...
                {
                    CreateDocumentAlias(deletedDocument, settings.AlternatingDocument.NodeID, path, deletedDocument.DocumentCulture);
                }
            }
        ...
        private static void CreateDocumentAlias(TreeNode deletedDocument, int nodeId, string path, string culture)
        {
            if (!DocumentAliasInfoProvider.IsUnique(path, 0, deletedDocument.DocumentCulture, deletedDocument.DocumentExtensions, deletedDocument.NodeSiteName, false, nodeId))
            {
                return;
            }
