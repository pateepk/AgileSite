...
namespace CMS.DocumentEngine
{
    ...
    public class DocumentURLProvider : AbstractBaseProvider<DocumentURLProvider>
    {
        ...
        /// </summary>
        /// <remarks>
        /// Returning <c>null</c> the event handler indicates that given <see cref="TreeNode"/> parameter is not a valid AB test variant or AB testing feature is not allowed. 
        /// </remarks>
        ...
        public static List<DocumentCultureUrl> GetDocumentCultureUrls(TreeNode node, string originalUrl, string excludedCultureCode = "", UrlOptionsEnum options = UrlOptionsEnum.ExcludeUntranslatedDocuments | UrlOptionsEnum.UseCultureSpecificURLs)
        {
            ...
            // Try to get original AB variant node if current node (parameter) is a variant of running AB test.
            // If found then generate culture URLs for the original variant node.
            var originalVariantNode = GetABTestOriginalVariant(node);
            string originalAliasPath = originalVariantNode?.NodeAliasPath ?? node.NodeAliasPath;

            // Find culture versions of page
            TreeNodeCollection translatedDocuments = originalVariantNode?.CultureVersions ?? node.CultureVersions;
        ...
        public static string GetAbsoluteLiveSiteURL(TreeNode node)
        {
            ...
            try
            {
                presentationUri = new UriBuilder(GetPresentationUrl(node, RequestContext.FullDomain));
            }
        ...
        protected virtual string GetUrlInternal(string aliasPath, string urlPath, string siteName, string langPrefix, string extension)
        {
            ...
            if ((si == null) || (CMSHttpContext.Current == null))
            {
        ...
        /// <summary>
        /// Returns a <see cref="TreeNode"/> which represents an original variant of the given <paramref name="node"/> if it an AB test variant.
        /// If the AB Testing module is not installed, returns <c>null</c>.
        /// </summary>
        /// <param name="node">The variant node</param>
        private static TreeNode GetABTestOriginalVariant(TreeNode node)
        {
            return OnGetABTestOriginalVariant?.Invoke(node);
        }
