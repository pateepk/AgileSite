...
namespace CMS.DocumentEngine
{
    ...
    public class TreeNode : AbstractCompositeInfo<TreeNode>, ITreeNode, IDisposable, ITreeNodeMethods
    {
        ...
        /// <param name="readonlyMode">Indicates if preview link should be in read only mode.</param>
        /// <param name="embededInAdministration">Indicates if page is embedded in administration inside an iframe.</param>
        public virtual string GetPreviewLink(string userName, bool ensureQueryHash = false, bool readonlyMode = true, bool embededInAdministration = false)
        {
            return new PreviewLinkGenerator(this).Generate(userName, ensureQueryHash, readonlyMode, embededInAdministration);
        }
        ...
        /// </remarks>
        protected DocumentAttachment GetFieldDocumentAttachment(string fieldName)
        {
            if (fieldName == null)
            {
            ...
            var attachmentGuid = ValidationHelper.GetGuid(GetValue(fieldName), Guid.Empty);
            if (attachmentGuid == Guid.Empty)
            {
                return null;
            }

            return AllAttachments.SingleOrDefault(x => x.GetGuidValue("AttachmentGUID", Guid.Empty) == attachmentGuid);
        }
        ...
        /// </remarks>
        protected IEnumerable<DocumentAttachment> GetFieldDocumentAttachments(string fieldName)
        {
            var collection = GroupedAttachments[fieldName];

            return collection ?? Enumerable.Empty<DocumentAttachment>();
        }
        ...
        /// This method returns null if the specified field does not exist or there is no associated attachment.
        /// </remarks>
        [Obsolete("Use GetFieldDocumentAttachment().")]
        protected Attachment GetFieldAttachment(string fieldName)
        {
            var attachment = GetFieldDocumentAttachment(fieldName);
            return attachment == null ? null : new Attachment(attachment);
        }
        ...
        /// This method returns an empty collection if the specified field does not exist or there are no associated attachments.
        /// </remarks>
        [Obsolete("Use GetFieldDocumentAttachments().")]
        ...
        private void EnsureWildcardValues()
        {
            ...
            DocumentWildcardRule = DocumentAliasInfoProvider.CreateWildcardRule(wildcardUrlPath, out var wildcardSectionsCount);

            // Count priority and set it
            int slashCount = wildcardUrlPath.Split('/').Length - 1;
            DocumentPriority = slashCount - wildcardSectionsCount;
        }
        ...
        {
            // Reset DocumentGUID for new culture version
            if (TreeProvider.GenerateNewGuid)
            {
                DocumentGUID = Guid.NewGuid();
            }

            GenerateNewWorkflowCycleGUID();
        ...
        /// </summary>
        private void EnsureDefaultDataForInsert()
        {
            EnsureNodeGUID();
            EnsureDocumentGUID();
            GenerateNewWorkflowCycleGUID();
        ...
        private void GenerateNewWorkflowCycleGUID()
        {
