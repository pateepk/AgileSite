...
namespace CMS.BannerManagement
{
    ...
    public class BannerCategoryInfoProvider : AbstractInfoProvider<BannerCategoryInfo, BannerCategoryInfoProvider>
    {
        ...
        /// <summary>
        /// Logs export task for each banner belonging to the category and deletes the category.
        /// </summary>
        /// <param name="info">Banner category to delete.</param>
        protected override void DeleteInfo(BannerCategoryInfo info)
        {
            var categoryId = info.BannerCategoryID;

            var banners = BannerInfoProvider.GetBanners()
                .Where(b => b.BannerCategoryID == categoryId);

            foreach (var banner in banners)
            {
                ExportTaskInfoProvider.LogTask(banner, TaskTypeEnum.DeleteObject);
            }
