...
namespace CMS.SharePoint
{
    ...
    internal class SharePoint2010ListService : SharePointAbstractService, ISharePointListService
    {
        ...
        private readonly string MULTIPLE_VALUE_DELIMITER = Environment.NewLine;
        ...
        private DataTable ListItemCollectionToDataTable(ListItemCollection listItemCollection)
        {
            ...
            foreach (ListItem listItem in listItemCollection)
            {
                ...
                {
                    dr[pair.Key] = GetFieldValue(pair.Value);
                }
        ...
        /// <summary>
        /// Returns Sharepoint list item's field value in the form which is suitable for storing in <see cref="DataRow"/> value.
        ...
        /// <remarks>
        /// <paramref name="fieldValue"/> may represent a complex type which can't be simply stored in <see cref="DataRow"/> value. The method transforms
        ...
        private object GetFieldValue(object fieldValue)
        {
            ...
            var transformFunc = GetTransformFunction(type);

            if (isArray)
            {
                return String.Join(MULTIPLE_VALUE_DELIMITER, ((IEnumerable<object>)fieldValue).Select(o => transformFunc(o)));
            }
        ...
        private Func<object, object> GetTransformFunction(Type t)
        {
            // This condition involves FieldUserValue type
            if (typeof(FieldLookupValue).IsAssignableFrom(t))
            {
                return instance => 
                {
                    var fieldValue = (FieldLookupValue)instance;
                    return $"{fieldValue.LookupId};{fieldValue.LookupValue}";
                };
            }

            if (t == typeof(TaxonomyFieldValue))
            {
                return instance =>
                {
                    var taxonomyFieldValue = (TaxonomyFieldValue)instance;
                    return $"{taxonomyFieldValue.WssId};{taxonomyFieldValue.Label};{taxonomyFieldValue.TermGuid}";
                };
            }

            if (t == typeof(TaxonomyFieldValueCollection))
            {
                return instance =>
                {
                    var taxonomyCollection = (TaxonomyFieldValueCollection)instance;
                    return GetFieldValue(taxonomyCollection.ToArray());
                };
            }
