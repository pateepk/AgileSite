...
namespace CMS.FormEngine
{
    ...
    public static class FormHelper
    {
            ...
            using (var tr = new CMSTransactionScope())
            {
                ...
                foreach (IDataDefinitionItem elem in elems)
                {
                    ...
                    if (ffi != null)
                    {
                        if (ffi.PrimaryKey)
                        {
                            // Remember the index of currently processed item
                            previousItemIndex = inheritedFi.ItemsList.FindLastIndex(x => x is FormFieldInfo field && field.PrimaryKey);
                        }
                        else
                        {
                            // Prepare the info
                            ...
                            else
                            {
                                ...
                                {
                                    inheritedFi.AddFormItem(ffi, previousItemIndex + 1);
                                }
                                else
                                {
                            ...
                            // Remember the index of currently processed item
                            previousItemIndex = inheritedFi.ItemsList.FindLastIndex(x => x is FormFieldInfo field && String.Equals(field.Name, ffi.Name, StringComparison.OrdinalIgnoreCase));
                        }
                    }
                    else
                    {
                        ...
                        if (fci != null)
                        {
                            ...
                            else
                            {
                                if (previousItemIndex >= 0)
                                {
                                    inheritedFi.AddFormItem(fci, previousItemIndex + 1);
                                }
                                else
                                {
                                    inheritedFi.AddFormItem(fci, index);
                                }                              
                            }
                            ...
                            // Remember the index of currently processed item
                            previousItemIndex = inheritedFi.ItemsList.FindLastIndex(x => x is FormCategoryInfo cat && String.Equals(cat.CategoryName, fci.CategoryName, StringComparison.OrdinalIgnoreCase));
                        }
                    }
        ...
        /// <summary>
        /// Gets BizForm temporary files folder physical path based on <see cref="TempFileInfoProvider.GetTempFilesRootFolderPath(string)"/>.
        /// </summary>
        /// <param name="webFullPath">Physical path to the root of the web project (e.g. c:\WebProject\). If it is not specified the web application physical path is used.</param>
        ...
        {
            return Path.Combine(TempFileInfoProvider.GetTempFilesRootFolderPath(webFullPath), "FormFileUploader");
        }
