...
namespace CMS.Search
{
    ...
    public class SearchIndexer
    {
        ...
        public virtual void ExecuteTask(SearchTaskInfo sti)
        {
            ...
            {
                Logger.LogMessage(ResHelper.GetString("smartsearch.taskfinishederror"));
                throw;
            }

            Logger.LogMessage(ResHelper.GetString("smartsearch.taskfinishedok"));
        }
        ...
        public virtual bool FillSearchResult(SearchResultItem resultItem, DataRow resultDr, ILuceneSearchDocument doc)
        {
            ...
            resultItem.Title = ValidationHelper.GetString(DataHelper.GetDataRowValue(resultDr, dci.ClassSearchTitleColumn), String.Empty);
            resultItem.Content = ValidationHelper.GetString(DataHelper.GetDataRowValue(resultDr, dci.ClassSearchContentColumn), String.Empty);
            resultItem.Created = ValidationHelper.GetDateTime(DataHelper.GetDataRowValue(resultDr, dci.ClassSearchCreationDateColumn), DateTimeHelper.ZERO_TIME, CultureHelper.EnglishCulture);
            resultItem.Image = ValidationHelper.GetString(DataHelper.GetDataRowValue(resultDr, dci.ClassSearchImageColumn), String.Empty);
        ...
        public virtual string GetSearchImageUrl(SearchResultItem resultItem)
        {
            if (resultItem == null)
            {
            ...
            if (!String.IsNullOrEmpty(resultItem.Type))
            {
                var info = ModuleManager.GetReadOnlyObjectByClassName(resultItem.Type) as ISearchable;

                // Try to get object type-specific image
                if (info != null)
                {
                    return info.GetSearchImageUrl(resultItem.Image);
                }
            }
        ...
        [Obsolete("Use CMS.Search.SearchResultItem.GetSearchValue of corresponding search result instead.", true)]
