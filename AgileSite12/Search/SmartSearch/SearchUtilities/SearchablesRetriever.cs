...
namespace CMS.Search.Internal
{
    ...
    public class SearchablesRetriever
    {
        ...
        internal virtual IEnumerable<ISearchable> GetSearchableObjects(SearchIndexInfo index, string nodeAliasPath, string siteName = null)
        {
            throw new NotSupportedException($"Getting searchable objects by {nameof(nodeAliasPath)} is not supported by this class.");
        }
        ...
        private void LogFormBatchStart(int batchNumber, int lastID, string formDisplayName)
        {
            if (batchNumber == 1)
            {
                Logger.LogMessage(ResHelper.GetStringFormat("smartsearch.taskbatch.startform", formDisplayName));
            }
            else
            {
