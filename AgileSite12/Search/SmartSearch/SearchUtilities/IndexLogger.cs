...
namespace CMS.Search.Internal
{
    ...
    public class IndexLogger
    {
        ...
        public void LogTaskStart(SearchTaskInfo taskInfo)
        {
            ...
            string msg = ResHelper.GetStringFormat(
                ...
                objectInfo != null ? objectInfo.GetFullObjectName(false, true, false) : ResHelper.GetString("general.objectnotfound"),
        ...
        {
            LogMessage(ResHelper.GetStringFormat("smartsearch.taskbatch.processingstart", batchNumber, lastID));
        }
        ...
        {
            LogContext.Append(" " + ResHelper.GetStringFormat("smartsearch.taskbatch.processingend", itemsCount), "Search");
        }
        ...
        public void LogBatchStart(int batchNumber, int lastID, string objectType)
        {
            if (batchNumber == 1)
            {
                LogMessage(ResHelper.GetStringFormat("smartsearch.taskbatch.startobject", TypeHelper.GetNiceObjectTypeName(objectType)));
            }
            else
            {
        ...
        public void LogCustomTableBatchStart(int batchNumber, int lastID, string tableName)
        {
            ...
            if (batchNumber == 1)
            {
                LogMessage(ResHelper.GetStringFormat("smartsearch.taskbatch.startcustomtable", customTableDisplayName));
            }
            else
            {
                LogBatchStart(batchNumber, lastID);
