...
namespace CMS.Newsletters
{
    ...
    public class LinkConverter
    {
        ...
        private static readonly Regex linkRegex = RegexHelper.GetRegex(@"(?<prefix>\s(src|href)\s*=\s*)((?<quote>[""'])(?<link>.+?)\k<quote>)", RegexOptions.IgnoreCase);

        // Regular expression that matches an anchor hyperlink.
        private static readonly Regex hyperlinkRegex = RegexHelper.GetRegex(@"(?<prefix><a[\s|\S]*?href(\s)*?=(\s)*?)(?<quote>[""'])(?<url>.*?)\k<quote>(?<suffix>[\s|\S]*?>)(?<name>[\s|\S]*?)</a>", RegexOptions.IgnoreCase);
        ...
        {
            return RegexHelper.GetRegex($@"\s{attributeName}=(?<quote>[""'])(?<value>.*?)\k<quote>", RegexOptions.IgnoreCase);
        }
