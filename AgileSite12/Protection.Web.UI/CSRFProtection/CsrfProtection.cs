...
namespace CMS.Protection.Web.UI
{
    ...
    public sealed class CsrfProtection
    {
        ...
        /// </summary>
        private const string PURPOSES = "CSRF token protection";
        ...
        {
            return ValidationHelper.GetBoolean(SettingsHelper.AppSettings["CMSEnableCsrfProtection"], true);
        }
        ...
        private void ValidateTokenInPage(byte[] csrfToken)
        {
            ...
            if (IsVulnerableRequest(currentRequest))
            {
                ...
                {
                    throw new CsrfException("The CSRF hidden field was missing.", null);
                }
                ...
                catch (Exception exception)
                {
                    ...
                    throw new CsrfException("The CSRF hidden field was malformed.", exception);
                }
        ...
        internal void ValidateCsrfTokens(byte[] cookieToken, byte[] hiddenFieldToken)
        {
            if (cookieToken == null)
            {
            ...
            {
                throw new CsrfException("The CSRF hidden field value didn't match the CSRF cookie value.", null);
            }
        }
        ...
        internal byte[] EnsureCsrfToken()
        {
            ...
            if (String.IsNullOrEmpty(csrfCookieValue))
            {
                if (IsVulnerableRequest(currentRequest))
                {
                    throw new CsrfException("The CSRF cookie was missing.", null);
                }
            ...
            else
            {
                ...
                catch (FormatException ex)
                {
                    if (IsVulnerableRequest(currentRequest))
                    {
                        throw new CsrfException("The CSRF cookie was malformed.", ex);
                    }
        ...
        /// </summary>
        private byte[] EnsureCsrfTokenInternal()
        {
...
#pragma warning restore BH1011 // 'Page.IsPostBack' should not be used. Use 'RequestHelper.IsPostBack()' instead.
        ...
        private void IncludeTokenInPage(Page page, byte[] csrfToken)
        {
