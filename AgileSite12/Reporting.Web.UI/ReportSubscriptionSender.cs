...
namespace CMS.Reporting.Web.UI
{
    ...
    public class ReportSubscriptionSender : ITask
    {
            ...
            if (subscriptions.Any())
            {
                ...
                foreach (var subscription in subscriptions)
                {
                    ...
                    if (content != String.Empty)
                    {
                        ...
                        Dictionary<string, byte[]> dict = RequestStockHelper.GetItem(ri.ReportName) as Dictionary<string, byte[]>;
                        if (dict != null)
                        {
                            ...
                            RequestStockHelper.Remove(ri.ReportName);
                        }
        ...
        public static void AddToRequest(String key, String item, byte[] data)
        {
            ...
            {
                Dictionary<string, byte[]> arr = RequestStockHelper.GetItem(key) as Dictionary<string, byte[]>;
                arr?.Add(item, data);
                RequestStockHelper.Add(key, arr);
            }
            else
            {
