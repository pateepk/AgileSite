                            ...
                            if ((taskType == TaskTypeEnum.UpdateObject) || (taskType == TaskTypeEnum.CreateObject))
                            {
                                ...
                                if (infoObj != null)
                                {
                                    ...
                                    if (!DataHelper.DataSourceIsEmpty(dsData))
                                    {
                                        ...
                                        // Write data
                                        DataHelper.WriteDataSetToXml(dsData, xml, "FileBinaryData", null);
                    ...
                    if (h.CanContinue())
                    {
                        ...
                        if (SynchronizationActionManager.CanExecuteActions(settings))
                        {
                            ...
                            if (settings.RunAsynchronously)
                            {
                                ...
                                var key = settings.GetDuplicityKey("SynchronizationWorker");
                                Action locObjectChangeAction = () =>
                                {
                                ...
                                if (!InfoCloneActionContext.TryEnqueue(key, locObjectChangeAction, SynchronizationQueueWorker.Current.Enqueue))
                                {
                                    SynchronizationQueueWorker.Current.Enqueue(key, locObjectChangeAction);
                                }
                            }
                            else
                            {
