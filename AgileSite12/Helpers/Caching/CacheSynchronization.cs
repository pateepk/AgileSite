...
namespace CMS.Helpers
{
    ...
    public class CacheSynchronization
    {
        ...
        public static void LogTouchKeysTask(params string[] keys)
        {
            if (SynchronizeCache)
            {
                WebFarmHelper.CreateTask(new TouchCacheItemWebFarmTask() { Keys = keys });
            }
        }
        ...
        public static void LogRemoveTask(string originalKey, bool caseSensitive)
        {
            if (SynchronizeCache)
            {
                WebFarmHelper.CreateTask(new RemoveCacheItemWebFarmTask
                {
                    Key = originalKey,
                    IsCaseSensitive = caseSensitive
                });
        ...
        public static void LogClearFullPageCacheTask()
        {
            if (SynchronizeCache)
            {
                WebFarmHelper.CreateTask(new ClearFullPageCacheWebFarmTask());
            }
        }
    }
}