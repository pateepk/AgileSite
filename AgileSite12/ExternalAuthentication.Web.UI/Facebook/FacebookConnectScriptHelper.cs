...
namespace CMS.ExternalAuthentication.Web.UI
{
    ...
    public static class FacebookConnectScriptHelper
    {
        ...
        public static string GetFacebookInitScriptForSite(string siteName)
        {
            ...
            string script = $@"window.fbAsyncInit = function() {{ FB.init({{ appId: '{FacebookConnectHelper.GetFacebookApiKey(siteName)}', status: true, cookie: false, xfbml: true, oauth: true, version: '{sdkVersion}' }}); }};
...
FB.getLoginStatus(function (response) {
    ...
    if (hr.indexOf('confirmed=') < 0) {
      ...
      if (!window.fbRedirectedAfterLogon) {
        ...
        {
            return $"FB.getLoginStatus(function(response) {{ if (response.status === 'connected') {{ FB.logout(function () {{ {additionalScriptForLogout} }}); }} else {{ {additionalScriptForLogout} }} }});";
        }
