...
namespace CMS.DocumentEngine.Web.UI
{
    ...
    /// </summary>
    [ToolboxData("<{0}:BasicRepeaterWithEffect runat=server></{0}:BasicRepeaterWithEffect>")]
    [ToolboxItem(typeof(WebControlToolboxItem))]
    [ParseChildren(true)]
    [PersistChildren(true)]
    public class BasicRepeaterWithEffect : BasicRepeater, INamingContainer
    {
        ...
        /// </summary>
        [Category("Effect settings"), DefaultValue(""), Description("Indicates whether the data source contains a selected item. If it does, the control does not apply the effect.")]
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (!StopProcessing)
            {
                ...
                if (!string.IsNullOrEmpty(resolvedInlineCSS))
                {
                    ...
                    CssRegistration.RegisterCssBlock(Page, "basicRepeaterWithEffectInlineCss_" + ClientID, resolvedInlineCSS);
                }
            }
        ...
        protected override void Render(HtmlTextWriter output)
        {
            if (Context == null)
            {
                output.Write("[BasicRepeaterWithEffect: " + ClientID + " ]");
                return;
            }
            ...
            {
                // Apply the effect only if the item is not selected
                if (!IsSelected && !HideLayoutForZeroRows)
                {
