...
namespace CMS.DocumentEngine.Web.UI
{
    ...
    public class CMSAbstractTransformation : AbstractUserControl, IDataItemContainer, IRelatedData
    {
            ...
            get
            {
                ...
                {                
                    case DataRowView drv:
                        return GerDataViewCount(drv);

                    case SearchResultItem sri:
                    {                       
                        return sri.Result.Items.Count;
                    }
        ...
        private static int GerDataViewCount(DataRowView drv)
        {
            if (drv.Row.Table.Columns.Contains(SystemColumns.TOTAL_RECORDS))
            {
                return ValidationHelper.GetInteger(drv.Row[SystemColumns.TOTAL_RECORDS], 0);
            }

            if (drv.Row.Table.Columns.Contains(SystemColumns.SUB_TOTAL_RECORDS))
            {
                return ValidationHelper.GetInteger(drv.Row[SystemColumns.SUB_TOTAL_RECORDS], 0);
            }
        ...
        {
            return TransformationHelper.HelperObject.GetSearchImageUrl((SearchResultItem)DataItem, noImageUrl, maxSideSize);
        }
        ...
        {
            return TransformationHelper.HelperObject.SearchHighlight((SearchResultItem)DataItem, text, startTag, endTag);
        }
        ...
        {
            return TransformationHelper.HelperObject.GetSearchedContent((SearchResultItem)DataItem, content);
        }
        ...
        {
            return TransformationHelper.HelperObject.GetSearchValue((SearchResultItem)DataItem, columnName);
        }
        ...
        {
            return TransformationHelper.HelperObject.SearchResultUrl((SearchResultItem)DataItem, absolute, addLangParameter);
        }
