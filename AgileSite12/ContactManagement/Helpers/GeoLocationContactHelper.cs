...
namespace CMS.ContactManagement
{
    ...
    public sealed class GeoLocationContactHelper
    {
        ...
        {
            var location = GeoIPHelper.GetLocationByIp(mIPAddress);
            if (location != null)
            {
        ...
        {
            string contactColumn = SettingsKeyInfoProvider.GetValue("CMSCMGeoOrganization", mSiteName);
        ...
        private void UpdateContactColumn(string geoIPsetting, object value)
        {
            if (value != null)
            {
                string contactColumn = SettingsKeyInfoProvider.GetValue(geoIPsetting, mSiteName);
                if (!String.IsNullOrEmpty(contactColumn) && String.IsNullOrEmpty(mContact.GetStringValue(contactColumn, null)))
                {
                    if (geoIPsetting.Equals("CMSCMGeoCountry", StringComparison.OrdinalIgnoreCase))
                    {
                        InsertCountryValue(contactColumn, value);
                    }
                    else if (geoIPsetting.Equals("CMSCMGeoState", StringComparison.OrdinalIgnoreCase))
                    {
        ...
        private object AppendSuffix(string contactColumn, object value)
        {
            if (mContact.Generalized.GetColumnType(contactColumn) == typeof(string))
            {
                if (mSuffix == null)
                {
                    mSuffix = SettingsKeyInfoProvider.GetValue("CMSCMGeoSuffix", mSiteName);
