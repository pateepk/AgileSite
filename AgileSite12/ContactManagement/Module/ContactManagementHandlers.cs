...
namespace CMS.ContactManagement
{
    ...
    internal class ContactManagementHandlers
    {
        ...
        public static void Init()
        {
            ...
            RuleInfo.TYPEINFO.Events.Insert.After += RefreshScore;
            RuleInfo.TYPEINFO.Events.Update.Before += RefreshScoreWhenRuleChanged;
            RuleInfo.PERSONARULETYPEINFO.Events.Insert.After += RefreshScore;
            RuleInfo.PERSONARULETYPEINFO.Events.Update.Before += RefreshScoreWhenRuleChanged;

            ContactGroupMemberInfo.TYPEINFOCONTACT.Events.Insert.After += ClearContactInfoGroupsCache;
            ContactGroupMemberInfo.TYPEINFOCONTACT.Events.Delete.Before += ClearContactInfoGroupsCache;
            ContactGroupMemberInfo.TYPEINFOCONTACT.Events.Update.Before += ClearContactInfoGroupCacheWhenChanged;
        }


        private static void ClearContactInfoGroupsCache(object sender, ObjectEventArgs e)
        {
            var member = e.Object as ContactGroupMemberInfo;
            if (member == null)
            {
                return;
            }

            ClearContactInfoGroupCache(member);
        }


        private static void ClearContactInfoGroupCacheWhenChanged(object sender, ObjectEventArgs e)
        {
            var member = e.Object as ContactGroupMemberInfo;
            if (member == null)
            {
            ...
            if (member.ChangedColumns()
                      .Intersect(new [] { "ContactGroupMemberContactGroupID", "ContactGroupMemberRelatedID"})
                      .Any())
            {
                ClearContactInfoGroupCache(member);
            }
        ...
        private static void ClearContactInfoGroupCache(ContactGroupMemberInfo member)
        {
            var keyParts = new List<string> { "ContactInfo.ContactGroups", member.ContactGroupMemberRelatedID.ToString() };
            var cacheKey = CacheHelper.BuildCacheItemName(keyParts);
            CacheHelper.Remove(cacheKey);
        }


        private static void RefreshScoreWhenRuleChanged(object sender, ObjectEventArgs e)
        {
            var rule = e.Object as RuleInfo;
            if (rule == null)
            {
                return;
            }
             
            var refreshScore = rule.ChangedColumns().Except(new[] { "RuleDisplayName", "RuleCodeName", "RuleLastModified" }).Any();
            if (!refreshScore)
            {
                return;
            }

            RefreshScore(rule.RuleScoreID);
        }


        private static void RefreshScore(object sender, ObjectEventArgs e)
        {
            var rule = e.Object as RuleInfo;
            if (rule == null)
            {
        ...
        private static void RefreshScore(int scoreId)
        {
        ...
        private static void TouchActivityDependencyCacheKeys(IList<IActivityInfo> activities)
        {
            ...
            contactIDs.AddRangeToSet(activities.Select(activity => activity.ActivityContactID));
        ...
        {
            var info = e.Object as ActivityInfo;
            if (info == null)
            {
                return;
            }
        ...
        private static void RunContactTriggers(IList<ContactChangeData> contactChanges)
        {
            ...
            var contactIds = notMergedContactChanges.Select(x => x.ContactID).ToHashSetCollection();
