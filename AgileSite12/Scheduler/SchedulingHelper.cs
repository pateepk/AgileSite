...
namespace CMS.Scheduler
{
    ...
    public static class SchedulingHelper
    {
        ...
        /// </summary>
        internal static bool? mEnableScheduler;
        ...
        public static void Clear(bool logTask)
        {
            ...
            if (logTask)
            {
                WebFarmHelper.CreateTask(new ClearSchedulingEnabledStatusWebFarmTask());
            }
        }
        ...
        public static DateTime GetFirstRunTime(TaskInterval taskInterval, DateTime? currentTime = null)
        {
            ...
            var isNoTimeWithPeriodOnce = (nextRunTime == TaskInfoProvider.NO_TIME) && taskInterval.Period.Equals(PERIOD_ONCE, StringComparison.Ordinal);

            return isNoTimeWithPeriodOnce
        ...
        public static DateTime GetNextTime(TaskInterval taskInterval, DateTime? lastPlannedRunTime = null, DateTime? currentTime = null)
        {
            ...
            // Special case when interval is set to one week period which can't match the day of week condition
            if (taskInterval.IsWeekPeriodWithSpecificDays())
            {
                var dummyTaskInterval = new TaskInterval
                {
                    BetweenEnd = taskInterval.BetweenEnd,
                    BetweenStart = taskInterval.BetweenStart,
                    Days = taskInterval.Days,
                    Day = taskInterval.Day,
                    Order = taskInterval.Order,
                    Period = taskInterval.Period,
                    StartTime = taskInterval.StartTime,
                    UseSpecificTime = taskInterval.UseSpecificTime,
                    Every = 1
                };

                return taskInterval.StartTime.MoveToNearestPeriodInFuture(taskInterval, baseTime).ShiftIntoAllowedDayAndTime(dummyTaskInterval, baseTime);
            }
            ...
            else
            {
                ...
                if (runScheduler)
                {
                    if (RunSchedulerWithinRequest)
                    {
                        ...
                        {
                            // Flush the output
                            CMSHttpContext.Current.Response.Flush();
                        }
                        catch
                        {
        ...
        /// </summary>
        /// <param name="taskInterval">Information about task repetition</param>
        private static bool IsWeekPeriodWithSpecificDays(this TaskInterval taskInterval)
        {
            return taskInterval.Period.Equals(PERIOD_DAY, StringComparison.Ordinal) && (taskInterval.Every % 7) == 0
                                                                                    && taskInterval.Days != null && taskInterval.Days.Any();
        }
        ...
        {
            if (!Enum.TryParse(taskInterval.Day, true, out DayOfWeek dayOfWeek))
            {
