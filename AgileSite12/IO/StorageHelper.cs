...
namespace CMS.IO
{
    ...
    public static class StorageHelper
    {
        ...
        {
            return Path.EnsureEndBackslash(GetFullFilePhysicalPath(path, webFullPath));
        }
        ...
        {
            DeleteOldFiles(dir, olderThan, deleteEmptyFolder, true);
        }
        ...
        /// <param name="olderThan">Limit time for deletion of a file</param>
        /// <param name="deleteEmptyFolder">Indicates if the specified folder should be deleted if there are no files left at the end</param>
        /// <param name="recursively">Indicates if the subfolders of the folder are to be processed.</param>
        /// <param name="deleteFileCallback">Callback accepting folder name and file respectively and returning a value indicating whether the file is to be deleted.</param>
        internal static void DeleteOldFiles(DirectoryInfo dir, DateTime olderThan, bool deleteEmptyFolder = true, bool recursively = true, Func<string, string, bool> deleteFileCallback = null)
        {
            if (dir.Exists)
            {
                // Go through files
                ...
                foreach (FileInfo file in files)
                {
                    ...
                    {
                        if (deleteFileCallback?.Invoke(dir.FullName, file.Name) ?? true)
                        {
                            file.Delete();
                        }
                    }
