...
namespace CMS.IO
{
    ...
    public abstract class AbstractStorageProvider
    {
        ...
        private const string STORAGE_MISCONFIGURED_MESSAGE = "The storage provider is not configured properly. If you are using a custom storage provider, make sure it is registered correctly.";
                ...
                if (mDirectoryProviderObject == null)
                {
                    lock (lockObject)
                    {
                        if (mDirectoryProviderObject == null)
                        {
                            ...
                            mDirectoryProviderObject = GetDirectoryProviderObject();
                        }
                    }
                }
        ...
        private AbstractDirectory GetDirectoryProviderObject()
        {
            try
            {
                return CreateDirectoryProviderObject();
            }
            catch (FileNotFoundException ex)
            {
                throw new StorageProviderException(STORAGE_MISCONFIGURED_MESSAGE, ex);
            }
        }
            ...
            get
            {
                if (mFileProviderObject == null)
                {
                    lock (lockObject)
                    {
                        ...
                        {
                            // Create new directory provider object
                            mFileProviderObject = GetFileProviderObject();
                        }
                    }
                }
        ...
        private AbstractFile GetFileProviderObject()
        {
            try
            {
                return CreateFileProviderObject();
            }
            catch (FileNotFoundException ex)
            {
                throw new StorageProviderException(STORAGE_MISCONFIGURED_MESSAGE, ex);
        ...
        /// </remarks>
        internal static StorageProvider DefaultProvider
        {
            get
            {
