...
namespace CMS.IO
{
    ...
    public class VirtualPathHelper
    {
        ...
        /// </summary>
        public static bool VirtualPathProviderAllowedByDefault
        {
            get;
            set;
        } = true;
        ...
        {
            var relativePath = GetRelativePathFromVirtual(virtualPath, SystemContext.ApplicationPath);
            ...
            if (relativePath != null)
            {
                ...
                {
                    if (relativePath.StartsWith(registeredPaths.Keys[i], StringComparison.OrdinalIgnoreCase))
                    {
                        IVirtualFileObject virtFile = registeredPaths.Values[i].Invoke(relativePath);
                        if (virtFile != null)
                        {
                            return virtFile;
                        }
        ...
        public static bool RegisterVirtualPathProvider()
        {
            ...
            if (!CoreServices.AppSettings["CMSUseVirtualPathProvider"].ToBoolean(VirtualPathProviderAllowedByDefault))
            {
                return false;
            }
        ...
        /// <summary>
        /// Converts virtual path (e.g. /appPath/level1/file.ascx) to relative path (e.g. ~/level1/file.ascx).
        /// </summary>
        /// <param name="virtualPath">Virtual path</param>
        /// <param name="appPath">Application path</param>
        /// <returns>Relative path</returns>
        /// <exception cref="ArgumentNullException">Throws when <paramref name="virtualPath" /> or <paramref name="appPath" /> is <c>null</c>.</exception>
        internal static string GetRelativePathFromVirtual(string virtualPath, string appPath)
        {
            ...
            // Do not convert already relative path
            if (virtualPath.StartsWith("~", StringComparison.OrdinalIgnoreCase))
            {
                return virtualPath;
            }

            virtualPath = FixVirtualPathSlashes(virtualPath);

            var virtualPathForCompare = EnsureTrailingSlash(virtualPath);
            var appPathForCompare = EnsureTrailingSlash(appPath);

            // If virtualPath does not start with applicationPath return null
            if (!virtualPathForCompare.StartsWith(appPathForCompare, StringComparison.OrdinalIgnoreCase))
            {
                return null;
            }

            var relativePath = virtualPath.Substring(virtualPath.IndexOf(appPath, StringComparison.OrdinalIgnoreCase) + appPath.Length);
        ...
        /// <summary>
        /// Ensure trailing slash at the end of path
        /// </summary>
        ...
        /// <summary>
        /// Normalize slashes and remove slash duplicates
        /// </summary>
        private static string FixVirtualPathSlashes(string virtualPath)
        {
            ...
            // Get rid of slash duplicates "//////" -> "/"
            int doubleSlashesIndex = 0;
            while (true)
            {
                doubleSlashesIndex = virtualPath.IndexOf("//", doubleSlashesIndex, StringComparison.Ordinal);

                if (doubleSlashesIndex < 0)
                {
                    break;
                }

                virtualPath = virtualPath.Remove(doubleSlashesIndex, 1);
            }
