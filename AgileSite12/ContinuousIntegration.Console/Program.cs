...
#pragma warning restore BH1014 // Do not use System.IO
                    ...
                    if (!FileSystemRepositoryHelper.IsRepositoryInitialized(webAppPhysicalPath))
                    {
                        throw new InvalidOperationException(@"The Continuous Integration repository is either not initialized or in an incorrect location on the file system. 
Ensure Continuous Integration is enabled and that the repository is located at the default 'CMS\App_Data\CIRepository' path or in a correctly created and registered custom location. 
You can enable Continuous Integration by selecting 'Enable continuous integration' in the 'Continuous integration' application in the Kentico instance's Administration interface.
");
                    }
