            ...
            if (Directory.Exists(directoryPath))
            {
                ...
                if (filesInfos != null)
                {
                    ...
                    foreach (FileInfo file in filesInfos)
                    {
                        ...
                        if (!synchronization)
                        {
                            WebFarmHelper.CreateIOTask(new DeleteForumAttachmentWebFarmTask
                            {
                                SiteName = siteName,
        ...
        public static string SaveAttachmentFileToDisk(string siteName, string guid, string fileName, string fileExtension, BinaryData fileData, bool synchronization)
        {
            if (!String.IsNullOrEmpty(fileName))
            {
                ...
                if (fileData != null)
                {
                    ...
                    if (!synchronization)
                    {
                        WebFarmHelper.CreateIOTask(new UpdateForumAttachmentWebFarmTask
                        {
                            SiteName = siteName,
            ...
            if (info != null)
            {
                ...
                if (si != null)
                {
                    ...
                    using (var tr = BeginTransaction())
                    {
                        ...
                        {
                            // Log web farm task
                            WebFarmHelper.CreateTask(new UpdateForumAttachmentWebFarmTask
                            {
                                SiteName = siteName,
                                FileGuid = info.AttachmentGUID,
                                FileName = guid,
                                FileExtension = info.AttachmentFileExtension
                            });
                        }
