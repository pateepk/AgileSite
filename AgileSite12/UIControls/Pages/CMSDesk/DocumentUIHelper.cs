...
namespace CMS.UIControls
{
    ...
    {
        /// <summary>
        /// Name of the Page element for MVC site.
        /// </summary>
        internal const string MVC_PAGE_ELEMENT_NAME = "pagemvc";


        /// <summary>
        /// Name of the Page element for standard pages.
        /// </summary>
        internal const string PAGE_ELEMENT_NAME = "page";


        /// <summary>
        /// Name of the Form element for standard pages or Content tab for content-only pages.
        /// </summary>
        internal const string FORM_ELEMENT_NAME = "editform";
        ...
        /// </summary>
        private static ISet<string> ContentOnlyHiddenUIElements => mContentOnlyHiddenUIElements ?? (mContentOnlyHiddenUIElements = GetHiddenContentOnlyElements());
        ...
        public static string GetUIElementDisplayName(UIElementInfo element, TreeNode node)
        {
            ...
            if ((node != null) && (node.NodeIsContentOnly || node.Site.SiteIsContentOnly))
            {
                ...
                switch (element.ElementName?.ToLowerInvariant())
                {
                    ...
                    case MVC_PAGE_ELEMENT_NAME:
                        elementDisplayName = ResHelper.GetString("content.ui.page");
                        break;
                }
            }
            ...
            switch (settings.Action?.ToLowerInvariant())
            {
                ...
                case "notallowed":
                    {
                        ...
                        result = URLHelper.AddParameterToUrl(result, "action", QueryHelper.GetString("subaction", String.Empty).ToLowerInvariant());
                    }
                    break;
                ...
                default:
                    if (!String.IsNullOrEmpty(settings.Action))
                    {
                        ...
                        if (string.Equals(settings.Action, "edit", StringComparison.Ordinal))
                        {
        ...
        public static string GetPageTabElementName(TreeNode node)
        {
            if (node == null)
            {
            ...
            var isMvcPage = node.NodeIsContentOnly && node.Site.SiteIsContentOnly;

            return isMvcPage ? MVC_PAGE_ELEMENT_NAME : PAGE_ELEMENT_NAME;
        }
        ...
        public static string GetDocumentPageUrl(UIPageURLSettings settings, Func<UIPageURLSettings, string> getActionUrl)
        {
            try
            {
                ...
                settings.Mode = settings.Mode?.ToLowerInvariant();
                ...
                {
                    case "edit":
                        elementName = GetPageTabElementName(settings.Node);
                        break;
                        ...
                        case "preview":
                            {
                                ...
                                if (String.IsNullOrEmpty(url))
                                {
                                    if (settings.Node.NodeIsContentOnly)
                                    {
                                        ...
                                        url = settings.Node.GetPreviewLink(MembershipContext.AuthenticatedUser.UserName, readonlyMode: false, embededInAdministration: true);
                                    }
                                    else
                                    {
        ...
        public static DialogConfiguration GetDocumentDialogConfig(string action, string culture)
        {
            return new DialogConfiguration
            {
                ...
                CustomFormatCode = action?.ToLowerInvariant(),
                Culture = culture
            };
        }
        ...
        public static string GetPageNotAvailable(string reason, bool showLink, string documentName)
        {
            ...
            url = URLHelper.AddParameterToUrl(url, "showlink", showLink.ToString().ToLowerInvariant());
            if (reason != null)
            {
                url = URLHelper.AddParameterToUrl(url, "reason", reason.ToLowerInvariant());
            }
        ...
        private static bool CheckValidationVisibility(UIPageURLSettings settings)
        {
            ...
            return
                ...
                && !validationExcludedClassNames.Contains(";" + settings.Node.NodeClassName?.ToLowerInvariant() + ";")
        ...
        public static string TransformToCompareUrl(string url)
        {
            ...
            parameters.Add(VirtualContext.PARAM_CULTURE, PortalUIHelper.SplitModeCultureCode);
            parameters.Add(VirtualContext.PARAM_USERNAME, MembershipContext.AuthenticatedUser.UserName);
            ...
            url = URLHelper.RemoveParameterFromUrl(url, VirtualContext.PARAM_CULTURE);
        ...
        public static bool IsElementHiddenForNode(UIElementInfo element, TreeNode node)
        {
            ...
            var nodeIsContentOnly = node.NodeIsContentOnly;
            var siteIsContentOnly = node.Site.SiteIsContentOnly;
            var displayPageTab = DataClassInfoProvider.GetDataClassInfo(node.NodeClassName).ClassIsMenuItemType;

            // Show MVC Page tab for content-only item on content-only site
            if (nodeIsContentOnly && siteIsContentOnly && IsMvcPageElement(name) && displayPageTab)
            {
                return false;
            }

            // Show Page tab for content-only item on portal engine site
            if (nodeIsContentOnly && !siteIsContentOnly && IsPageElement(name) && displayPageTab)
            {
                return false;
            }

            // Check blacklist in case of content-only node or content-only site
            if (nodeIsContentOnly || siteIsContentOnly)
            {
                return ContentOnlyHiddenUIElements.Contains(name);
            }

            // MVC Page tab is not visible for standard page on standard site
            return IsMvcPageElement(name);
        }


        private static bool IsMvcPageElement(string name)
        {
            return string.Equals(name, MVC_PAGE_ELEMENT_NAME, StringComparison.InvariantCultureIgnoreCase);
        }


        private static bool IsPageElement(string name)
        {
            return string.Equals(name, PAGE_ELEMENT_NAME, StringComparison.InvariantCultureIgnoreCase);
        }
        ...
        public static string GetDocumentMarks(Page page, string siteName, string preferredCultureCode, WorkflowStepTypeEnum stepType, IDataContainer container, bool renderIconEnvelope = false)
        {
            if ((page != null) && (container != null))
            {
                ...
                {
                    documentTranslated = String.Equals(preferredCultureCode, ValidationHelper.GetString(container.GetValue("DocumentCulture"), String.Empty), StringComparison.OrdinalIgnoreCase);
                }
