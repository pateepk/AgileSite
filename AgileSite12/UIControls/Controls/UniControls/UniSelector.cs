  ...
  if (window.US_Changed != null) {
    US_Changed();
        ...
        private bool GetSelectionWhereCondition(SelectCondition condition, string itemIds, out string where)
        {
            ...
            {
                string controlWhere = !string.IsNullOrEmpty(ListingWhereCondition) ? ListingWhereCondition : WhereCondition;
                where = SqlHelper.AddWhereCondition(where, ContextResolver.ResolveMacros(controlWhere));
            }
        ...
        protected ListItem NewListItem(DataRow dr, string securityPurpose)
        {
            ...
            // Create new item
            ListItem newItem = NewListItem(itemname, dr[ReturnColumnName].ToString(), securityPurpose);
            if (newItem != null)
            {
        ...
        /// <param name="value">Value.</param>
        /// <param name="securityPurpose">Prevents the control values from being maliciously used in a different control. A suitable value can be the control's <see cref="Control.ClientID"/>.</param>
        protected ListItem NewListItem(string text, string value, string securityPurpose)
        {
            ...
            newItem.Attributes.Add(SpecialFieldsDefinition.DATA_HASH_ATTRIBUTE, ValidationHelper.GetHashString(newItem.Value, new HashSettings(securityPurpose)));
        ...
        /// </summary>
        /// <param name="securityPurpose">Prevents the control values from being maliciously used in a different control. A suitable value can be the control's <see cref="Control.ClientID"/>.</param>
        protected void LoadSpecialFields(string securityPurpose)
        {
            ...
            try
            {
                SpecialFields.FillItems(DropDownSingleSelect.Items, securityPurpose);
            }
