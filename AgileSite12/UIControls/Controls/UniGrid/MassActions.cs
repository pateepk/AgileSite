...
namespace CMS.UIControls
{
    ...
    public abstract class MassActions : CMSUserControl, ICallbackEventHandler, IExtensibleMassActions
    {
        ...
        public string GetCallbackResult()
        {
            ...
            {
                throw new InvalidOperationException($"Specified mass action ({mCallbackArguments.ActionCodeName}) was not found. Mass actions has to be added on every request (event postback).");
            }

            var additionalParameters = AdditionalParameters?.Value;
            ...
            return JsonConvert.SerializeObject(result, new JsonSerializerSettings { StringEscapeHandling = StringEscapeHandling.EscapeHtml });
        }
