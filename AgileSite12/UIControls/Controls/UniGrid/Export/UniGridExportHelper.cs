...
namespace CMS.UIControls
{
    ...
    public class UniGridExportHelper : DataExportHelper
    {
        ...
        protected override object GetExportedValue(DataRowView drv, string columnName, DataRow dataRow)
        {
            var value = base.GetExportedValue(drv, columnName, dataRow);

            if (value is Control control)
            {
                mRenderedControls.Add(new DataExportRenderedControl(control, dataRow, columnName));
                value = null;
            }
        ...
        protected override DataSet GetExportedData(DataSet dataSet, bool encode = true)
        {
            ...
            foreach (DataExportRenderedControl c in mRenderedControls)
            {
                ...
                if (ctrl is IOutputEncodingControl encodingControl)
                {
                    encodingControl.EncodeOutput = false;
                }
