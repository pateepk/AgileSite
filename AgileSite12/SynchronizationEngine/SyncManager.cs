    ...
    public class SyncManager : AbstractManager<SyncManager>, ISyncManager
    {
        ...
        public TreeProvider TreeProvider
        {
            get
            {
                ...
                {
                    mTreeProvider = new TreeProvider(AdministratorUser)
                    {
                        GenerateNewGuid = false,
                        UseCustomHandlers = UseTreeCustomHandlers,
                        UseAutomaticOrdering = UseAutomaticOrdering,
                        UpdateUser = false,
        ...
        protected virtual UpdateResultEnum UpdateObject(ref GeneralizedInfo infoObj, DataSet ds, TranslationHelper th, bool updateChildren, DataSet binaryData, bool addToSite, List<GeneralizedInfo> postProcessList)
        {
            using (var context = new CMSActionContext())
            {
                ...
                UpdateMetaFiles(infoObj, ds, th, originalId);
        ...
        private void UpdateMetaFiles(GeneralizedInfo infoObj, DataSet ds, TranslationHelper th, int originalId)
        {
            var ti = infoObj.TypeInfo;
            if (ti.HasMetaFiles)
            {
                DataTable filesDT = ds.Tables["CMS_MetaFile"];
                var translationHelper = ProceedWithTranslations(th) ? th : null;
                MetaFileInfoProvider.UpdateMetaFiles(infoObj, filesDT, originalId, translationHelper, null, LogTasks);
            }
        }
        ...
        protected virtual void UpdateDocument(DataSet documentDS, string safeClassName, bool processChildren)
        {
            ...
            foreach (DataRow nodeRow in documentTable.Rows)
            {
                ...
                if (processChildren)
                {
                    ...
                    UpdateRelationships(node, relationshipsTable, th);
                    ...
                    UpdateAlternativeUrls(documentDS, node, th);
        ...
        protected virtual void PublishDocument(DataSet documentDS, string safeClassName)
        {
            ...
            foreach (DataRow nodeRow in documentTable.Rows)
            {
                ...
                foreach (DataRow versionRow in versions)
                {
                    ...
                    if (ProceedWithTranslations(th) && ColumnsTranslationEvents.TranslateColumns.IsBound)
                    {
                        ColumnsTranslationEvents.TranslateColumns.StartEvent(th, VersionHistoryInfo.OBJECT_TYPE, version);
                    }
                    ...
                    if (attachmentHistoryTable != null)
                    {
                        ...
                        foreach (DataRow mainAttachment in mainAttachments)
                        {
                            ...
                            if (ProceedWithTranslations(th) && ColumnsTranslationEvents.TranslateColumns.IsBound)
                            {
                                ColumnsTranslationEvents.TranslateColumns.StartEvent(th, AttachmentHistoryInfo.OBJECT_TYPE, version);
                            }
                        ...
                        foreach (DataRow attachmentVariant in attachmentVariants)
                        {
                            ...
                            if (newParentId != 0)
                            {
                                ...
                                if (ProceedWithTranslations(th) && ColumnsTranslationEvents.TranslateColumns.IsBound)
                                {
                                    ColumnsTranslationEvents.TranslateColumns.StartEvent(th, AttachmentHistoryInfo.OBJECT_TYPE, history);
                                }
        ...
        /// <param name="th">Translation helper</param>
        protected virtual void UpdateRelationships(TreeNode node, DataTable relationshipsTable, TranslationHelper th)
        {
            ...
            foreach (DataRow relationshipRow in relationshipsTable.Rows)
            {
                ...
                if (currentRelationship != null)
                {
                    ...
                    if (update)
                    {
                        ...
                        if (ProceedWithTranslations(th) && ColumnsTranslationEvents.TranslateColumns.IsBound)
                        {
                            ColumnsTranslationEvents.TranslateColumns.StartEvent(th, RelationshipInfo.OBJECT_TYPE, relation);
                        }
            ...
            if (categoriesTable != null)
            {
                ...
                foreach (DataRow categoryRow in categoriesTable.Rows)
                {
                    ...
                    if (categoryId != 0)
                    {
                        ...
                        if (ProceedWithTranslations(th) && ColumnsTranslationEvents.TranslateColumns.IsBound)
                        {
                            ColumnsTranslationEvents.TranslateColumns.StartEvent(th, DocumentCategoryInfo.OBJECT_TYPE, infoObj);
                        }
        ...
        protected virtual void UpdateAliases(TreeNode node, DataTable aliasesTable, TranslationHelper th)
        {
            ...
            foreach (DataRow aliasRow in aliasesTable.Rows)
            {
                ...
                if (ProceedWithTranslations(th) && ColumnsTranslationEvents.TranslateColumns.IsBound)
                {
                    ColumnsTranslationEvents.TranslateColumns.StartEvent(th, DocumentAliasInfo.OBJECT_TYPE, aliasInfo);
                }
        ...
        /// </summary>
        protected virtual void UpdateAlternativeUrls(DataSet documentDS, TreeNode node, TranslationHelper th)
        {
            var alternativeUrlsClass = DataClassInfoProvider.GetDataClassInfo(AlternativeUrlInfo.OBJECT_TYPE);
            var alternativeUrlsTable = documentDS.Tables[alternativeUrlsClass.ClassTableName];

            if (alternativeUrlsTable == null)
            {
                return;
            }

            var alternativeUrlsManager = new AlternativeUrlSynchronizationManager(this, node, th);
            var alternativeUrls = alternativeUrlsTable.Rows.OfType<DataRow>().Select(row => new AlternativeUrlInfo(row)).ToList();

            alternativeUrlsManager.Synchronize(alternativeUrls);
        }
        ...
        private static void TranslateTemplateColumn(TreeNode node, TranslationHelper th, string colName)
        {
            ...
            if (templateId > 0)
            {
                ...
                if (dr != null)
                {
                    ...
                    // Get correct site ID
                    int templateSiteId = string.IsNullOrEmpty(templateSiteName) ? 0 : node.OriginalNodeSiteID;
        ...
        protected virtual void UpdateACLs(TreeNode node, DataSet documentDS, TranslationHelper th)
        {
            ...
            if (!DataHelper.DataSourceIsEmpty(aclItemTable))
            {
                ...
                foreach (DataRow dr in aclItemTable.Rows)
                {
                    ...
                    if (ProceedWithTranslations(th) && ColumnsTranslationEvents.TranslateColumns.IsBound)
                    {
                        ColumnsTranslationEvents.TranslateColumns.StartEvent(th, AclItemInfo.OBJECT_TYPE, originalACLItem);
                    }
