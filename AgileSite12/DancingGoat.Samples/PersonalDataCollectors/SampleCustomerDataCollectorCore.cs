...
namespace CMS.DancingGoat.Samples
{
    ...
    internal class SampleCustomerDataCollectorCore
    {
        ...
        private readonly List<CollectedColumn> addressInfoColumns = new List<CollectedColumn> {
        ...
        private readonly List<CollectedColumn> orderAddressInfoColumns = new List<CollectedColumn> {
            ...
            new CollectedColumn("AddressGUID", "GUID" ),
            new CollectedColumn("AddressLastModified", "Last modified"),
            new CollectedColumn("AddressOrderID", "")
        };
        ...
        {
            public BaseInfo Address { get; set; }
        ...
        private void WriteOrders(int customerID)
        {
            ...
            var orderBillingAddresses = GetOrderAddresses(orders, AddressType.Billing);
            var orderShippingAddresses = GetOrderAddresses(orders, AddressType.Shipping);
            var orderCompanyAddresses = GetOrderAddresses(orders, AddressType.Company);

            foreach (var orderInfo in orders)
            {
                WriteOrder(orderInfo, orderBillingAddresses, orderShippingAddresses, orderCompanyAddresses);
            }
        }


        private Dictionary<int, FullAddressInfo> GetOrderAddresses(IEnumerable<OrderInfo> orders, AddressType addressType)
        {
            ...
            var orderAddresses = OrderAddressInfoProvider.GetAddresses()
                                                         ...
                                                         .WhereIn("AddressOrderID", orders.Select(i => i.OrderID).ToArray())
                                                         .WhereEquals("AddressType", (int)addressType)
            ...
            var orderAddressesDict = new Dictionary<int, FullAddressInfo>();
            orderAddresses.ForEachRow(row =>
            {
                var address = new OrderAddressInfo(row);
                ...
                orderAddressesDict.Add(address.AddressOrderID, result);
            });

            return orderAddressesDict;
        }
        ...
        /// </summary>
        private void WriteOrder(OrderInfo orderInfo, IReadOnlyDictionary<int, FullAddressInfo> orderBillingAddresses, 
                                IReadOnlyDictionary<int, FullAddressInfo> orderShippingAddresses, IReadOnlyDictionary<int, FullAddressInfo> orderCompanyAddresses)
        {
            FullAddressInfo billingAddressInfo, shippingAddressInfo, companyAddressInfo;
            orderBillingAddresses.TryGetValue(orderInfo.OrderID, out billingAddressInfo);
            orderShippingAddresses.TryGetValue(orderInfo.OrderID, out shippingAddressInfo);
            orderCompanyAddresses.TryGetValue(orderInfo.OrderID, out companyAddressInfo);
        ...
        private void WriteAddress(FullAddressInfo addressInfo, string addressTypeName, string addressTypeDisplayName)
        {
            ...
            writer.WriteBaseInfo(addressInfo.Address, addressInfoColumns);
        ...
        private void WriteShoppingCarts(int userID)
        {
            ...
            var shoppingCartsAddresses = AddressInfoProvider.GetAddresses()
                .Columns(addressInfoColumns.Union(countryInfoColumns).Union(stateInfoColumns).Select(c => c.Name))
            ...
            shoppingCartsAddresses.ForEachRow(row =>
            {
