...
",
        ...
        private static void InitializeMasterDatabase(DatabaseProperties database)
        {
            ...
            try
            {
                var settings = new DatabaseInstallationSettings
                {
                    ...
                    ScriptsFolder = scriptPath,
                    Logger = HandleDatabaseInstallationMessage,
                    ApplyHotfix = false
                };

                SettingsHelper.AppSettings[ValidationHelper.APP_SETTINGS_HASH_STRING_SALT] = DEFAULT_HASH_STRING_SALT;
                SqlInstallationHelper.InstallDatabase(settings);
            }
            finally
            {
                SqlInstallationHelper.AfterDataGet -= UpdateMacroSignatures;
        ...
        private void InitSharedDatabase(string suffix = "SHARED")
        {
        ...
        public void ExecuteWithExtraDatabase(string dbName, Action action)
        {
            if (!ExtraDatabaseExists(dbName))
            {
                Assert.Fail("Extra database '" + dbName + "' not found. You must add attribute [ExtraDatabase(\"" + dbName + "\")] to your test fixture or call EnsureExtraDatabase(\"" + dbName + "\") before accessing it.");
            }
