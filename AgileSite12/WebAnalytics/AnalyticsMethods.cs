        ...
        public static void LogAnalytics(IPageInfo currentPage, string siteName)
        {
            if (currentPage != null)
            {
                ...
                if (!String.IsNullOrEmpty(userPageString))
                {
                    ...
                    {
                        Service.Resolve<IEventLogService>().LogException("Web analytics", "DeserializeUserPage", ex, new LoggingPolicy(TimeSpan.FromMinutes(20)));
                    }
                }
                ...
                CookieHelper.SetValue("CMSUserPage", JsonConvert.SerializeObject(userPage, new JsonSerializerSettings { StringEscapeHandling = StringEscapeHandling.EscapeHtml }), now.AddMinutes(20));
            }
        }
