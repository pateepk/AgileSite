...
namespace CMS.WebAnalytics
{
    ...
    public class HitLogProvider
    {
        ...
        public static void LogHit(string codeName, string siteName, string culture, string objectName, int objectId, int count, double value)
        {
            ...
            if (!LogDirectly && LogDirectory == null)
            {
                return;
            }
            ...
            try
            {
                ...
                if (LogDirectly)
                {
                    LogHitDirectly(codeName, siteName, culture, objectName, objectId, count, value);

                    return;
                }

                // Transform semicolons in object name for serialization
        ...
        public static void LogConversions(string siteName, string culture, string objectName, int objectId, int count, double value)
        {
            ...
            if (SearchCrawler.IsCrawlerRequest() || SiteInfoProvider.GetSiteInfo(siteName).SiteIsContentOnly)
            {
                return;
            }
        ...
        /// <summary>
        /// Log hit directly to database, using <see cref="HitLogDirectProcessor"/>.
        /// </summary>
        /// <param name="codeName">Hit codename</param>
        /// <param name="siteName">Site name</param>
        /// <param name="culture">Site culture code</param>
        /// <param name="objectName">Object name</param>
        /// <param name="objectId">Object ID</param>
        /// <param name="count">Hit count</param>
        /// <param name="value">Hit value</param>
        private static void LogHitDirectly(string codeName, string siteName, string culture, string objectName, int objectId, int count, double value)
        {
            ...
            HitLogDirectProcessor.Current.Enqueue(logRecord);
        }
    ...
    internal class MemoryStorage
    {
        ...
        private static readonly object dirsLock = new object();
        ...
        private VirtualDirectory GetDirectory(DateTime dirDate)
        {
            ...
            VirtualDirectory dirValue = null;

            // Check whether directory exist
            lock (dirsLock)
            {
                // Create new directory if not exist
                if (!dirs.TryGetValue(dirName, out dirValue))
                {
                    dirValue = new VirtualDirectory(DirectoryPath);

                    // Add directory to the collection
                    dirs.Add(dirName, dirValue);
