...
namespace CMS.Core
{
    ...
    public class ModuleEntryManager
    {
        ...
        // Most modules are discovered during ModuleCollectionsInitialization, only a few are registered via RegisterModule().
        ...
        private static bool mModuleCollectionsInitialized;
        ...
        public static IEnumerable<ModuleInfo> Modules
        {
            get
            {
                if (!mModuleCollectionsInitialized)
                {
                    throw new InvalidOperationException("Application pre-initialization has to be executed before accessing this property.");
                }
        ...
        [Obsolete("Use AssemblyDiscoveryHelper.RegisterAdditionalAssemblies method instead.")]
        ...
        /// </summary>
        private static void ModuleCollectionsInitialization()
        {
            if (!mModuleCollectionsInitialized)
            {
                lock (mModulesLock)
                {
                    if (!mModuleCollectionsInitialized)
                    {
                        ...
                        mModuleCollectionsInitialized = true;
                    }
                }
            }
        }
        ...
        /// If the application life-cycle has already started the module initialization starts immediately
        /// to catch up (if called from multiple threads, the method does not return until the module has caught up the application life-cycle).
        /// </remarks>
        ...
        [Obsolete("Related public API is obsolete.")]
        ...
        {
            var entries = new ModuleDiscovery().GetModules().ToList();
        ...
        private static IEnumerable<ModuleEntry> GetModuleEntriesForInitialization()
        {
            // Select modules which are either non-installable or suitable for init
            var modules = Modules.Where(x => x.Module != null && (!x.Module.IsInstallable || x.Module.IsInstalled)).Select(x => x.Module);
        ...
        {
            ModuleCollectionsInitialization();

            TypeManager.PreInitializeTypes();

            var modules = Modules.Where(x => x.Module != null).Select(x => x.Module);
        ...
        internal static void Init()
        {
            ...
            var modules = GetModuleEntriesForInitialization();
            foreach (var module in modules)
            {
                module.Init();
            }
