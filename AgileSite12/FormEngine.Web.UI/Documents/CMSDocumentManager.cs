    ...
    public class CMSDocumentManager : CMSAbstractManager<DocumentManagerEventArgs, SimpleDocumentManagerEventArgs>, IPostBackEventHandler, ICMSDocumentManager
    {
                        ...
                        case FormModeEnum.InsertNewCultureVersion:
                            ...
                            if (SourceNode != null)
                            {
                                ...
                                if (SourceDocumentID > 0)
                                {
                                    ...
                                    CopyNodeDataSettings settings = new CopyNodeDataSettings(true, excludedColumns)
                                    {
                                        ResetChanges = true
                                    };
        ...
        public void RenderScripts()
        {
            ...
            if (RenderScript)
            {
                ...
                sb.Append("function CheckConsistency_", ClientID, "() { ", GetPostBackEventReference(null), "; } \n");
                if (IsActionAllowed(ComponentEvents.SAVE))
                {
                    sb.Append("function SaveDocument_", ClientID, "(createAnother) { if(createAnother) { ", GetSaveAnotherScript(), "}", GetSubmitScript(), GetPostBackEventReference(ComponentEvents.SAVE), "; } \n");
                }
                if (IsActionAllowed(DocumentComponentEvents.APPROVE))
                {
                    sb.Append("function Approve_", ClientID, "(stepId, comment) { SetComment_", ClientID, "(comment); SetParam_", ClientID, "(stepId);", GetPostBackEventReference(DocumentComponentEvents.APPROVE), "; } \n");
                    setStepId = true;
                }
                if (IsActionAllowed(DocumentComponentEvents.REJECT))
                {
                    sb.Append("function Reject_", ClientID, "(historyId, comment) { SetComment_", ClientID, "(comment); SetParam_", ClientID, "(historyId);", GetPostBackEventReference(DocumentComponentEvents.REJECT), "; } \n");
                    setStepId = true;
                }
                if (IsActionAllowed(DocumentComponentEvents.PUBLISH))
                {
                    sb.Append("function Publish_", ClientID, "(comment) { SetComment_", ClientID, "(comment); ", GetPostBackEventReference(DocumentComponentEvents.PUBLISH), "; } \n");
                }
                if (IsActionAllowed(DocumentComponentEvents.ARCHIVE))
                {
                    sb.Append("function Archive_", ClientID, "(stepId, comment) { SetComment_", ClientID, "(comment); SetParam_", ClientID, "(stepId);", GetPostBackEventReference(DocumentComponentEvents.ARCHIVE), "; } \n");
                    setStepId = true;
                }
                if (IsActionAllowed(DocumentComponentEvents.CHECKOUT))
                {
                    sb.Append("function CheckOut_", ClientID, "() { ", GetPostBackEventReference(DocumentComponentEvents.CHECKOUT), "; } \n");
                }
                if (IsActionAllowed(DocumentComponentEvents.CHECKIN))
                {
                    sb.Append("function CheckIn_", ClientID, "(comment) { SetComment_", ClientID, "(comment); ", GetPostBackEventReference(DocumentComponentEvents.CHECKIN), "; } \n");
                }
                if (IsActionAllowed(DocumentComponentEvents.UNDO_CHECKOUT))
                {
                    sb.Append("function UndoCheckOut_", ClientID, "() { if(!confirm(" + ScriptHelper.GetString(ResHelper.GetString("EditMenu.UndoCheckOutConfirmation", ResourceCulture)) + ")) return false; ", GetPostBackEventReference(DocumentComponentEvents.UNDO_CHECKOUT), "; } \n");
                }
        ...
        private string GetPostBackEventReference(string argument)
        {
            var original = ControlsHelper.GetPostBackEventReference(this, argument);
            if (OnGetActionScript != null)
            {
                return OnGetActionScript(this, new GetActionScriptEventArgs { ActionName = argument, OriginalScript = original });
            }
        ...
        protected override void OnPreRender(EventArgs e)
        {
            ...
            CheckDocumentConflictWithAlternativeUrls();
        }
            ...
            try
            {
                ...
                if (AutoCheck)
                {
                    ...
                    else
                    {
                        ...
                        {
                            // Check out the document
                            var step = VersionManager.CheckOut(Node, Node.IsPublished, true, true);
                            if (step != null)
                            {
                                // Check in the document
                                VersionManager.CheckIn(Node, null);
        ...
        /// <summary>
        /// Logs warning if URL of edited document is in conflict with some alternative URL.
        /// </summary>
        private void CheckDocumentConflictWithAlternativeUrls()
        {
            if (EditedDocument != null && EditedDocument.Site.SiteIsContentOnly && EditedDocument.NodeIsContentOnly &&
                SettingsKeyInfoProvider.GetBoolValue("CMSAlternativeUrlUIEnabled", EditedDocument.Site.SiteID))
            {
                var conflictingAlternativeUrl = AlternativeUrlHelper.GetConflictingAlternativeUrl(EditedDocument);

                if (conflictingAlternativeUrl != null)
                {
                    var altUrlDocument = DocumentHelper.GetDocument(conflictingAlternativeUrl.AlternativeUrlDocumentID, new TreeProvider());
                    
                    // Ensure script which allows redirecting to 'Alternative URLs' tab of conflicting page
                    ScriptHelper.RegisterEditScript(Page, false);

                    var editDocumentLink = $"<a onclick=\"EditDocument({altUrlDocument.NodeID}, 'Properties.AlternativeURLS', {ScriptHelper.GetString(altUrlDocument.DocumentCulture)})\">{AlternativeUrlHelper.GetDocumentIdentification(altUrlDocument.DocumentNamePath, altUrlDocument.DocumentCulture)}</a>";
                    AddWarning(ResHelper.GetStringFormat("alternativeurl.currentpageisinconflictwithalternativeurl", conflictingAlternativeUrl.AlternativeUrlUrl, editDocumentLink));
                }
            }
        }
