...
namespace CMS.MacroEngine
{
    ...
    internal class SystemMethods : MacroMethodContainer
    {
            ...
            if (parameters.Length > 1)
            {
                ...
                if (!string.IsNullOrEmpty(objectType))
                {
                    ...
                    if (info != null)
                    {
                        ...
                        if (info != null)
                        {
                            if (MacroSecurityProcessor.CheckSecurity(context, info, false))
                            {
        ...
        public static object GetObject(EvaluationContext context, params object[] parameters)
        {
            ...
            if (id > 0)
            {
                result = ProviderHelper.GetInfoById(objectType, id);
            }
                    ...
                    case 2:
                        {
                            ...
                            if (guid != Guid.Empty)
                            {
                                result = ProviderHelper.GetInfoByGuid(objectType, guid);
                                break;
                            }

                            // If it's not GUID, it must be code name.
                            result = ProviderHelper.GetInfoByName(objectType, GetStringParam(parameters[1]));
                            break;
                        }
                    ...
                    case 3:
                        {
                            ...
                            if (guid != Guid.Empty)
                            {
                                result = ProviderHelper.GetInfoByGuid(objectType, guid, siteId);
                                break;
                            }

                            // If it's not GUID, it must be code name.
                            result = ProviderHelper.GetInfoByName(objectType, GetStringParam(parameters[1]), siteId);
                            break;
                        }
                }
            }
        ...
        {
            return AuthenticationMode.IsFormsAuthentication();
        }
        ...
        {
            return AuthenticationMode.IsWindowsAuthentication();
        }
