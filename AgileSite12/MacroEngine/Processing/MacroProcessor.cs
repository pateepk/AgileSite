...
namespace CMS.MacroEngine
{
    ...
    public static class MacroProcessor
    {
        ...
        internal static string ProcessMacros(string text, string replacement, IMacroProcessingParameters parameters, OnProcessMacro processMacro, List<string> type = null, bool processOpenExpressions = true)
        {
            ...
            if (!String.IsNullOrEmpty(result))
            {
                ...
                while ((index >= 0) && (index < result.Length - 1))
                {
                    ...
                    {
                        int bracket = result.IndexOf(")", index + 1, StringComparison.Ordinal);
                    ...
                    if (valid)
                    {
                        ...
                        int endIndex = result.IndexOf(endBracket, index + len, StringComparison.Ordinal);
                        ...
                        if ((endIndex == -1) && (currentType == '&') && (result.IndexOf("amp;", startIndex, StringComparison.OrdinalIgnoreCase) == startIndex))
                        {
                            endIndex = result.IndexOf("&amp;}", index, StringComparison.OrdinalIgnoreCase);
                            len += "amp;".Length;
                        }
                        ...
                        if (endIndex >= 0)
                        {
                            ...
                            string exprWithoutParams = (currentType == '%' ? MacroSecurityProcessor.RemoveMacroSecurityParams(expr, out identityOption, out _) : expr);
                            string exprTrim = exprWithoutParams.Trim();
                            string resolved;

                            if (processOpenExpressions && exprTrim.EndsWith("{", StringComparison.Ordinal))
                            {
                                bool isElseExpr = exprTrim.StartsWith("}", StringComparison.Ordinal) && (currentType == openExprType) && exprTrim.Contains("else");
                                if (isElseExpr)
                                {
                            ...
                            else if (processOpenExpressions && exprTrim.StartsWith("}", StringComparison.Ordinal) && (currentType == openExprType))
                            {
                    ...
                    {
                        index = result.IndexOf("{", startIndex, StringComparison.Ordinal);
                    }
                    else
                    {
                        index = -1;
        ...
        {
            return RemoveParameter(expression, name, out _);
        }
        ...
        public static string RemoveParameter(string expression, string name, out string value)
        {
            ...
            {
                int indexOfQuotes = expression.LastIndexOf("\"", StringComparison.Ordinal);
                string param = String.Concat("|(", name, ")");
                int index = expression.LastIndexOf(param, StringComparison.OrdinalIgnoreCase);
                ...
                if (index > indexOfQuotes)
                {
                    ...
                    if (index >= 0)
                    {
                        var endIndex = expression.IndexOf(escaped ? "\\|" : "|", index + 1, StringComparison.Ordinal);
        ...
        {
            if (value.EndsWith($"{macroChar}}}", StringComparison.Ordinal))
            {
                value = value.Substring(0, value.Length - 2);
            }
            if (value.StartsWith($"{{{macroChar}", StringComparison.Ordinal))
            {
        ...
        public static bool IsLocalizationMacro(string text)
        {
            ...
            if (keyTrimmed.StartsWith("{$", StringComparison.Ordinal) && keyTrimmed.EndsWith("$}", StringComparison.Ordinal) && (keyTrimmed.Length > 4))
            {
