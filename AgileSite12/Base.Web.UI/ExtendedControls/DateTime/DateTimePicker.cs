...
namespace CMS.Base.Web.UI
{
    ...
    public class DateTimePicker : AbstractDateTimePicker, ICallbackEventHandler
    {
        ...
        public override string CultureCode
        {
            get
            {
                return UseCustomCalendar ? mCultureCode : System.Globalization.CultureInfo.CurrentUICulture.Name;
            }
            set
            {
                mCultureCode = value;
...
function SetDate(result, context) { 
    ...
    $cmsj(elem).trigger('change');
        ...
        public string GenerateNonCustomCalendarImageEvent()
        {
            ...
            return String.Format(" modalDialog('{0}/calendar.aspx?seldate=' + document.getElementById('{1}').value + '&controlid={1}&allowempty={2}&editTime={3}&UILang={4}{5}', 'calendar', 370, 300); return false;", 
                UrlResolver.ResolveUrl(SupportFolder), 
                DateTimeTextBox.ClientID, 
                AllowEmptyValue, 
                EditTime, 
        ...
        private DateTime GetDateTime(bool applyTimeZone)
        {
            ...
            else
            {
                try
                {
                    ...
                    {
                        IFormatProvider culture = CultureHelper.GetCultureInfo(TextBoxCultureCode);
        ...
        protected override string DateTimeToStringFormat(string format, System.Globalization.CultureInfo culture)
        {
            if (!UseCustomCalendar)
            {
                format = string.Empty;
