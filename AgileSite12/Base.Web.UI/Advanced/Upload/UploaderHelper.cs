...
namespace CMS.Base.Web.UI
{
    ...
    public class UploaderHelper
    {
        ...
        /// <summary>
        /// Hashing purpose for hash of Allowed extensions.
        /// </summary>
        public const string ALLOWED_EXTENSIONS_HASHING_PURPOSE = "MultiFileUploader - AllowedExtensions";

        /// <summary>
        /// Hashing purpose for hash of Addiotional parameters.
        /// </summary>
        public const string ADDIOTIONAL_PARAMETERS_HASHING_PURPOSE = "MultiFileUploader - AddiotionalParameters";

        /// <summary>
        /// Hashing purpose for hash of Media library arguments.
        /// </summary>
        public const string MEDIA_LIBRARY_ARGS_HASHING_PURPOSE = "MultiFileUploader - MediaLibraryArgs";

        /// <summary>
        /// Hashing purpose for hash of File arguments.
        /// </summary>
        public const string FILE_ARGS_HASHING_PURPOSE = "MultiFileUploader - FileArgs";

        /// <summary>
        /// Hashing purpose for hash of Meta file arguments.
        /// </summary>
        public const string META_FILE_ARGS_HASHING_PURPOSE = "MultiFileUploader - MetaFileArgs";

        /// <summary>
        /// Hashing purpose for hash of Forum arguments.
        /// </summary>
        public const string FORUM_ARGS_HASHING_PURPOSE = "MultiFileUploader - ForumArgs";

        /// <summary>
        /// Hashing purpose for hash of Attachment arguments.
        /// </summary>
        public const string ATTACHEMENT_ARGS_HASHING_PURPOSE = "MultiFileUploader - AttachementArgs";
        ...
        private readonly HttpContextBase mCtx;
        ...
        /// <summary>
        /// Creates a new instance of <see cref="UploaderHelper"/>.
        /// </summary>
        ...
        internal UploaderHelper(HttpContextBase context)
        {
            ...
            {
                throw new InvalidOperationException("The page parameters are not consistent, please try to reopen the page.");
            }
        ...
        public bool ProcessFile()
        {
            try
            {
                ...
                if (GetBytes)
                {
                    ...
                    mCtx.Response.Write(fileInfo.Exists ? fileInfo.Length.ToString() : "0");
                    mCtx.Response.Flush();
                    return false;
                }

                // Uploading mode
                using (var fileStream = StartByte > 0 ? File.Open(FilePath, FileMode.Append, FileAccess.Write) : File.Create(FilePath))
                {
                    CopyDataFromRequestToFileStream(mCtx.Request, fileStream);
                    ...
                    if (CurrentFileSize > UPLOAD_CHUNK_SIZE)
                    {
                        ...
                        if (fileStream is IMultiPartUploadStream multiPartStream)
                        {
        ...
        private static bool ValidArguments(string argument, string purpose, char separator)
        {
            ...
            // Validate hash
            if (ValidationHelper.ValidateHash(split[0], split[1], new HashSettings(purpose)))
            {
                return true;
            }
        ...
        /// </summary>
        private void IsExtensionAllowed()
        {
            // Load allowed extensions from settings
            if (String.IsNullOrEmpty(AllowedExtensions))
            {
                mAllowedExtensions = SettingsKeyInfoProvider.GetValue(IsMediaLibraryUpload ? "CMSMediaFileAllowedExtensions" : "CMSUploadExtensions", SiteContext.CurrentSiteName);
            }

            string extensions = $";{AllowedExtensions?.ToLowerInvariant()};";
            string extension = Extension.TrimStart('.').ToLowerInvariant();
        ...
        private void CopyDataFromRequestToFileStream(HttpRequestBase request, FileStream fileStream)
        {
            SystemIO.Stream inputStream;
        ...
        private void UseMultiPartUpload(IMultiPartUploadStream stream, MultiPartUploadData multipartUploadData)
        {
            ...
            var identifiers = stream.UploadStreamContentAsMultiPart(multipartUploadData.UploadSessionID, multipartUploadData.PartNumber).ToList();
        ...
        private void AbortCurrentMultiPartUpload()
        {
            if (CurrentFileSize > UPLOAD_CHUNK_SIZE)
            {
                ...
                {
                    if (stream is IMultiPartUploadStream multiPartStream)
                    {
                        var multipartUploadData = JsonConvert.DeserializeObject<MultiPartUploadData>(mCtx.Request.Form.Get(0));
