...
namespace CMS.Membership
{
    ...
    public class UserInfoProvider : AbstractInfoProvider<UserInfo, UserInfoProvider>
    {
        ...
        private const string CLEAR_CACHED_USERS_COUNT_ACTION_NAME = "ClearCachedUsersCount";
        ...
        public static UserInfo AdministratorUser
        {
            get
            {
                ...
                return CacheHelper.Cache(GetAdministratorUser,
                    ...
                    {
                        GetCacheDependency = () => CacheHelper.GetCacheDependency(new[] { "cms.user|all", "cms.settingskey|cmsdefaultuserid" })
                    }
                );
            }
        }
...
#pragma warning restore BH2000 // Method 'WhereLike()' or 'WhereNotLike()' should not be used used.
        ...
        /// </summary>
        public static void ClearLicenseValues(bool logTasks = true)
        {
            LicEditors.Clear();
            ...
            if (logTasks)
            {
                ProviderObject.CreateWebFarmTask(CLEAR_CACHED_USERS_COUNT_ACTION_NAME, null);
            }
        }
        ...
        #region "Passwords & Enabled state"

        /// <summary>
        /// Sets the enabled state for the specified user.
        /// Toggling a user from enabled state to disabled also generates a new <see cref="UserInfo.UserSecurityStamp"/> for the user.
        /// </summary>
        /// <param name="userInfo">User whose enabled state is to be set.</param>
        /// <param name="enabled">Enabled state to be set.</param>
        /// <exception cref="ArgumentNullException">Thrown when <paramref name="userInfo"/> is null.</exception>
        public static void SetEnabled(UserInfo userInfo, bool enabled)
        {
            if (userInfo == null)
            {
            ...
            {
                userInfo.UserSecurityStamp = GenerateSecurityStamp();
            }
        ...
        public static bool IsUserPasswordDifferent(UserInfo ui, string password)
        {
            ...
            if (ui.UserIsDomain && AuthenticationMode.IsMixedAuthentication())
            {
        ...
        /// <summary>
        /// Checks given <paramref name="user"/> for type and logs an event if instance is type of <see cref="CurrentUserInfo"/>.
        /// </summary>
        private static void CheckInstanceBeforeSet(UserInfo user)
        {
            if (user is CurrentUserInfo && SystemContext.DiagnosticLogging)
            {
                var eventDescription = $"CurrentUserInfo cannot be set directly as the incorrect instance is cached in hashtables afterwards, only UserInfo objects can be set via provider. Stack trace: {new StackTrace()}";

                EventLogProvider.LogEvent(EventType.ERROR, "UserInfoProvider", "INVALIDSET", eventDescription);
            }
        }
        ...
        {
            if (String.Equals(actionName, CLEAR_CACHED_USERS_COUNT_ACTION_NAME, StringComparison.OrdinalIgnoreCase))
            {
                ClearLicenseValues(false);
            }
            else
            {
