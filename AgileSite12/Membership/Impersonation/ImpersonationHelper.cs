    ...
    internal static class ImpersonationHelper
    {
        ...
        internal static ImpersonationCookieData DecodeData(string encoded)
        {
            if (!string.IsNullOrEmpty(encoded))
            {
                ...
                {
                    EventLogDataHandler.SkipOriginalUserName = true;
                    EventLogProvider.LogEvent(EventType.WARNING, IMPERSONATION_KEY, "CookieDecode", e.Message, loggingPolicy: new LoggingPolicy(TimeSpan.FromMinutes(1)));
                    EventLogDataHandler.SkipOriginalUserName = false;
                }
            }
        ...
        /// </summary>
        /// <returns></returns>
        public static IHttpCookie GetCookie()
        {
