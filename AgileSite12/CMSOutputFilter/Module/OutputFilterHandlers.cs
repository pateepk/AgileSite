...
namespace CMS.OutputFilter
{
    ...
    internal class OutputFilterHandlers
    {
        ...
        {
            WebFarmHelper.RegisterTask<ClearCacheItemsWebFarmTask>();
            ...
            if (SystemContext.IsCMSRunningAsMainApplication)
            {
                ...
                CacheEvents.CacheItemRemoved.Execute += CacheItemRemoved;
                CacheEvents.ClearFullPageCache.Execute += (s, e) => OutputHelper.ClearFullPageCacheFiles();
            }
        ...
        /// <summary>
        /// Removes persistent cache item from filesystem when removed from cache.
        /// </summary>
        private static void CacheItemRemoved(object sender, CMSEventArgs<CacheItemRemovedEventArgs> e)
        {
            var eventArgs = e.Parameter;
            if (eventArgs.Reason == CMSCacheItemRemovedReason.Removed || eventArgs.Reason == CMSCacheItemRemovedReason.DependencyChanged)
            {
                // Handle the removal of persistent items
                var persistentItem = eventArgs.Value as IPersistentCacheItem;
                if (persistentItem != null && !Environment.HasShutdownStarted && (System.Web.Hosting.HostingEnvironment.ShutdownReason == System.Web.ApplicationShutdownReason.None))
                {
                    // Allow empty context for this operation - Cache callback isn't initiated from request
                    CMSThread.AllowEmptyContext();

                    OutputHelper.DeletePersistent(persistentItem.CacheKey, persistentItem.SiteName);
                }
            }
        }
