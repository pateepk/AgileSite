...
namespace CMS.WebDAV
{
    ...
    public static class WebDAVHelper
    {
        ...
        {
            if (!RequestStockHelper.Contains("WebDAVEnabled"))
            {
                // Get current domain
                string domain = null;
                ...
                bool webDAVEnabled = AuthenticationMode.IsWindowsAuthentication() && LicenseKeyInfoProvider.IsFeatureAvailable(domain, FeatureEnum.WebDav) && SettingsKeyInfoProvider.GetBoolValue(siteName + ".CMSEnableWebDAV");

                RequestStockHelper.Add("WebDAVEnabled", webDAVEnabled);

                return webDAVEnabled;
            }

            return (bool)RequestStockHelper.GetItem("WebDAVEnabled");
        }
