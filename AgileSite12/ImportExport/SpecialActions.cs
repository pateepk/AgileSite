...
namespace CMS.CMSImportExport
{
    ...
    public static class SpecialActions
    {
        ...
        public static void Init()
        {
            ...
            ImportExportEvents.ImportObject.Before += SkipProgressiveCachingSettingsKey;
        }
        ...
        private static void GetExistingObject_Before(object sender, ImportEventArgs e)
        {
            ...
            if (objectType == SiteInfo.OBJECT_TYPE)
            {
                ...
                // Preserve the following fields only if importing to an existing site
                if (settings.ExistingSite)
                {
                    siteObj.SiteIsContentOnly = settings.SiteIsContentOnly;
                    siteObj.SitePresentationURL = settings.SitePresentationUrl;
                    siteObj.Description = settings.SiteDescription;
                }
        ...
        private static void SkipProgressiveCachingSettingsKey(object sender, ImportEventArgs e)
        {
            if (e.Settings.IsLowerVersion("12.0") && IsProgressiveCachingSettingsKey(e.Object))
            {
                e.Cancel();
            }
        }
        ...
        private static bool IsProgressiveCachingSettingsKey(BaseInfo infoObj)
        {
            return (infoObj.TypeInfo.ObjectType == SettingsKeyInfo.OBJECT_TYPE) && String.Equals(infoObj.Generalized.ObjectCodeName, "CMSProgressiveCaching", StringComparison.OrdinalIgnoreCase);
        }
    }
}