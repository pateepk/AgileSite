    ...
    public static class ImportProvider
    {
                            ...
                            case SiteInfo.OBJECT_TYPE:
                                {
                                    ...
                                    settings.SiteDomain = ValidationHelper.GetString(infoObj.GetValue("SiteDomainName"), string.Empty);
                                    settings.SiteIsContentOnly = ValidationHelper.GetBoolean(infoObj.GetValue("SiteIsContentOnly"), false);
                                    settings.SiteDescription = ValidationHelper.GetString(infoObj.GetValue("SiteDescription"), null);
                                    settings.SitePresentationUrl = ValidationHelper.GetString(infoObj.GetValue("SitePresentationUrl"), null);
                                }
                                break;
                        }
                    }
        ...
        private static bool ProcessMetaFiles(SiteImportSettings settings, GeneralizedInfo infoObj, ImportParameters parameters, bool parentIsNew)
        {
            if (infoObj.TypeInfo.HasMetaFiles)
            {
                ...
                return MetaFileInfoProvider.UpdateMetaFiles(infoObj, filesDT, parameters.ObjectOriginalID, parameters.TranslationHelper, o => LoadBinaryData(settings, o, parameters), false, parentIsNew, settings.ImportOnlyNewObjects);
            }
        ...
        private static bool ProcessChildren(SiteImportSettings settings, GeneralizedInfo infoObj, ImportParameters parameters, bool parentIsNew)
        {
            ...
            using (var hChildren = ImportExportEvents.ImportChildren.StartEvent(eChildren))
            {
                ...
                {
                    var sortedChildObjectTypes = GetSortedChildObjectTypes(infoObj.TypeInfo.ChildObjectTypes);
                    foreach (string childType in sortedChildObjectTypes)
                    {
                                ...
                                if (childTypeInfo.ImportExportSettings.IncludeToExportParentDataSet != IncludeToParentEnum.None)
                                {
                                    ...
                                    if (!DataHelper.DataSourceIsEmpty(childDT))
                                    {
                                        ...
                                        var childWhere = new WhereCondition()
                                                         .WhereEquals(childTypeInfo.ParentIDColumn, parameters.ObjectOriginalID)
                                                         .Where(childTypeInfo.WhereCondition);
        ...
        private static IList<string> GetSortedChildObjectTypes(List<string> childObjectTypes)
        {
            ...
            var childObjectTypeAnalyzer = new ObjectTypeSequenceAnalyzer(new ImportChildObjectTypeFilter(), childObjectTypes);
            var orderedChildObjectTypes = childObjectTypeAnalyzer
                .GetSequence()
                .Select(objectType => objectType.ObjectType)
                .Distinct()
                .ToList();

            return orderedChildObjectTypes;
        }
        ...
        public static List<string> GetImportFilesList(string folder)
        {
            ...
            {
                path = CMSHttpContext.Current.Server.MapPath(folder);
            }
