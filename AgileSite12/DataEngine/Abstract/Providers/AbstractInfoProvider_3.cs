...
namespace CMS.DataEngine
{
        ...
        where TQuery : ObjectQueryBase<TQuery, TInfo>, new()
    {
        ...
        {
            return GetObjectQuery().WhereEquals(columnName, value).BinaryData(true).TopN(1).FirstOrDefault();
        }
        ...
        {
            WebFarmHelper.CreateTask(new ProcessObjectWebFarmTask
            {
                ObjectType = TypeInfo.ObjectType,
                ActionName = actionName,
                ObjectData = data,
                TaskTarget = $"{TypeInfo.ObjectType}|{actionName}|0"
            });
        }
        ...
        public virtual void CreateWebFarmTask(string actionName, string data, byte[] binary, string filePath)
        {
            ...
            WebFarmHelper.CreateIOTask(new ProcessObjectWebFarmTask
            {
                ObjectType = TypeInfo.ObjectType,
                ActionName = actionName,
                ObjectData = data,
                TaskFilePath = filePath,
                TaskBinaryData = binary,
                TaskTarget = $"{TypeInfo.ObjectType}|{actionName}|{binaryFlag}"
            });
        }
