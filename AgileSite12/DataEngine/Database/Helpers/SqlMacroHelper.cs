...
namespace CMS.DataEngine
{
    ...
    internal class SqlMacroHelper
    {
        ...
        /// </summary>
        public static CMSRegex NoOrderByRegExp = new CMSRegex("(?:\\s+order\\s+by\\s+(1|\\(\\s*1\\s*\\)))", true);

        /// <summary>
        /// Regular expression to match not needed GROUP BY expression.
        /// </summary>
        public static CMSRegex NoGroupByRegExp = new CMSRegex("(?:\\s+group\\s+by\\s+(##groupby##|\\(\\s*##groupby##\\s*\\)))", true);
        ...
        /// </summary>
        public static CMSRegex NoOutputRegExp = new CMSRegex("(?:\\s+output\\s+(##output##|\\(\\s*##output##\\s*\\)))", true);
