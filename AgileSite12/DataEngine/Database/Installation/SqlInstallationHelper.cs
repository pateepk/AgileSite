...
namespace CMS.DataEngine
{
    ...
    public static class SqlInstallationHelper
    {
        ...
        private static bool ChangeDefaultSchema(IDataConnection conn, string newSchema, Action<string, MessageTypeEnum> log)
        {
            try
            {
...
",
        ...
        public static bool InstallDatabase(DatabaseInstallationSettings installationSettings)
        {
            bool success = true;

            var connection = DataConnectionFactory.GetNativeConnection(installationSettings.ConnectionString);

            var dbSchema = EnsureDefaultSchema(installationSettings.Logger, connection, ref success);

            // Create database objects 
            if (success)
            {
                success = ProceedSQLScripts("defaultDBObjects.txt", connection, dbSchema, installationSettings, installationSettings.DatabaseObjectInstallationErrorMessage);
            }

            if (success)
            {
                success = ImportDefaultData(connection, installationSettings);
            }

            // Insert default data with SQL scripts
            if (success)
            {
                success = ProceedSQLScripts("defaultData.txt", connection, dbSchema, installationSettings, installationSettings.DataInstallationErrorMessage);
            }

            // Apply hotfix scripts
            if (success)
            {
                success = ApplyHotfix(connection, dbSchema, installationSettings);
            }

            // Dispose the zip storage provider to release memory and the file
            if (ZipStorageProvider.IsZipFolderPath(installationSettings.ScriptsFolder))
            {
                ZipStorageProvider.Dispose(installationSettings.ScriptsFolder);
            }

            // Set database version
            connection.ExecuteNonQuery("UPDATE [CMS_SettingsKey] SET KeyDefaultValue='" + CMSVersion.MainVersion + "', KeyValue='" + CMSVersion.MainVersion + "' WHERE KeyName='CMSDBVersion'", null, QueryTypeEnum.SQLQuery, false);
        ...
        private static string EnsureDefaultSchema(Action<string, MessageTypeEnum> log, IDataConnection conn, ref bool success)
        {
        ...
        private static void HandleError(string errorPrefix, string errorMessage, Action<string, MessageTypeEnum> log)
        {
            log?.Invoke($"{errorPrefix} {errorMessage}", MessageTypeEnum.Error);
        }


        private static void HandleError(string errorPrefix, Exception errorException, Action<string, MessageTypeEnum> log)
        {
        ...
        private static void ReportProgress(string filePath, string fileExtension, Action<string, MessageTypeEnum> log)
        {
        ...
        private static bool ProceedSQLScripts(string fileName, IDataConnection conn, string dbSchema, DatabaseInstallationSettings installationSettings, string defaultErrorMessage)
        {
            ...
            try
            {
                ...
                {
                    scriptsList = LoadScriptsFromFile(DirectoryHelper.CombinePath(installationSettings.ScriptsFolder, fileName));
                }
            }
            catch (Exception ex)
            {
                HandleError(defaultErrorMessage, ex, installationSettings.Logger);
                return false;
            }
            ...
            if (scriptsList != null)
            {
                ...
                {
                    ReportProgress(scriptName, SQL_EXTENSION, installationSettings.Logger);

                    // Execute script
                    Exception error = RunSQLScript(DirectoryHelper.CombinePath(installationSettings.ScriptsFolder, Path.EnsureBackslashes(scriptName)), conn, dbSchema);
                    if (error != null)
                    {
                        HandleError(defaultErrorMessage, error, installationSettings.Logger);
...
";
        ...
        /// <param name="connection">Connection to database.</param>
        /// <param name="installationSettings">Settings for installation</param>
        private static bool ImportDefaultData(IDataConnection connection, DatabaseInstallationSettings installationSettings)
        {
            if (connection == null)
            {
                HandleError(installationSettings.DataInstallationErrorMessage, "Connection must not be null", installationSettings.Logger);
                return false;
            }

            if (String.IsNullOrEmpty(installationSettings.ScriptsFolder) || !Directory.Exists(installationSettings.ScriptsFolder))
            {
                HandleError(installationSettings.DataInstallationErrorMessage, "Folder with default data must exist.", installationSettings.Logger);
                return false;
            }
            ...
            try
            {
                ...
                foreach (var tableName in orderedTables)
                {
                    ...
                    string filePath = Path.Combine(installationSettings.ScriptsFolder, shortFilePath);
                    ...
                    ReportProgress(shortFilePath, XML_EXTENSION, installationSettings.Logger);
            ...
            {
                HandleError(installationSettings.DataInstallationErrorMessage, ex, installationSettings.Logger);
                return false;
            }
        ...
        /// <param name="dbSchema">Name of the schema (all "dbo." and "[dbo]." occurrences are replaced by this schema)</param>
        /// <param name="installationSettings">Settings for installation</param>
        private static bool ApplyHotfix(IDataConnection connection, string dbSchema, DatabaseInstallationSettings installationSettings)
        {
            if (!installationSettings.ApplyHotfix)
            {
                return true;
            }

            const string HOTFIX_FOLDER_NAME = "Hotfix";
            string hotfixFolderPath = Path.Combine(installationSettings.ScriptsFolder, HOTFIX_FOLDER_NAME);
            ...
            if (Directory.Exists(hotfixFolderPath))
            {
                ...
                if (error == null && settings != null)
                {
                    ...
                    foreach (SQLScript sqlScript in settings.Scripts)
                    {
                        installationSettings.Logger(Path.Combine(HOTFIX_FOLDER_NAME, sqlScript.SQLFileName), MessageTypeEnum.Info);
                ...
                if (error != null)
                {
                    HandleError(installationSettings.DataInstallationErrorMessage, error, installationSettings.Logger);
                    return false;
                }
            }
