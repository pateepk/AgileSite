        ...
        where TInfo : DataClassInfoBase<TInfo>, new()
    {
        ...
        public virtual string ClassDefaultObjectType
        {
            get
            {
                ...
                {
                    return PredefinedObjectType.CUSTOM_TABLE_ITEM_PREFIX + ClassName.ToLowerInvariant();
                }

                if (ClassIsForm)
                {
                    return PredefinedObjectType.BIZFORM_ITEM_PREFIX + ClassName.ToLowerInvariant();
                }

                if (ClassIsDocumentType)
                {
                    return PredefinedObjectType.DOCUMENT_ITEM_PREFIX + ClassName.ToLowerInvariant();
                }
