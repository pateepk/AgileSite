...
namespace CMS.DataEngine
{
    ...
    public abstract class QueryColumnListBase : IEnumerable<IQueryColumn>
    {
        ...
        internal static readonly BracketInvariantStringComparer NameComparer = new BracketInvariantStringComparer();
        ...
        public void Remove(IQueryColumn col)
        {
            ...
            var columnToRemove = mColumnsList.Find(c => NameComparer.Equals(c.Name, col.Name));
            bool removed = mColumnsList.Remove(columnToRemove);

            // If column removed and no other column exists, remove that column from set
            if (removed && !mColumnsList.Any(it => NameComparer.Equals(it.Name, col.Name)))
            {
        ...
        {
            mColumnsList.RemoveAll(c => NameComparer.Equals(c.Name, col.Name));
            PresentColumns.Remove(col.Name);
        ...
        {
            for (var i = 0; i < mColumnsList.Count; i++)
            {
                // If the column name matches, replace the column
                if (NameComparer.Equals(mColumnsList[i].Name, originalName))
                {
        ...
        {
            var cols = new HashSet<string>(NameComparer);
