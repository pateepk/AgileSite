...
namespace CMS.DataEngine
{
    ...
    public class HierarchyHelper
    {
                    ...
                    case ExportFormatEnum.JSON:
                        {
                            Hashtable metadata = new Hashtable
                            {
                                ...
                                ["columns"] = (isProperties ? "" : colList),
                                ["properties"] = (isProperties ? colList : "")
                            };

                            string retval = JsonConvert.SerializeObject(metadata, new JsonSerializerSettings { StringEscapeHandling = StringEscapeHandling.EscapeHtml });
        ...
        public static TranslationHelper LoadObjectFromXML(OperationTypeEnum operation, BaseInfo obj, string xml, bool disconnectObject = true, bool updateOnlySpecifiedColumns = false, string cultureName = null, List<string> excludedColumns = null)
        {
            ...
            DataHelper.ReadDataSetFromXml(ds, xmlReader, null, null, out updatedColumns, cultureName);
        ...
        public static TranslationHelper LoadObjectFromJSON(BaseInfo obj, string json, bool disconnectObject = true, bool updateOnlySpecifiedColumns = false, List<string> excludedColumns = null)
        {
            ...
            JObject values = JsonConvert.DeserializeObject<JObject>(json);

            List<string> columnsToUpdate = LoadDataSetFromJSON(ds, ds.Tables[0].TableName, new JObject[] { values });
            if (!updateOnlySpecifiedColumns)
            {
        ...
        protected static List<string> LoadDataSetFromJSON(DataSet ds, string tableName, IEnumerable<JObject> values)
        {
            ...
            if (dt != null)
            {
                ...
                foreach (JObject obj in values)
                {
                    ...
                    foreach (var val in obj)
                    {
                        ...
                        if ((val.Value is JArray) && !isBinary)
                        {
                            result.AddRange(LoadDataSetFromJSON(ds, val.Key, ((JArray)val.Value).Cast<JObject>()));
                        }
                        else
                        {
                            if (val.Value != null)
                            {
                                if (isBinary)
                                {
                                    ...
                                    dr[columnName] = ((JArray)val.Value)
                            ...
                            result.Add(columnName.ToLowerInvariant());
                        }
                    }
                    dt.Rows.Add(dr);
                }
