...
namespace CMS.MediaLibrary
{
    ...
    public class MediaLibraryHelper
    {
        ...
        {
            string hiddenFolder = Service.Resolve<ISettingsService>()[siteName + ".CMSMediaFileHiddenFolder"];
        ...
        {
            return !String.IsNullOrEmpty(MediaLibraryCustomPath) ? MediaLibraryCustomPath : Service.Resolve<ISettingsService>()[siteName + ".CMSMediaLibrariesFolder"].TrimEnd('/');
        }
        ...
        internal static void CopyRecursiveInternal(int sourceLibraryID, int destinationLibraryID, DirectoryInfo dir, string destinationDirectory, string sourcePath, int libraryPathIndex, string startingPoint, bool copyDB, int userId, bool overwrite = false, CloneSettings cloneSettings = null, CloneResult cloneResult = null, Dictionary<Guid, Guid> fileGUIDs = null)
        {
            ...
            // Empty sourcePath indicates the beginning of the recursion
            if (copyDB && String.IsNullOrEmpty(sourcePath))
            {
                WebFarmHelper.CreateTask(new CloneFolderMediaWebFarmTask
                {
        ...
        {
            var file = MediaFileInfoProvider.GetMediaFileInfo(fileGuid, siteName);
            if (file == null)
            {
                return null;
            }

            var usePermanentURL = SettingsKeyInfoProvider.GetBoolValue(siteName + ".CMSMediaUsePermanentURLs");
            return GetMediaFileUrl(file, usePermanentURL);
        }
        ...
        public static string GetDirectUrl(MediaFileInfo mediaFile)
        {
            ...
            return GetMediaFileUrl(mediaFile, false);
        }
        ...
        /// <exception cref="System.ArgumentNullException"><paramref name="mediaFile" /> is null.</exception>
        /// <remarks>
        /// Generates various URL types depending on the media library location and 'Files friendly URL extension' setting in CMS:
        /// <list type="table">
        ///   <listheader>
        ///     <term>URL</term>
        ///     <description>Location</description>
        ...
        public static string GetPermanentUrl(MediaFileInfo mediaFile)
        {
            ...
            return GetMediaFileUrl(mediaFile, true);
        }


        private static string GetMediaFileUrl(MediaFileInfo file, bool permanent)
        {
            var library = MediaLibraryInfoProvider.GetMediaLibraryInfo(file.FileLibraryID);
            if (library == null)
            {
                return null;
            }

            var site = SiteInfoProvider.GetSiteInfo(file.FileSiteID);
            if (site == null)
            {
                return null;
            }

            var differentSite = !string.Equals(SiteContext.CurrentSiteName, site.SiteName, StringComparison.InvariantCultureIgnoreCase);

            if (permanent)
            {
                var fullFileName = AttachmentHelper.GetFullFileName(file.FileName, file.FileExtension);
                if (differentSite)
                {
                    // URL in format 'http://domainame/getmedia/123456-25245-45454-45455-5455555545/testfile.png'
                    return MediaFileURLProvider.GetMediaFileAbsoluteUrl(site.SiteName, file.FileGUID, fullFileName);
                }
                else
                {
                    // URL in format '/cms/getmedia/123456-25245-45454-45455-5455555545/testfile.png'
                    return MediaFileURLProvider.GetMediaFileUrl(file.FileGUID, fullFileName);
                }
            }
            ...
            if (differentSite)
            {
                ...
                return MediaFileURLProvider.GetMediaFileAbsoluteUrl(site.SiteName, library.LibraryFolder, file.FilePath);
            }

            // URL in format '/cms/EcommerceSite/media/testlibrary/folder1/testfile.png'
            return MediaFileURLProvider.GetMediaFileUrl(site.SiteName, library.LibraryFolder, file.FilePath);
        }
                    ...
                    try
                    {
                        ...
                        lock (locker)
                        {
                            ...
                            if (mli != null)
                            {
                                ...
                                th.RegisterRecord(mli);

                                if (mli.LibraryGroupID > 0)
                                {
                                    // Register group
                                    var group = ModuleCommands.CommunityGetGroupInfo(mli.LibraryGroupID);
                                    if (group != null)
                                    {
                                        th.RegisterRecord(group);
                                    }
                                }
