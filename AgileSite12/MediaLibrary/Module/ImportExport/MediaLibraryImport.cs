        ...
        static void ProcessFileOperation_After(object sender, ImportProcessFileOperationEventArgs e)
        {
            ...
            if (objectType == MediaLibraryInfo.OBJECT_TYPE)
            {
                if (operation.ParameterType == FileOperationParamaterTypeEnum.Webfarm)
                {
                    ...
                    {
                        var webFarmTask = JsonConvert.DeserializeObject<UpdateMediaWebFarmTask>(operation.Parameter);
                        webFarmTask.TaskFilePath = operation.DestinationPath;
                        webFarmTask.TaskBinaryData = file;

                        WebFarmHelper.CreateIOTask(webFarmTask);
                    }
                }
            }
        }
            ...
            if (!DataHelper.DataSourceIsEmpty(dataTable))
            {
                foreach (DataRow dr in dataTable.Rows)
                {
                    ...
                    if (libraryInfo != null)
                    {
                        ...
                        var webFarmTaskData = new UpdateMediaWebFarmTask
                        {
                            SiteName = settings.SiteName,
                            LibraryFolder = libraryInfo.LibraryFolder,
                            LibrarySubFolderPath = Path.GetDirectoryName(filePath),
                            FileName = fileName,
                            FileExtension = fileExtension,
                            FileGuid = ValidationHelper.GetGuid(fileGuid, Guid.Empty)
                        };
                        string objectParameter = JsonConvert.SerializeObject(webFarmTaskData);
