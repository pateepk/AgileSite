...
namespace CMS.Search.Lucene3
{
    ...
    internal class SearchDirectory : Directory
    {
        ...
        {
            // Files in zip folders must be opened in read-only mode
            var shareOption = ZipStorageProvider.IsZipFolderPath(directoryName) ? FileShare.Read : FileShare.ReadWrite;

            string fullPath = Path.Combine(directoryName, name);

            var stream = FileStream.New(fullPath, FileMode.Open, FileAccess.Read, shareOption);
