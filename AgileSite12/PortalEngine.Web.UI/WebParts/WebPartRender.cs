...
namespace CMS.PortalEngine.Web.UI
{
    ...
    public partial class CMSAbstractWebPart
    {
            ...
            if (!webPart.Removed)
            {
                ...
                if (!webPart.StandAlone && !webPart.IsVariant)
                {
                    ...
                    if (render)
                    {
                        ...
                        if (renderActive)
                        {
                            containerEnd = webPart.RenderContainerStart(writer, true, viewMode);
                        }
        ...
        {
            if (PartInfo != null
                && PagePlaceholder?.PageInfo != null
                && ParentZone?.ZoneInstance != null
                && PartInstance != null)
            {
        ...
        private bool GeneratePreviewAnchor()
        {
            ...
            if (previewObjectName != String.Empty)
            {
                ...
                if (previewObjectIdentifier != String.Empty)
                {
                    ...
                    switch (previewObjectName.ToLowerInvariant())
                    {
                        ...
                        case TransformationInfo.OBJECT_TYPE:
                            ...
                            {
                                if (string.Equals(PartInstance.GetValue(prop).ToString(), previewObjectIdentifier, StringComparison.InvariantCultureIgnoreCase))
                                {
                                    return true;
                                }
                            }
        ...
        private string RenderContainerStart(HtmlTextWriter writer, bool renderActive, ViewModeEnum viewMode)
        {
            // Get the outer style
            string innerStyle;
