...
public class SearchTaskListControlExtender : ControlExtender<UniGrid>
{
    ...
    public override void OnInit()
    {
        ...
        // We need to enable header action and display message after the action is executed.  
    ...
    private object RenderResult(DataRowView row)
    {
        ...
        return string.Format("<a target=\"_blank\" href=\"{0}\" onclick=\"modalDialog('{0}', 'taskresult', 700, 500); return false;\">{1}</a>", resultUrl, ResHelper.GetString("smartsearch.resultfailed"));
    }
    ...
    private object RenderRelatedObject(DataRowView row)
    {
        ...
        if (typeInfo != null)
        {
            transformation.NoDataTransformation = ResHelper.GetStringFormat("smartsearch.searchtaskrelatedobjectnotexist", typeInfo.GetNiceObjectTypeName(), objectId);
        }
    ...
    private void DisplayTaskRunningMessage()
    {
        ...
        if (IsIndexerThreadRunning)
        {
            ...
            string message = ResHelper.GetStringFormat("smartsearch.taskprocessingrunning",
