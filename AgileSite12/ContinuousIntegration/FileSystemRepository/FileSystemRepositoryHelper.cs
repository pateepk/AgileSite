...
namespace CMS.ContinuousIntegration
{
    ...
    public class FileSystemRepositoryHelper : AbstractHelper<FileSystemRepositoryHelper>
    {
        ...
        /// </summary>
        private static readonly HashAlgorithm HashAlgorithm = CryptoHelper.GetHashAlgorithm();
        ...
        public static bool IsRepositoryInitialized(string applicationRootPath)
        {
            return HelperObject.IsRepositoryInitializedInternal(applicationRootPath);
        }
        ...
        /// <param name="applicationRootPath">Path to the root of the application.</param>
        /// <returns>True if repository is initialized, otherwise false.</returns>
        protected virtual bool IsRepositoryInitializedInternal(string applicationRootPath)
        {
            var repositoryRootPath = FileSystemRepositoryConfigurationBuilder.GetRepositoryRootPath(applicationRootPath);
            var absoluteRepositoryConfigPath = Path.Combine(repositoryRootPath, FileSystemRepositoryConfigurationBuilder.REPOSITORY_CONFIG_FILENAME);

            return File.Exists(absoluteRepositoryConfigPath);
        }
        ...
        private string ComputeFileNameHash(string value, int hashLength)
        {
            ...
            return CryptoHelper.BytesToHexa(data, hashLength);
        }
