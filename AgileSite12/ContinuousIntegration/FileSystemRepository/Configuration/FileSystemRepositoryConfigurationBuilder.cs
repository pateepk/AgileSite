...
namespace CMS.ContinuousIntegration.Internal
{
    ...
    public sealed class FileSystemRepositoryConfigurationBuilder : IFileSystemRepositoryConfigurationBuilder
    {
        ...
        internal const string DEFAULT_REPOSITORY_ROOT_PATH = @"App_Data\CIRepository";
        internal const string REPOSITORY_CONFIG_FILENAME = "repository.config";
        ...
        /// </summary>
        /// <param name="repositoryRootPath">Relative path to repository root in the main application. Default repository path (<see cref="DEFAULT_REPOSITORY_ROOT_PATH"/>) is used if not provided.</param>
        ...
        {
            mRepositoryRootPath = GetRepositoryRootPath(relativeRepositoryPath: repositoryRootPath);
        ...
        /// </summary>
        /// <param name="applicationPath">Path to the file system application root. <see cref="SystemContext.WebApplicationPhysicalPath"/> is used if not provided.</param>
        /// <param name="relativeRepositoryPath">Relative path to repository root in the main application. Default repository path (<see cref="DEFAULT_REPOSITORY_ROOT_PATH"/>) is used if not provided.</param>
        internal static string GetRepositoryRootPath(string applicationPath = null, string relativeRepositoryPath = null)
        {
            var customPath = CoreServices.AppSettings["CMSCIRepositoryPath"];
            if (!String.IsNullOrEmpty(customPath))
            {
                return Path.IsPathRooted(customPath) ? customPath : Path.Combine(applicationPath ?? SystemContext.WebApplicationPhysicalPath, customPath);
            }

            if (String.IsNullOrEmpty(relativeRepositoryPath))
            {
                relativeRepositoryPath = DEFAULT_REPOSITORY_ROOT_PATH;
            }

            return Path.EnsureEndBackslash(Path.Combine(applicationPath ?? SystemContext.WebApplicationPhysicalPath, relativeRepositoryPath));
        }
        ...
        private FileSystemRepositoryConfiguration LoadConfigurationFromFileOrInfos()
        {
            ...
            var customTableObjectTypes = GetCustomTableObjectTypes(configFile);

            var allObjectTypes = objectTypes.Union(customTableObjectTypes);
