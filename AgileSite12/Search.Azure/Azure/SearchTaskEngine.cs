    ...
    internal class SearchTaskEngine : ISearchTaskEngine
    {
        ...
        private void ExecuteProcessTask(SearchTaskAzureInfo task)
        {
            ...
            foreach (var indexInfo in indexes ?? Enumerable.Empty<SearchIndexInfo>())
            {
                ...
                UpdateIndex(indexInfo, new[] { searchable }, (searchableObj, fields) =>
                {
                    ...
                    var azureDoc = DocumentCreator.Instance.CreateDocument(document, searchableObj, indexInfo, fields);
        ...
        private void ProcessDocumentCategoryChange(SearchTaskAzureInfo task)
        {
            ...
            foreach (var index in indexes)
            {
                ...
                updatedDocumentIds.AddRangeToSet(documentIds);
            }
        }
        ...
        private void ExecuteUpdateOnIndex(SearchIndexInfo indexInfo, IEnumerable<ISearchable> searchables, DateTime taskCreated)
        {
            ...
            UpdateIndex(indexInfo, searchables, (searchableObj, fields) =>
            {
                var document = searchableObj.GetSearchDocument(indexInfo);
        ...
        private static Dictionary<SearchIndexInfo, IEnumerable<ISearchable>> GetIndexesWithSearchableObjectsForPartialRebuild(string objectType, string siteNodeIdentifier, SearchablesRetriever searchableRetriever)
        {
            ...
            return searchableRetriever.GetRelevantIndexes(objectType, siteNode.SiteName, SearchIndexInfo.AZURE_SEARCH_PROVIDER)
                                 .ToDictionary(index => index, index => searchableRetriever.GetSearchableObjects(index, siteNode.NodeAliasPath, siteNode.SiteName));
        }
        ...
        private void Rebuild(SearchIndexInfo indexInfo)
        {
            ...
            try
            {
                ...
                UpdateIndex(indexInfo, SearchablesRetrievers.Get(indexInfo.IndexType).GetSearchableObjects(indexInfo), (searchableObj, fields) =>
                {
                    var azureDoc = DocumentCreator.Instance.CreateDocument(searchableObj.GetSearchDocument(indexInfo), searchableObj, indexInfo, fields);
                    return IndexAction.MergeOrUpload(azureDoc);
                });
        ...
        private void UpdateIndex(SearchIndexInfo indexInfo, IEnumerable<ISearchable> searchables, Func<ISearchable, IEnumerable<Field>, IndexAction> transformSearchableToIndexAction, Index existingIndex = null)
        {
            var searchServiceManager = CreateSearchServiceManager(SearchService.FromAdminApiKey(indexInfo.IndexSearchServiceName, indexInfo.IndexAdminKey));
            ...
            using (var searchablesEnumerator = searchables.GetEnumerator())
            {
                ...
                while (exists)
                {
                    ...
                    var fields = searchable == null ? null : DocumentFieldCreator.Instance.CreateFields(searchable, indexInfo);

                    var indexAction = transformSearchableToIndexAction(searchable, fields);
                    ...
                    if (indexAction.ActionType != IndexActionType.Delete)
                    {
                        ...
                        var usedFields = fields.Where(f => indexAction.Document.Keys.Contains(f.Name, StringComparer.OrdinalIgnoreCase));
                        updateIndex = MergeFields(usedFields, azureIndexFields) || updateIndex;
                    }
